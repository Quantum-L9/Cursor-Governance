...
emit_bootstrap_status() {
  ...
  # Generate, then read THAT receipt. Every bootstrap ceremony generates its
  # own receipt by running the installer, and the reader is bound to it by a
  # ceremony id the installer stamps into the file. The previous design read
  # whatever receipt was on disk and ran the installer only as a repair, once
  # per governance revision and only for a non-ready verdict, AFTER printing:
  # a session reported a receipt written by an earlier session as its own
  # ("ready … 1056s ago"), and a session that did repair printed the expired
  # verdict it had just replaced ("receipt expired (303728s old)") and never
  # read the receipt its own installer wrote. Neither is a bootstrap receipt.
  #
  # Budget: the installer is clamped to what is LEFT of this hook's budget
  # minus a floor kept for the reporting below, so generation can never starve
  # the must-emit lines (SESSION_START_SPEC). When the budget cannot cover a
  # generation it is NOT started, and the bound reader then says the receipt was
  # not generated by this bootstrap — the truth, instead of an older verdict.
  # Fail-open throughout: a generation that cannot run degrades the session, it
  # never blocks it.
  local revision installer ceremony_id gen_log
  ...
  ceremony_id="${_L9_HOOK_START}-$$-${RANDOM}"
  gen_log="$HOME/.l9/claude/bootstrap-${revision}.log"
  if [ ! -f "$installer" ]; then
    say "bootstrap receipt: NOT GENERATED — installer absent at $installer"
  else
    mkdir -p "$HOME/.l9/claude"
    _gen_left=$(( $(_l9_budget_left) - ${L9_BOOTSTRAP_REPORT_FLOOR:-3} ))
    ...
    if ! type run_with_timeout >/dev/null 2>&1; then
      ...
      env L9_BOOTSTRAP_ID="$ceremony_id" L9_BOOTSTRAP_LOG_PATH="$gen_log" \
      bash "$installer" --governance "$GOV" --workspace "$WORKSPACE" \
      >"$gen_log" 2>&1; then
      say "bootstrap receipt: generated this bootstrap (id $ceremony_id, ${revision:0:8})"
    else
      _gen_rc=$?
      _gen_how="$(head -n 3 "$gen_log" 2>/dev/null | tr '\n' ' ')"
...
try:
  ...
  block="$("$py" "$reader" --read --reprobe --bootstrap-id "$ceremony_id" 2>/dev/null || true)"
  if [ -n "$block" ]; then
