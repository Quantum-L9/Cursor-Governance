...
if GOV=$(resolve_governance_dir); then
  ...
  # --- Bootstrap receipt generation (every ceremony) ------------------------
  # Every bootstrap ceremony generates its own receipt by running the adapter
  # installer, and emit_bootstrap_status reads back THAT receipt, bound by the
  # ceremony id the installer stamps into it. The installer runs the same
  # projection engine as the standalone step below, so when it runs it IS this
  # ceremony's projection: running both spent a cold projection (~8 s measured)
  # twice and left too little budget to generate the receipt at all.
  #
  # The previous design read whatever receipt was on disk and ran the installer
  # only as a repair — for a non-ready verdict, once per governance revision,
  # after printing. A session reported an earlier session's receipt as its own
  # ("ready … 1056s ago") while generating nothing, and a session that did
  # repair printed the expired verdict it had just replaced and never read the
  # receipt its own installer wrote.
  #
  # Budget: clamped to what is LEFT minus a floor kept for the must-emit lines
  # (SESSION_START_SPEC), so generation never starves them. When the budget
  # cannot cover it the installer is NOT started, and the bound reader then
  # says the receipt was not generated by this bootstrap — the truth, instead
  # of an older verdict. Fail-open: it degrades the session, never blocks it.
  _L9_CEREMONY_ID="${_L9_HOOK_START}-$$-${RANDOM}"
  BOOTSTRAP_INSTALLER="$GOV/environment/agents/adapters/claude-code/install.sh"
  if [ -f "$BOOTSTRAP_INSTALLER" ]; then
    _gen_rev="$(git -C "$GOV" rev-parse HEAD 2>/dev/null || echo unknown)"
    _gen_log="$HOME/.l9/claude/bootstrap-${_gen_rev}.log"
    mkdir -p "$HOME/.l9/claude"
    # Claude Code reads settings ONCE, before any hook runs — including the
    # governance refresh this very SessionStart performs. A container cached
    # at an older revision therefore runs this session under that revision's
    # budget: 2026-09-24 a cache made at b575601 (30 s, the #652 regression)
    # clamped the installer to 30-2-4-2-1-6 = 15 s although a29657e, fetched
    # seconds earlier, registers 60 s. Say which budget is in force and why.
    _gen_want="$(sed -n 's/.*"L9_SESSION_START_BUDGET": *"\([0-9][0-9]*\)".*/\1/p' \
      "$GOV/environment/agents/adapters/claude-code/settings.template.json" 2>/dev/null | head -n 1)"
    if [ -n "$_gen_want" ] && [ "${L9_SESSION_START_BUDGET:-60}" != "$_gen_want" ]; then
      say "SessionStart budget: ${L9_SESSION_START_BUDGET:-60}s loaded at session start; governance ${_gen_rev:0:8} registers ${_gen_want}s — settings predate this session's governance refresh and are read once, so ${_gen_want}s applies from the next session"
    fi
    _gen_left=$(( $(_l9_budget_left) - ${L9_BOOTSTRAP_REPORT_FLOOR:-6} ))
    ...
    if [ "$_gen_cap" -ge "${L9_BOOTSTRAP_GENERATE_MIN:-15}" ] \
      ...
      # the process group the deadline tears down), with no fd on this hook's
      ...
      if kill -0 "$_gen_pid" 2>/dev/null; then
        ...
        say "bootstrap receipt: installer still running after ${_gen_cap}s (hook budget ${L9_SESSION_START_BUDGET:-60}s) — NOT killed; it finishes detached and writes ~/.l9/claude/bootstrap-state.json (log $_gen_log) — last: ${_gen_how:-no log bytes}"
      else
        _gen_rc=0
    ...
    elif ! type run_with_timeout >/dev/null 2>&1; then
  ...
  elif [ "${L9_SKIP_SESSION_PROJECTION:-}" != "1" ] \
...
emit_bootstrap_status() {
  ...
  # Read back the receipt THIS ceremony generated (see "Bootstrap receipt
  # generation" above), bound by its ceremony id. A receipt carrying any other
