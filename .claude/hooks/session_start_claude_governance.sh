...
# Order: explicit override -> the workspace itself (the governance repo IS the
# clone when a session runs directly against Cursor-Governance) -> conventional
# CLI clone -> a clone dropped by web/setup.sh into $HOME. On Web/Mobile the setup
# script is what puts it there; the workspace candidate lets the producer repo
# self-activate with no separate clone. Consumer repos lack CANONICAL_LAW.md, so
# they correctly fall through to their $HOME clone.
