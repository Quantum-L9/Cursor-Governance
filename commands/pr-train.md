...
1. **OPEN_TRAIN** — unique commits on the **current branch** (not every local ref). Same-path, generated-prefix clobber, and `git merge-tree` conflict stay one PR (unknown probe fail-closes). Commits that conflict with the stack tip are skipped, not cherry-picked. Then remediator publish (`git push` of the extract + `gh pr create` if needed). Do not run `make pr`.
