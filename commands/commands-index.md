...
| `/readme` | Generate subsystem READMEs via DAG pipeline | After adding modules, README refresh |
...
**What it does:** Systematically generates and validates subsystem READMEs through a DAG-enforced 6-phase pipeline.

**When to use:**
- After adding new modules or subsystems
- Periodic README refresh (ensuring coverage)
- After major refactoring that changes module structure

**The pipeline:**
1. Gap analysis — find missing subsystems
2. Config enrichment — add entries to readme_config.yaml
3. Template update — enhance README template if needed
4. Generate — run the generator script
5. Validate — verify output quality
6. Report — summarize results

**Think of it as:** A documentation factory that ensures every subsystem has a proper README.

**Example:** `/readme --tier core` regenerates all core tier READMEs.
...
| "New module needs README" | Run `/readme --subsystem modulename` |
| "READMEs out of date" | Run `/readme` for full pipeline regeneration |
