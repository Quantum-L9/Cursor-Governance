...
FILE_EMMA_RE = re.compile(
    r"^File:\s*Emma/(.+?)(?:\s*\((UPDATED FULL|REPLACEMENT|FULL REWRITE)[^)]*\))?\s*$"
)
...
class HarvestExecutor:
            ...
            if line.startswith("```") and not in_code_block:
                ...
                if code_lang in ("python", "py"):
                    ...
                    for j in range(max(0, code_start - 5), code_start):
                        ...
                        match = re.search(r"`([^`]+\.py)`|[*]{2}([^*]+\.py)[*]{2}", prev_line)
                        if match:
    ...
    def _parse_file_marker_blocks(self, lines: list[str]) -> list[dict]:
        ...
        while i < len(lines):
            ...
            if (
                "CANONICAL" in path
                or "FILE TREE" in path
                or ("/" not in path and "." not in path and len(path) > 30)
            ):
                i += 1
                continue
    ...
    def _parse_phase2_sprint_blocks(self, lines: list[str]) -> list[dict]:
        ...
        while i < len(lines):
            ...
            if not path:
                for marker in PHASE2_PATH_MARKERS:
                    if marker in line:
                        ...
                        elif "0001_init_core" in segment or (
                            marker == "alembic.ini" and "0001" in line
                        ):
            ...
            while end < len(lines):
                next_line = lines[end]
                if FILE_EMMA_RE.match(next_line.strip()) or any(
                    m in next_line for m in PHASE2_PATH_MARKERS
                ):
                    break
                if any(next_line.strip().startswith(p) for p in BLOCK_STOP_PHRASES):
                    break
