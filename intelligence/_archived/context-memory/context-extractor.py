...
Runs hourly to:
- Extract session context from Cursor chat exports (SQLite, not LevelDB)
- Use Bayesian probabilistic reasoning for context meaningfulness assessment
- Save meaningful contexts to indexed JSON files for cross-session memory

Integration: Uses probabilistic_engine for evidence-based assessment
"""
...
class ContextExtractor:
    ...
    def _extract_creation_summary(self, text):
        ...
        for sentence in sentences:
            if (
                "creat" in sentence.lower()
                or "build" in sentence.lower()
                or "add" in sentence.lower()
            ):
                return sentence.strip()[:100]
    ...
    def _is_meaningful_context(self, context):
        ...
        if not BAYESIAN_AVAILABLE:
            return (
                len(context["key_actions"]) > 0
                or len(context["files_modified"]) > 0
                or len(context["decisions"]) > 0
                or context["message_count"] > 3
            )
        ...
        try:
            ...
            evidence_list = [
                ...
                Evidence(
                    name="message_volume",
                    value=min(
                        context["message_count"] / 10.0, 1.0
                    ),  # Normalize: 10+ messages = high signal
                    weight=0.15,
                    confidence=1.0,
            ...
            print(
                f"  📊 Bayesian assessment: {calibrated_probability:.3f} (confidence: {confidence:.2f})"
            )
