...
class ChatLearningExtractor:
    ...
    def extract_from_conversation(
        self, conversation_text: str, conversation_id: str = None
    ...
    def _detect_patterns_in_message(
        self, message: dict, index: int, all_messages: list[dict]
    ) -> list[dict]:
        ...
        for pattern_def in self.learning_patterns:
            ...
            for match in matches:
                detection = {
                    ...
                    "context_before": all_messages[max(0, index - 1)]["content"][:200]
                    if index > 0
                    else "",
                    "context_after": all_messages[min(len(all_messages) - 1, index + 1)]["content"][
                        :200
                    ]
    ...
    def _generate_learning_pattern(
        self, detection: dict, conversation_id: str
    ...
    def update_meta_learning_log(self, patterns: list[LearningPattern]) -> bool:
        ...
        try:
            ...
            if self.meta_learning_log.exists():
                with open(self.meta_learning_log) as f:
                    content = f.read()
            else:
                content = ""
    ...
    def process_current_conversation(self, conversation_text: str = None) -> list[LearningPattern]:
        ...
        if patterns:
            ...
            if success:
                ...
                for pattern in patterns:
                    print(
                        f"   - {pattern.learning_summary} (confidence: {pattern.confidence_score:.2f})"
                    )
            else:
