...
class FeedbackCollector:
    ...
    def _parse_feedback(self, feedback_text: str) -> FeedbackType:
        ...
        if any(word in text_lower for word in ["correct", "right", "good", "fine", "yes"]):
            ...
            word in text_lower for word in ["too strict", "too harsh", "unnecessary", "false alarm"]
        ):
            return FeedbackType.TOO_STRICT
        elif any(
            word in text_lower
            for word in ["too lenient", "should have blocked", "missed", "should have warned"]
        ):
            return FeedbackType.TOO_LENIENT
        else:
            return FeedbackType.UNCLEAR

    def _infer_feedback_from_behavior(self, user_action: str, context: dict) -> FeedbackType | None:
        """Infer feedback from user behavior"""
