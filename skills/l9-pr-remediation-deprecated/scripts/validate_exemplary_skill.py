...
def _scan_skill_md(path: Path) -> list[str]:
    ...
    for required in ("references/autonomy-controller.md", "references/ci-signal-boundary.md", "references/terminal-escalation.md", "scripts/validate_autonomy_contract.py", "scripts/render_ci_pipeline_issues.py", "scripts/package_run_deliverable.py"):
        if not (path / required).exists():
            errors.append(f"missing {required} (required for autonomous tier)")
