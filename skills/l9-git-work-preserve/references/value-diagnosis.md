...
`triage_preserved_refs.py` implements this split over the refs `/ff` parked:
`patch_id` refs go to its `superseded` bucket, `content_superset` refs to
`review`, which is printed and never offered a force-delete command. Triage
itself deletes nothing — see `triage-handoff.md`.
