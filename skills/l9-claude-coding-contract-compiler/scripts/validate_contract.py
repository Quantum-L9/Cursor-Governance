...
def _load(path):
    ...
    if path.endswith((".yaml", ".yml")):
        try:
            ...
            print("PyYAML required for YAML contracts", file=sys.stderr)
            sys.exit(2)
...
def check_read_only_authority(contract, fit):
    ...
    for i, item in enumerate(ro):
        if not isinstance(item, dict):
            fit.append(f"read_only_authority[{i}]: must be object")
            continue
...
def check_git_workflow(contract, fit):
    ...
    if git.get("commit_command") != expected_commit:
        fit.append(
            "git_workflow.commit_command must be the compiler-derived exact one-commit command"
        )
    proof = git.get("completion_proof")
    gate = contract.get("commit_gate", {}).get("required_before_commit", [])
    if not proof or proof not in gate:
        fit.append(
            "git_workflow.completion_proof must be present in commit_gate.required_before_commit"
        )
...
def main(argv):
    ...
    if red:
        for r in red:
            dpk_msgs.append(f"DPK red-line (score=0): {r}")
