...
def fits_one(m: dict, cfg: dict) -> tuple[bool, list[str]]:
    reasons = []
    if m.get("new_files", 0) > cfg["max_new_files"]:
        reasons.append(f"new_files={m['new_files']} > {cfg['max_new_files']}")
    if m.get("commits", 0) > cfg["max_commits"]:
        reasons.append(f"commits={m['commits']} > {cfg['max_commits']}")
    if m.get("matrix_cases", 0) > cfg["max_matrix_cases"]:
        reasons.append(f"matrix_cases={m['matrix_cases']} > {cfg['max_matrix_cases']}")
    if m.get("deliverables", 0) > cfg["max_deliverables"]:
        reasons.append(f"deliverables={m['deliverables']} > {cfg['max_deliverables']}")
...
def main():
    ...
    ap.add_argument(
        "--groups", required=True, help="JSON file: list of commit groups e.g. [[1],[2,3],[4,5]]"
    )
