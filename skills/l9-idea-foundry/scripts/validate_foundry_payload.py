...
def sequence(
    value: object, label: str, failures: list[str], *, nonempty: bool = False
) -> list[Any]:
...
def validate_authority(authority: dict[str, Any], failures: list[str]) -> None:
    ...
    for i, raw in enumerate(claims):
        ...
        sequence(
            ...
            f"AUTHORITY_MAP.claims[{i}].source_refs",
            failures,
            nonempty=True,
        )
...
def validate_blueprint(blueprint: dict[str, Any], failures: list[str]) -> dict[str, Any]:
    ...
    nonempty_string(
        identity.get("repository"), "IMPLEMENTATION_BLUEPRINT.identity.repository", failures
    )
    ...
    nonempty_string(
        objective.get("product_thesis"),
        "IMPLEMENTATION_BLUEPRINT.objective.product_thesis",
        failures,
    )
    nonempty_string(
    ...
    compilation = mapping(
        blueprint.get("compilation"), "IMPLEMENTATION_BLUEPRINT.compilation", failures
    )
    fail_if(
        compilation.get("ingress_role") != "PRE_CODE_SSOT",
        "compilation.ingress_role must be PRE_CODE_SSOT",
        failures,
    )
    fail_if(
    ...
    for i, raw in enumerate(reuse):
        ...
        if disposition not in REUSE_DISPOSITIONS:
            failures.append(
                f"reuse_map[{i}].disposition must be one of {sorted(REUSE_DISPOSITIONS)}"
            )
    ...
    acceptance = mapping(
        blueprint.get("acceptance"), "IMPLEMENTATION_BLUEPRINT.acceptance", failures
    )
    sequence(acceptance.get("path"), "acceptance.path", failures, nonempty=True)
    sequence(
        acceptance.get("evidence_required"), "acceptance.evidence_required", failures, nonempty=True
    )
    sequence(
        blueprint.get("validation_obligations"), "validation_obligations", failures, nonempty=True
    )

    planning = mapping(blueprint.get("planning"), "IMPLEMENTATION_BLUEPRINT.planning", failures)
    fail_if(
        planning.get("owner") != "l9-plan-simple", "planning.owner must be l9-plan-simple", failures
    )
    nonempty_string(planning.get("plan_document_ref"), "planning.plan_document_ref", failures)
    fail_if(
        not valid_sha256(planning.get("plan_digest")),
        "planning.plan_digest must be sha256:<64 lowercase hex>",
        failures,
    )
    fail_if(
        planning.get("validation_status") != "PASSED",
        "planning.validation_status must be PASSED",
        failures,
    )
    ...
    if handoff == "EMBEDDED":
        fail_if(
            ...
            "first-class EMBEDDED handoff cannot be marked compatibility fallback",
            failures,
        )
        nonempty_string(planning.get("mode_evidence_ref"), "planning.mode_evidence_ref", failures)
    if handoff == "EMBEDDED_PRE_BIRTH":
        fail_if(
            fallback is not True,
            "legacy EMBEDDED_PRE_BIRTH must set compatibility_fallback: true",
            failures,
        )
    ...
    for name in ("direction", "constellation_alignment", "first_order"):
        ...
        sequence(
            ...
            f"architecture_questions.{name}.evidence_refs",
            failures,
            nonempty=True,
        )
...
def validate_traceability(traceability: dict[str, Any], failures: list[str]) -> None:
    capabilities = sequence(
        traceability.get("capabilities"), "TRACEABILITY.capabilities", failures, nonempty=True
    )
    seen: set[str] = set()
    for i, raw in enumerate(capabilities):
        ...
        if status not in TRACE_STATUSES:
            failures.append(
                f"TRACEABILITY.capabilities[{i}].status must be one of {sorted(TRACE_STATUSES)}"
            )
        requirement_refs = sequence(
            item.get("requirement_refs"),
            f"TRACEABILITY.capabilities[{i}].requirement_refs",
            failures,
        )
        sequence(
            item.get("architecture_refs"),
            f"TRACEABILITY.capabilities[{i}].architecture_refs",
            failures,
        )
        sequence(item.get("harvest_refs"), f"TRACEABILITY.capabilities[{i}].harvest_refs", failures)
        plan_refs = sequence(
            item.get("plan_todo_refs"), f"TRACEABILITY.capabilities[{i}].plan_todo_refs", failures
        )
        impl_paths = sequence(
            item.get("implementation_paths"),
            f"TRACEABILITY.capabilities[{i}].implementation_paths",
            failures,
        )
        evidence_refs = sequence(
            item.get("evidence_refs"), f"TRACEABILITY.capabilities[{i}].evidence_refs", failures
        )
        sequence(item.get("unknown_ids"), f"TRACEABILITY.capabilities[{i}].unknown_ids", failures)
        if status == "IMPLEMENTED":
            if not requirement_refs:
                failures.append(
                    f"TRACEABILITY capability {cid or i} IMPLEMENTED without requirement_refs"
                )
            if not plan_refs:
                failures.append(
                    f"TRACEABILITY capability {cid or i} IMPLEMENTED without plan_todo_refs"
                )
            if not impl_paths:
                failures.append(
                    f"TRACEABILITY capability {cid or i} IMPLEMENTED without implementation_paths"
                )
            if not evidence_refs:
                failures.append(
                    f"TRACEABILITY capability {cid or i} IMPLEMENTED without evidence_refs"
                )
    decisions = sequence(
        traceability.get("implementation_decisions"),
        "TRACEABILITY.implementation_decisions",
        failures,
    )
    for i, raw in enumerate(decisions):
        ...
        sequence(
            ...
            f"implementation_decisions[{i}].affected_paths",
            failures,
            nonempty=True,
        )
...
def validate_receipt(
    receipt: dict[str, Any], blueprint_planning: dict[str, Any], failures: list[str]
) -> dict[str, Any]:
    ...
    fail_if(
        not valid_sha256(source.get("inventory_digest")),
        "FOUNDRY_RECEIPT source.inventory_digest invalid",
        failures,
    )
    ...
    fail_if(
        planning.get("owner") != "l9-plan-simple",
        "receipt planning.owner must be l9-plan-simple",
        failures,
    )
    fail_if(
        planning.get("validation_status") != "PASSED",
        "receipt planning.validation_status must be PASSED",
        failures,
    )
    fail_if(
        planning.get("plan_handoff") not in PLAN_HANDOFFS,
        f"receipt planning.plan_handoff must be one of {sorted(PLAN_HANDOFFS)}",
        failures,
    )
    fail_if(
        not valid_sha256(planning.get("plan_digest")),
        "receipt planning.plan_digest invalid",
        failures,
    )
    nonempty_string(
        planning.get("plan_document_ref"), "receipt planning.plan_document_ref", failures
    )
    if blueprint_planning:
        for field in ("plan_document_ref", "plan_digest", "validation_status", "plan_handoff"):
            if planning.get(field) != blueprint_planning.get(field):
                failures.append(
                    f"receipt planning.{field} does not match blueprint planning.{field}"
                )

    payload = mapping(receipt.get("payload"), "FOUNDRY_RECEIPT.payload", failures)
    fail_if(
        payload.get("freeze_binding") != "EXTERNAL_RECEIPT",
        "FOUNDRY_RECEIPT payload.freeze_binding must be EXTERNAL_RECEIPT",
        failures,
    )
    fail_if(
        payload.get("resume_index_ref") != "docs/idea-origin/FOUNDRY_INDEX.json",
        "FOUNDRY_RECEIPT payload.resume_index_ref must point to "
        "docs/idea-origin/FOUNDRY_INDEX.json",
        failures,
    )
    ...
    fail_if(
        deployment.get("performed") is not False,
        "FOUNDRY_RECEIPT deployment.performed must be false",
        failures,
    )
...
def validate_index(
    ...
    fail_if(
        index.get("schema") != "l9.idea-foundry.index/v1", "FOUNDRY_INDEX schema mismatch", failures
    )
    source = mapping(index.get("source"), "FOUNDRY_INDEX.source", failures)
    fail_if(
        source.get("inventory_digest") != inventory_digest,
        "FOUNDRY_INDEX source inventory digest mismatch",
        failures,
    )
    compiled = mapping(index.get("compiled_intent"), "FOUNDRY_INDEX.compiled_intent", failures)
    fail_if(
        compiled.get("pre_code_ingress") != "docs/idea-origin/IMPLEMENTATION_BLUEPRINT.yaml",
        "FOUNDRY_INDEX pre_code_ingress mismatch",
        failures,
    )
    fail_if(
        compiled.get("raw_source_policy") != "EVIDENCE_ONLY_AFTER_ACCEPTANCE",
        "FOUNDRY_INDEX raw_source_policy mismatch",
        failures,
    )
    ...
    expected = {
        ...
        "unknown_register": (
            root / "docs/idea-origin/UNKNOWN_REGISTER.md",
            sha256_file(root / "docs/idea-origin/UNKNOWN_REGISTER.md"),
        ),
    ...
    for name, (path, digest) in expected.items():
        ...
        fail_if(
            entry.get("path") != path.relative_to(root).as_posix(),
            f"FOUNDRY_INDEX {name} path mismatch",
            failures,
        )
    ...
    fail_if(
        lineage.get("inventory_digest") != inventory_digest,
        "FOUNDRY_INDEX lineage inventory mismatch",
        failures,
    )
    fail_if(
        lineage.get("authority_digest") != expected["authority_map"][1],
        "FOUNDRY_INDEX authority_digest mismatch",
        failures,
    )
    fail_if(
        lineage.get("blueprint_digest") != expected["implementation_blueprint"][1],
        "FOUNDRY_INDEX blueprint_digest mismatch",
        failures,
    )
    fail_if(
        lineage.get("traceability_digest") != expected["traceability"][1],
        "FOUNDRY_INDEX traceability_digest mismatch",
        failures,
    )
    fail_if(
        lineage.get("plan_digest") != receipt_planning.get("plan_digest"),
        "FOUNDRY_INDEX plan_digest mismatch",
        failures,
    )
    resume = mapping(index.get("resume"), "FOUNDRY_INDEX.resume", failures)
    fail_if(
        resume.get("entrypoint") != "docs/idea-origin/FOUNDRY_INDEX.json",
        "FOUNDRY_INDEX resume.entrypoint mismatch",
        failures,
    )
    fail_if(
        resume.get("repair_policy") != "EARLIEST_INVALID_LAYER",
        "FOUNDRY_INDEX repair_policy mismatch",
        failures,
    )
    deployment = mapping(index.get("deployment"), "FOUNDRY_INDEX.deployment", failures)
    fail_if(
        deployment.get("performed") is not False,
        "FOUNDRY_INDEX deployment.performed must be false",
        failures,
    )
...
def main() -> int:
    ...
    compilation = (
        blueprint.get("compilation") if isinstance(blueprint.get("compilation"), dict) else {}
    )
    ...
    if compilation.get("source_inventory_digest") != inventory_digest:
        failures.append(
            "blueprint compilation source_inventory_digest does not match "
            "receipt source.inventory_digest"
        )
    ...
    for path in test_files:
        ...
        try:
            ...
            continue
        if (
            re.search(r"\bdef\s+test_", text)
            or "unittest.TestCase" in text
            or re.search(r"\b(it|test)\s*\(", text)
        ):
    ...
    for path in code_files + test_files:
        ...
        for pattern in PLACEHOLDER_PATTERNS:
            if pattern.search(text):
                failures.append(
                    f"incomplete marker {pattern.pattern!r} in {path.relative_to(root)}"
                )
                break
        ...
        if args.freeze_receipt is None:
            ...
            if freeze_path == root or root in freeze_path.parents:
                ...
                if tree_digest and freeze.get("tracked_tree_digest") != tree_digest:
                    failures.append(
                        "freeze receipt tracked_tree_digest does not match current tracked tree"
                    )
                if freeze.get("tracked_file_count") != len(records):
                    failures.append(
                        "freeze receipt tracked_file_count does not match current tracked tree"
                    )
                if freeze.get("inventory_digest") != inventory_digest:
                    failures.append(
                        "freeze receipt inventory_digest does not match Foundry source identity"
                    )
                ...
                if freeze.get("foundry_index_digest") != sha256_file(index_path):
                    failures.append(
                        "freeze receipt foundry_index_digest does not match committed index"
                    )
                if head and freeze.get("git_revision") == head:
