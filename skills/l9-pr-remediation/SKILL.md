...
| Concurrency caps | `pr_fleet.py` `skill_caps` passes through the execution profile | inventing a remediator clamp |
...
wave_launch: merge_now_plus_remediate   # one message; execution-profile caps
...
| `scripts/activation_cases.json` | 20 labeled prompts (converge / diagnose / reject) scored by the intent-precedence rule above in `scripts/self_test.py` |
