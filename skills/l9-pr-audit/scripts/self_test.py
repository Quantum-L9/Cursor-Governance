...
def base_audit() -> dict[str, object]:
    shared = [
        evidence("E-AUTH", "SOURCE", A, "canonical rule exists", ["authority_resolution"]),
        evidence(
            "E-GUARD",
            "CONTRACT",
            A,
        ...
        evidence(
            ...
            ["implementation_surface", "behavior_mismatch"],
            ["F1"],
        ),
        evidence(
            "E-TEST",
        ...
        "findings": [
            {
                ...
                "proof_of_mismatch": (
                    "Diff evidence plus targeted failing test proves the mismatch."
                ),
                "impact": "Merge would ship incorrect behavior.",
                "root_cause": "The implementation takes the wrong branch.",
                "root_cause_state": "CONFIRMED",
                "behavioral_closure_condition": (
                    "The correct branch is selected for the governed input."
                ),
                "closing_validation": [
                    {
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        manifest["files"]["FABLE_REMEDIATION.md"] = {
            "sha256": bab.sha256_bytes(payloads["FABLE_REMEDIATION.md"]),
            "bytes": len(payloads["FABLE_REMEDIATION.md"]),
        }
        ...
        with zipfile.ZipFile(projection_zip, "w", zipfile.ZIP_DEFLATED) as target:
            for name, payload in payloads.items():
                target.writestr(name, payload)
        assert_has(
            bab.verify_zip(projection_zip), "derived projection mismatch: FABLE_REMEDIATION.md"
        )
