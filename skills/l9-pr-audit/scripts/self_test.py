...
def synthetic_change_ledger() -> dict[str, object]:
    snapshot = {
        ...
        "files": [
            ...
            {
                "path": "tests/test_core.py", "status": "modified", "additions": 2, "deletions": 1,
                "patch": "@@ -1,2 +1,2 @@\n-def test_rule():\n+def test_rule():\n+    assert True",
            },
        ],
...
def sync_red_team(audit: dict[str, object]) -> None:
    ...
    objective_status = {item["objective_id"]: item for item in audit["change_discipline"]["objective_closure"]}
    ...
    for claim in claims.values():
        ...
        if claim["claim_kind"] == "OBJECTIVE":
            ...
            claim["status"] = {"SATISFIED":"SUPPORTED","UNPROVEN":"UNKNOWN","NOT_IMPLEMENTED":"REFUTED","CONFLICTED":"REFUTED"}[row["status"]]
            finding_ids = list(row["finding_ids"])
        elif claim["claim_kind"] == "AUDIT_DOMAIN":
            row = domain_status[claim["subject"]]
            claim["status"] = {"PASS":"SUPPORTED","FAIL":"REFUTED","NOT_APPLICABLE":"NOT_APPLICABLE","UNKNOWN":"UNKNOWN"}[row["status"]]
            finding_ids = list(row["finding_ids"])
        elif claim["claim_kind"] == "CHANGED_SYMBOL":
            sid = next((item.get("machine_symbol_id") for item in synthetic_change_ledger()["claim_seeds"] if item["claim_id"] == claim["claim_id"]), None)
            row = symbols[sid]
            finding_ids = list(row["finding_ids"])
            claim["status"] = "REFUTED" if finding_ids else "UNKNOWN" if row["scope_disposition"] == "UNKNOWN" else "SUPPORTED"
        elif claim["claim_kind"] == "READINESS":
            num = claim["pr_numbers"][0]
            state = pr_verdict[num]["merge_readiness"]
            claim["status"] = "SUPPORTED" if state in bab.READY_STATES else "REFUTED" if state == "NOT_READY" else "UNKNOWN"
            ...
            claim["status"] = "SUPPORTED" if state == "CONVERGED" else "REFUTED" if state == "NOT_CONVERGED" else "UNKNOWN"
    ...
    for probe in audit["falsification_ledger"]:
        ...
        probe["result"] = {"SUPPORTED":"SURVIVED","REFUTED":"FALSIFIED","NOT_APPLICABLE":"NOT_APPLICABLE","UNKNOWN":"INCONCLUSIVE"}[claim["status"]]
...
def populate_v18_closures(audit: dict[str, object], ledger: dict[str, object]) -> None:
    ...
    for seed in ledger.get("closure_seeds", []):
        ...
        if kind == "DIFF_HUNK":
            ...
            rows.append({
                "closure_id": seed["closure_id"], "pr_number": seed["pr_number"], "closure_kind": kind,
                "subject": seed["subject"], "source": "MACHINE_SEEDED", "seed_hash": seed["seed_hash"],
                "status": "FINDING" if core else "PASS",
                "evidence_ids": ["E-FIND" if core else "E-TEST"], "finding_ids": ["F1"] if core else [],
                "details": {"disposition": "FINDING" if core else "VALIDATION_REQUIRED", "objective_ids": ["O1"]},
            })
        elif kind == "CI_CAUSALITY":
            rows.append({
                "closure_id": seed["closure_id"], "pr_number": seed["pr_number"], "closure_kind": kind,
                "subject": seed["subject"], "source": "MACHINE_SEEDED", "seed_hash": seed["seed_hash"],
                "status": "FINDING", "evidence_ids": ["E-TEST"], "finding_ids": ["F1"],
    ...
    audit["post_judgment_closure"] = {
        ...
        "severity_consistency": [{
            "finding_id": "F1", "machine_floor": "High", "machine_ceiling": "Critical",
            "actual_severity": "High", "status": "CONSISTENT", "evidence_ids": ["E-AUTH"],
        }],
        "validation_run_coverage": [{
            "evidence_id": "E-TEST", "disposition": "JUSTIFIED_NON_CLOSING", "claim_ids": [],
...
def base_audit() -> dict[str, object]:
    ...
    red_properties = [f"claim:{cid}" for cid in machine_claim_ids] + [f"claim_validation:{cid}" for cid in machine_claim_ids] + [f"falsification:{cid}" for cid in machine_claim_ids] + ["audit_convergence"]
    shared = [
        evidence("E-AUTH", "SOURCE", A, "canonical rule exists", ["authority_resolution", "merge_blocking_basis", "audit_domain:INTENT_SCOPE", "audit_domain:OWNERSHIP_AUTHORITY", "audit_domain:LEVERAGE_SIMPLICITY", "audit_domain:CHANGE_DISCIPLINE"]),
        evidence("E-GUARD", "CONTRACT", A, "repository mutation guard resolved", ["mutation_guard_resolution"]),
        evidence("E-VCMD", "CONTRACT", A, "repository validation command is authoritative", ["validation_procedure"]),
        evidence("E-CHECK", "CONFIGURATION", B, "required check identity resolved", ["required_check_identity", "audit_domain:TESTING_VALIDATION"]),
        evidence("E-REVIEW", "REVIEW_THREAD", B, "zero unresolved review threads observed", ["review_thread_coverage"]),
        evidence(
            "E-BYPASS", "DIFF", B, "diff inspected for weakening surfaces",
            ["anti_bypass_coverage"] + [f"anti_bypass:{kind}" for kind in sorted(bab.ANTI_BYPASS_KINDS)],
        ),
        evidence("E-FIND", "DIFF", B, "src/core.py violates canonical rule", ["implementation_surface", "behavior_mismatch", "finding_origin"], ["F1"]),
        evidence(
            "E-TEST", "TEST", B, "targeted test fails before remediation", ["closure_property", "mandatory_validation"], ["F1"],
            source_head_sha=B, tested_revision_sha=B, validation_result="FAIL", path_or_check="targeted-unit",
        ),
        evidence("E-RED", "DIFF", B, "claim-specific validation and adversarial probes were executed against the synthetic fixture", red_properties),
    ]
    bypass_checks = [
    ...
    audit = {
        ...
        "audit_coverage": {
            ...
            "domain_assessments": [
                {"domain":"INTENT_SCOPE","status":"FAIL","evidence_ids":["E-AUTH"],"finding_ids":["F1"],"closing_validation":None},
                {"domain":"COMMUNICATION_CONTRACTS","status":"NOT_APPLICABLE","evidence_ids":["E-AUTH"],"finding_ids":[],"closing_validation":None},
                {"domain":"ROUTING_INTEGRATION","status":"NOT_APPLICABLE","evidence_ids":["E-AUTH"],"finding_ids":[],"closing_validation":None},
                {"domain":"OWNERSHIP_AUTHORITY","status":"FAIL","evidence_ids":["E-AUTH"],"finding_ids":["F1"],"closing_validation":None},
                {"domain":"STRUCTURE_SOURCE_OF_TRUTH","status":"NOT_APPLICABLE","evidence_ids":["E-AUTH"],"finding_ids":[],"closing_validation":None},
                {"domain":"SCHEMA_CONFIGURATION","status":"NOT_APPLICABLE","evidence_ids":["E-AUTH"],"finding_ids":[],"closing_validation":None},
                {"domain":"SECURITY","status":"NOT_APPLICABLE","evidence_ids":["E-AUTH"],"finding_ids":[],"closing_validation":None},
                {"domain":"RELIABILITY_OBSERVABILITY","status":"NOT_APPLICABLE","evidence_ids":["E-AUTH"],"finding_ids":[],"closing_validation":None},
                {"domain":"TESTING_VALIDATION","status":"PASS","evidence_ids":["E-CHECK"],"finding_ids":[],"closing_validation":None},
                {"domain":"LEVERAGE_SIMPLICITY","status":"PASS","evidence_ids":["E-AUTH"],"finding_ids":[],"closing_validation":None},
                {"domain":"CROSS_PR","status":"NOT_APPLICABLE","evidence_ids":["E-AUTH"],"finding_ids":[],"closing_validation":None},
                {"domain":"CHANGE_DISCIPLINE","status":"PASS","evidence_ids":["E-AUTH"],"finding_ids":[],"closing_validation":None}
            ],
            "artifact_inventory": [
                {"artifact_id":"ART-LAW","path":"CANONICAL_LAW.md","revision":A,"pr_numbers":[1],"classification":"DOCUMENTATION","roles":["AUTHORITY"],"responsibility":"canonical behavior law","evidence_ids":["E-AUTH"]},
                {"artifact_id":"ART-GUARD","path":"ops/config/mutation-guard.json","revision":A,"pr_numbers":[1],"classification":"CONFIGURATION","roles":["AUTHORITY"],"responsibility":"mutation guard authority","evidence_ids":["E-GUARD"]},
                {"artifact_id":"ART-SRC","path":"src/core.py","revision":B,"pr_numbers":[1],"classification":"SOURCE","roles":["CHANGED","COUPLED"],"responsibility":"core implementation","evidence_ids":["E-FIND"]},
                {"artifact_id":"ART-TEST","path":"tests/test_core.py","revision":B,"pr_numbers":[1],"classification":"TEST","roles":["CHANGED","VALIDATION","COUPLED"],"responsibility":"core behavior validation","evidence_ids":["E-TEST"]},
            ],
        },
        ...
        "boundary_map": {
            "status": "COMPLETE",
            "components": [
                {"component_id":"core","responsibilities":["canonical core behavior"],"authority_ids":["A1"],"evidence_ids":["E-AUTH"]},
                {"component_id":"tests","responsibilities":["behavior validation"],"authority_ids":["A1"],"evidence_ids":["E-TEST"]}
            ],
            "boundaries": [
                {"boundary_id":"B-VALIDATION","kind":"VALIDATION","owner_component_id":"core","subject":"core behavior is validated by the canonical targeted test","participants":["core","tests"],"authority_ids":["A1"],"evidence_ids":["E-AUTH","E-TEST"]}
            ],
        },
        "pr_bindings": [{
            "pr_number": 1,
            ...
            "required_check_resolution": {
                "status": "RESOLVED", "checks": ["ci"], "evidence_ids": ["E-CHECK"]
            },
            "review_thread_coverage": {
                "unresolved_discovered": 0, "classified": 0, "unresolved_remaining": 0,
                "status": "COMPLETE", "evidence_ids": ["E-REVIEW"]
            },
        }],
        "executive_verdict": {
        ...
        "remediation_surface_index": [{
            ...
            "surface_evidence": [{
                "path": "src/core.py", "role": "IMPLEMENTATION", "evidence_ids": ["E-FIND"],
        ...
        "cross_pr_evidence_pack": {"status": "NOT_APPLICABLE", "relationships": [], "merge_order": []},
        "residual_unknowns": [],
    ...
    audit["deterministic_census_binding"] = {
        ...
        "ledger_bindings": [{"pr_number": 1, "head_sha": B, "ledger_sha256": bab.sha256_bytes(ledger_bytes)}],
    }
    symbol_claim_by_machine_id = {item.get("machine_symbol_id"): item["claim_id"] for item in ledger["claim_seeds"] if item.get("machine_symbol_id")}
    ...
    for seed in ledger["claim_seeds"]:
        audit["claim_validation_matrix"].append({
            ...
            "finding_ids": [],
            "falsification_ids": [item["falsification_id"] for item in ledger["falsification_seeds"] if item["claim_id"] == seed["claim_id"]],
        })
    ...
    audit["change_discipline"] = {
        ...
        "scope_fidelity": {"changed_surfaces": [
            {"pr_number": 1, "path": "src/core.py", "disposition": "REQUIRED", "objective_ids": ["O1"], "evidence_ids": ["E-FIND"], "finding_ids": ["F1"]},
            {"pr_number": 1, "path": "tests/test_core.py", "disposition": "VALIDATION_REQUIRED", "objective_ids": ["O1"], "evidence_ids": ["E-TEST"], "finding_ids": []},
        ]},
        "complexity_delta": [{
            "pr_number": 1,
            "files_added": 0,
    ...
    audit["audit_obligation_ledger"] = [
        {"obligation_id": "OB-O1", "pr_number": 1, "kind": "OBJECTIVE", "subject": "O1", "status": "FINDING", "evidence_ids": ["E-FIND"], "finding_ids": ["F1"]},
        {"obligation_id": "OB-S1", "pr_number": 1, "kind": "CHANGED_SURFACE", "subject": "src/core.py", "status": "FINDING", "evidence_ids": ["E-FIND"], "finding_ids": ["F1"]},
        {"obligation_id": "OB-S2", "pr_number": 1, "kind": "CHANGED_SURFACE", "subject": "tests/test_core.py", "status": "PASS", "evidence_ids": ["E-TEST"], "finding_ids": []},
        {"obligation_id": "OB-T1", "pr_number": 1, "kind": "TEST_DISCRIMINATION", "subject": "TD1", "status": "PASS", "evidence_ids": ["E-TEST"], "finding_ids": []},
        {"obligation_id": "OB-CA1", "pr_number": 1, "kind": "CONTROL_ADEQUACY", "subject": "CA1", "status": "PASS", "evidence_ids": ["E-TEST"], "finding_ids": []},
        {"obligation_id": "OB-CI1", "pr_number": 1, "kind": "CI_FAILURE", "subject": "targeted-unit", "status": "FINDING", "evidence_ids": ["E-TEST"], "finding_ids": ["F1"]},
    ]
    for symbol in audit["changed_symbol_ledger"]:
        audit["audit_obligation_ledger"].append({
            "obligation_id": f"OB-{symbol['symbol_id']}", "pr_number": 1, "kind": "CHANGED_SYMBOL",
            "subject": symbol["symbol_id"], "status": "FINDING" if symbol["finding_ids"] else "PASS",
            "evidence_ids": symbol["evidence_ids"], "finding_ids": symbol["finding_ids"],
        })
    sync_red_team(audit)
    ...
    audit["audit_passes"] = [
        {
            "pass_number": 1, "pass_type": "DISCOVERY",
        ...
        {
            "pass_number": 2, "pass_type": "VERIFICATION",
...
def main() -> int:
    ...
    residue = sorted(
        ...
        for path in script_root.parent.rglob("*.pyc")
    ) + sorted(
        str(path.relative_to(script_root.parent))
    ...
    if residue:
        raise AssertionError("skill tree contains bytecode/cache residue before validation: " + ", ".join(residue))
    ...
    bad["audit_coverage"]["artifact_inventory"] = [item for item in bad["audit_coverage"]["artifact_inventory"] if item["path"] != "src/core.py"]
    assert_has(bab.validate_audit(bad), "src/core.py must appear exactly once as CHANGED in artifact_inventory")
    ...
    bad = copy.deepcopy(audit)
    bad["audit_passes"][-1]["observed_obligation_ids"] = bad["audit_passes"][-1]["observed_obligation_ids"][:-1]
    assert_has(bab.validate_audit(bad), "final VERIFICATION pass must re-observe every audit obligation")

    bad = copy.deepcopy(audit)
    bad["audit_passes"][-1]["observed_claim_ids"] = bad["audit_passes"][-1]["observed_claim_ids"][:-1]
    assert_has(bab.validate_audit(bad), "final VERIFICATION pass must re-observe every claim")

    bad = copy.deepcopy(audit)
    bad["audit_passes"][-1]["observed_falsification_ids"] = bad["audit_passes"][-1]["observed_falsification_ids"][:-1]
    assert_has(bab.validate_audit(bad), "final VERIFICATION pass must re-observe every falsification probe")

    bad = copy.deepcopy(audit)
    supported = next(item for item in bad["claim_validation_matrix"] if item["status"] == "SUPPORTED" and item["materiality"] == "MATERIAL")
    ...
    if bab.validate_audit(audit, change_ledgers=external_ledgers):
        raise AssertionError("bound deterministic census fixture should validate:\n" + "\n".join(bab.validate_audit(audit, change_ledgers=external_ledgers)))

    bad = copy.deepcopy(audit)
    bad["changed_symbol_ledger"] = bad["changed_symbol_ledger"][:-1]
    assert_has(bab.validate_audit(bad, change_ledgers=external_ledgers), "machine changed-symbol ledger must exactly match deterministic census")

    bad = copy.deepcopy(audit)
    seeded_claim_id = ledger["claim_seeds"][0]["claim_id"]
    bad["claim_validation_matrix"] = [item for item in bad["claim_validation_matrix"] if item["claim_id"] != seeded_claim_id]
    assert_has(bab.validate_audit(bad, change_ledgers=external_ledgers), "claim matrix omitted deterministic claim seeds")

    bad = copy.deepcopy(audit)
    seeded_fal_id = ledger["falsification_seeds"][0]["falsification_id"]
    bad["falsification_ledger"] = [item for item in bad["falsification_ledger"] if item["falsification_id"] != seeded_fal_id]
    assert_has(bab.validate_audit(bad, change_ledgers=external_ledgers), "falsification ledger omitted deterministic seeds")


    # v2.0 deterministic-closure regressions.
    bad = copy.deepcopy(audit)
    bad["audit_passes"][-1]["observed_closure_ids"] = bad["audit_passes"][-1]["observed_closure_ids"][:-1]
    assert_has(bab.validate_audit(bad), "final VERIFICATION pass must re-observe every deterministic closure")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"] = bad["deterministic_closure_ledger"][:-1]
    assert_has(bab.validate_audit(bad, change_ledgers=external_ledgers), "deterministic_closure_ledger missing machine seeds")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-PUBLIC", "pr_number":1, "closure_kind":"PUBLIC_CONTRACT", "subject":"public-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"PRESERVED"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-PUBLIC"]))
    assert_has(bab.validate_audit(bad), "public-contract delta requires preservation_obligation_ids")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-SSOT", "pr_number":1, "closure_kind":"SSOT_UNIQUENESS", "subject":"ssot-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"COMPETING_AUTHORITY"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-SSOT"]))
    assert_has(bab.validate_audit(bad), "competing authority requires finding")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-BYPASS", "pr_number":1, "closure_kind":"BYPASS_PATH", "subject":"bp-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"BYPASS_FINDING"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-BYPASS"]))
    assert_has(bab.validate_audit(bad), "bypass finding requires finding")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-SUP", "pr_number":1, "closure_kind":"SUPERSESSION_LIVENESS", "subject":"sup-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"classification":"LIVE"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-SUP"]))
    assert_has(bab.validate_audit(bad), "LIVE supersession reference requires finding")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-CFG", "pr_number":1, "closure_kind":"CONFIG_PRECEDENCE", "subject":"cfg-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"SINGLE_WINNER","winner_path":"config.yml"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-CFG"]))
    assert_has(bab.validate_audit(bad), "winner_path must be a discovered source")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-DEP", "pr_number":1, "closure_kind":"DEPENDENCY_CAUSALITY", "subject":"dep-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"FINDING", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"UNJUSTIFIED"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-DEP"]))
    assert_has(bab.validate_audit(bad), "FINDING requires finding_ids")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-PAIR", "pr_number":1, "closure_kind":"DEPENDENCY_PAIRING", "subject":"pair-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"FINDING", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"MISMATCH"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-PAIR"]))
    assert_has(bab.validate_audit(bad), "FINDING requires finding_ids")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-HUNK", "pr_number":1, "closure_kind":"DIFF_HUNK", "subject":"hunk-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"REQUIRED","objective_ids":[]}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-HUNK"]))
    assert_has(bab.validate_audit(bad), "substantive hunk requires objective_ids")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-CI", "pr_number":1, "closure_kind":"CI_CAUSALITY", "subject":"ci-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"causality":"PRE_EXISTING"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-CI"]))
    assert_has(bab.validate_audit(bad), "requires ci_baseline evidence")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-GEN", "pr_number":1, "closure_kind":"GENERATED_PROVENANCE", "subject":"gen-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"MATCHED"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-GEN"]))
    assert_has(bab.validate_audit(bad), "MATCHED generated provenance requires generator/source/command")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-ARCH", "pr_number":1, "closure_kind":"ARCHITECTURE_ECONOMY", "subject":"arch-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"justification_basis":"VERIFIED_PATTERN_EXTENSION","objective_ids":[],"authority_ids":[]}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-ARCH"]))
    assert_has(bab.validate_audit(bad), "justified architecture requires objective_ids or authority_ids")

    # v2.0 deterministic-closure seed census.
    v19_snapshot = {
        "repository":"Quantum-L9/example", "pr_number":7, "base_sha":A, "head_sha":B,
        "repository_files_complete":True,
        "files":[
            {
                "path":"src/api.py","status":"modified","additions":3,"deletions":2,
                "base_content":"def public(x):\n    return x == 1\n",
                "head_content":"def public(x, y=0):\n    return x != 1\n",
                "patch":"@@ -1,2 +1,2 @@\n-def public(x):\n-    return x == 1\n+def public(x, y=0):\n+    return x != 1",
            },
            {
                "path":"src/new_adapter.py","status":"added","additions":2,"deletions":0,
        ...
        "repository_files":[
            ...
            {"path":"src/consumer.py","content":"from src.api import public\nvalue = public(1)\n"},
            {"path":"src/new_adapter.py","content":"class NewAdapter:\n    pass\n"},
        ],
        "review_threads":[{"thread_id":"RT-7","is_resolved":False,"author":"reviewer","path":"src/api.py","body":"Check compatibility"}],
        "ci_failures":[],
    }
    v19_ledger = bcl.build(v19_snapshot)
    v19_kinds = [x["closure_kind"] for x in v19_ledger["closure_seeds"]]
    for expected_kind in ("PRODUCER_CONSUMER","MUTATION_EXECUTION","REVIEW_THREAD_SEMANTIC","ORPHAN_ARTIFACT"):
    ...
    if not any(x["payload"].get("path")=="src/new_adapter.py" and x["payload"].get("consumer_count")==0 for x in orphans):
        raise AssertionError("orphan census failed to enumerate added unreferenced artifact")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-PC", "pr_number":1, "closure_kind":"PRODUCER_CONSUMER", "subject":"pc-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"MIGRATION_REQUIRED","preservation_obligation_ids":[]}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-PC"]))
    assert_has(bab.validate_audit(bad), "MIGRATION_REQUIRED requires finding or preservation obligation")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-MUT", "pr_number":1, "closure_kind":"MUTATION_EXECUTION", "subject":"mut-x",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-TEST"], "finding_ids":[],
        "details":{"result":"KILLED","baseline_exit_code":0,"mutant_exit_code":0,"original_repository_unchanged":True}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-MUT"]))
    assert_has(bab.validate_audit(bad), "KILLED requires baseline pass and mutant failure")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-REV", "pr_number":1, "closure_kind":"REVIEW_THREAD_SEMANTIC", "subject":"RT-X",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"PASS", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"FIXED_BY_FINDING"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-REV"]))
    assert_has(bab.validate_audit(bad), "FIXED_BY_FINDING requires finding")

    bad = copy.deepcopy(audit)
    bad["deterministic_closure_ledger"].append({
        "closure_id":"C-ORPH", "pr_number":1, "closure_kind":"ORPHAN_ARTIFACT", "subject":"src/new.py",
        "source":"AUDITOR_ADDED", "seed_hash":None, "status":"FINDING", "evidence_ids":["E-AUTH"], "finding_ids":[],
        "details":{"disposition":"ORPHAN"}
    })
    for ap in bad["audit_passes"]: ap["observed_closure_ids"] = list(dict.fromkeys(ap["observed_closure_ids"] + ["C-ORPH"]))
    assert_has(bab.validate_audit(bad), "FINDING requires finding_ids")
    ...
    bad["post_judgment_closure"]["root_cause_dominance"] = [{
        "finding_a":"F1","finding_b":"F1","disposition":"INDEPENDENT","evidence_ids":["E-AUTH"]
    }]
    assert_has(bab.validate_audit(bad), "root_cause_dominance must exactly cover machine candidate pairs")

    # Enhanced symbol census and bypass token parsing are executable, not etiquette.
    rich = bcl.build({
        "repository":"Quantum-L9/example","pr_number":2,"base_sha":A,"head_sha":B,
        "intent":{"objectives":[{"objective_id":"O2"}]},
        "files":[
            {"path":"src/api.py","status":"modified","additions":4,"deletions":3,
             "base_content":"LIMIT = 1\ndef ping(x):\n    return x\n",
             "head_content":"LIMIT = 2\n@app.get('/ping')\ndef ping(x, y=1):\n    return x+y\n",
             "patch":"@@ -1,3 +1,4 @@\n-LIMIT = 1\n-def ping(x):\n-    return x\n+LIMIT = 2\n+@app.get('/ping')\n+def ping(x, y=1):\n+    return x+y"},
            {"path":"config/app.yaml","status":"modified","additions":1,"deletions":1,"patch":"@@ -1 +1 @@\n-timeout: 1\n+timeout: 2"},
    ...
    if dc.code_token_present("src/x.py", "# gateway\nvalue='gateway'\n", "gateway"):
        raise AssertionError("Python comments/strings must not satisfy bypass boundary token presence")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        ledger_path.write_text(json.dumps(synthetic_change_ledger(), indent=2, sort_keys=True) + "\n", encoding="utf-8")
        ...
        with __import__("zipfile").ZipFile(zip_path) as zf:
            ...
            assert manifest["change_ledger_set_sha256"] == bab.sha256_bytes(zf.read("change-ledger.json"))
            ...
            assert handoff["convergence_requirements"]["primary_objective"] == "RESOLVE_AUDIT_FINDINGS"
            ...
            assert set(handoff["convergence_requirements"]["additional_requirements"]) == {
                "RESOLVE_CURRENT_CI_FAILURES", "RESOLVE_ALL_CURRENT_CODE_REVIEW_THREADS"
            }
        ...
        with zipfile.ZipFile(projection_zip, "w", zipfile.ZIP_DEFLATED) as target:
            for name, payload in payloads.items():
                target.writestr(name, payload)
        assert_has(bab.verify_zip(projection_zip), "derived projection mismatch: PR_REMEDIATION_CONTRACT.md")
    ...
    snapshot = {
        ...
        "files": [
            {"path": "src/core.py", "status": "modified", "additions": 6, "deletions": 1,
             "patch": "+++ b/src/core.py\n+class NewAdapter:\n+    pass\n+try:\n+    call()\n+except RuntimeError:\n+    raise"},
            {"path": "tests/test_core.py", "status": "deleted", "additions": 0, "deletions": 8, "patch": ""},
        ],
    ...
    assert {"CHANGED_SURFACE", "ARCHITECTURAL_GROWTH", "FAILURE_PATH", "TEST_DISCRIMINATION", "CI_FAILURE", "REVIEW_THREAD"}.issubset(kinds)
    ...
    for item in bad["shared_evidence_index"]:
        if item["evidence_id"] == "E-AUTH":
            item["properties_discriminated"] = [p for p in item["properties_discriminated"] if p != "audit_domain:INTENT_SCOPE"]
    ...
    bad["architecture_policy_adapters"] = {
        "status": "RESOLVED",
        "adapters": [{
            "adapter_id":"P1","name":"synthetic policy","revision":"r1","status":"APPLIED",
            "governing_authority_ids":["MISSING"],"scope":["src/core.py"],
            "rule_domains":["OWNERSHIP_AUTHORITY"],"validation_methods":["manual inspection"],
            "evidence_ids":["E-AUTH"]
        }],
        "conflicts": [],
    }
    ...
    assert_has(bab.validate_audit(bad), "control_adequacy must cover every material control boundary")

    preexisting = copy.deepcopy(audit)
    preexisting["findings"][0]["origin"] = "PRE_EXISTING"
    preexisting["shared_evidence_index"][0]["properties_discriminated"].append("pre_existing_blocking_relevance")
    if bab.validate_audit(preexisting):
        raise AssertionError("pre-existing fixture should be valid when blocking relevance is proven:\n" + "\n".join(bab.validate_audit(preexisting)))
    ...
    bad = copy.deepcopy(audit)
    bad["change_discipline"]["scope_fidelity"]["changed_surfaces"] = bad["change_discipline"]["scope_fidelity"]["changed_surfaces"][:-1]
    assert_has(bab.validate_audit(bad), "scope_fidelity must disposition every changed file exactly once")

    bad = copy.deepcopy(audit)
    bad["audit_obligation_ledger"] = [o for o in bad["audit_obligation_ledger"] if not (o["kind"] == "CHANGED_SURFACE" and o["subject"] == "tests/test_core.py")]
    assert_has(bab.validate_audit(bad), "CHANGED_SURFACE obligations must exactly match changed_files")
    ...
    bad["authority_resolution"]["conflicts"] = [{
        "description": "two owners conflict", "authority_ids": ["A1", "A-GUARD"], "status": "BLOCKING"
    }]
    ...
    bad["residual_unknowns"] = [{
        "unknown_id": "U1", "description": "consumer behavior unavailable", "affected_prs": [1],
        "affected_conclusion": "cross-boundary correctness", "blocks_readiness": False,
        "blocks_convergence": True, "evidence_needed": "consumer contract", "owner_class": "HUMAN"
    }]
    ...
    ready["findings"][0]["merge_blocking_basis"] = {
        "status": "NON_BLOCKING", "authority_id": "A1",
    ...
    ready["shared_evidence_index"].append(evidence(
        "E-CI", "CI", B, "required ci check passed", ["mandatory_validation", "required_check_result"],
        source_head_sha=B, tested_revision_sha=C, validation_result="PASS", path_or_check="ci"
    ))
    ...
    for symbol in ready["changed_symbol_ledger"]:
        symbol["finding_ids"] = []
    sync_red_team(ready)
    ready["post_judgment_closure"]["severity_consistency"][0].update({"machine_floor":"Low","machine_ceiling":"High","actual_severity":"High","status":"CONSISTENT"})
    ready["post_judgment_closure"]["validation_run_coverage"].append({
        "evidence_id":"E-CI", "disposition":"JUSTIFIED_NON_CLOSING", "claim_ids":[],
    ...
    cycle["remediation_surface_index"].append({
        ...
        "excluded_false_leads": [],
        "surface_evidence": [{
            "path": "src/other.py", "role": "IMPLEMENTATION", "evidence_ids": ["E-FIND2"],
            "reason": "Synthetic confirmed location."
        }],
    })
    cycle["finding_dependency_graph"] = [
        {"finding_id":"F1","depends_on_findings":["F2"],"shared_root_cause":"R1","affected_prs":[1],"remediation_order_class":"DEPENDENT"},
        {"finding_id":"F2","depends_on_findings":["F1"],"shared_root_cause":"R1","affected_prs":[1],"remediation_order_class":"DEPENDENT"},
    ]
    ...
    cycle["post_judgment_closure"]["severity_consistency"].append({
        "finding_id":"F2", "machine_floor":"High", "machine_ceiling":"Critical",
        "actual_severity":"High", "status":"CONSISTENT", "evidence_ids":["E-AUTH"]
    })
    sync_red_team(cycle)
    ...
    with tempfile.TemporaryDirectory(prefix="l9-pr-audit-mut-selftest-") as td:
        ...
        def run_probe(command: str, *, timeout: int = 10, output: Path | None = None) -> tuple[subprocess.CompletedProcess[str], dict[str, object]]:
            argv = [
            ...
            payload = json.loads(output.read_text(encoding="utf-8") if output is not None else cp.stdout)
            return cp, payload
        ...
        shellish_cp, shellish = run_probe("python -c 'import a; assert True' ; touch SHOULD_NOT_EXIST")
        assert shellish_cp.returncode == 1 and shellish["result"] == "SURVIVED"
        assert not marker.exists(), "implicit shell execution created a marker in the audited repository"
    ...
    if residue_after:
        raise AssertionError("validation created bytecode/cache residue: " + ", ".join(sorted(residue_after)))
