...
def _validate_structured(data: dict[str, Any]) -> list[str]:
    ...
    if authority_count > MAX_COUNTS["authority_order"]:
        errors.append(
            f"authority_order exceeds max {MAX_COUNTS['authority_order']}: {authority_count}"
        )

    heuristic_node = spec.get("expert_heuristics")
    heuristic_items = (
        heuristic_node.get("items", []) if isinstance(heuristic_node, dict) else heuristic_node
    )
    ...
    if heuristic_count > MAX_COUNTS["expert_heuristics"]:
        errors.append(
            f"expert_heuristics exceeds max {MAX_COUNTS['expert_heuristics']}: {heuristic_count}"
        )
    ...
    failure_items = (
        failure_node.get("items", []) if isinstance(failure_node, dict) else failure_node
    )
