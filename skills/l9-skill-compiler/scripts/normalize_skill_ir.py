...
def normalize(draft):
    ...
    ir["identity"].setdefault("updated", date.today().isoformat())
    ir.setdefault("objective", "")
    ir.setdefault("authority", {})
    ir.setdefault("workflow", {"entrypoint": None, "nodes": []})
    for key in LIST_KEYS:
        ir.setdefault(key, [])
    ir["capabilities"] = sorted(ir["capabilities"], key=lambda item: item.get("id", ""))
    ir["workflow"]["nodes"] = sorted(
        ir["workflow"].get("nodes", []), key=lambda item: item.get("id", "")
    )
    return ir
...
def main(argv):
    ...
    if len(argv) > 2:
        with open(argv[2], "w", encoding="utf-8") as handle:
            json.dump(ir, handle, indent=2, sort_keys=True)
    return emit(
        {
            "stage": "NORMALIZE_SKILL_IR",
            "status": "PASS",
            "round_trip_stable": round_trip(ir),
            "capability_count": len(ir["capabilities"]),
            "node_count": len(ir["workflow"]["nodes"]),
        }
    )
