...
2. **Pre-Validate** — bind the **current workspace** (branch, dirty, HEAD if useful). For code in scope on governed workspaces name `make pr-check` (**no commit, no push**). Do **not** lock `origin/main` or open a tip worktree.
