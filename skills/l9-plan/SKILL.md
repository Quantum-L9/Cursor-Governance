...
3. **Pre-Validate** — bind target; inventory baseline; for code in scope on governed workspaces name `OPEN_PR=0 make pr` (**no commit, no push**). KERNEL pack / PE overlay landings: new branch from `origin/main` without asking (`AGENTS.md` `KERNEL_PACK_NEW_BRANCH_DEFAULT_V1`; `rules/46-kernel-pack-new-branch.mdc`).
