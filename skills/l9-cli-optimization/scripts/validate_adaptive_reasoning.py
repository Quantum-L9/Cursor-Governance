...
def main() -> int:
    ...
    for snippet in (
        ...
        if snippet.lower() not in skill.lower():
            errors.append(f"SKILL.md missing adaptive doctrine: {snippet}")
    cases = json.loads((ROOT / "assets" / "adaptive-route-cases.json").read_text(encoding="utf-8"))
    for case in cases:
        if (
            not isinstance(case, dict)
            or "input" not in case
            or "expect" not in case
            or "name" not in case
        ):
            errors.append(f"malformed route case: {case!r}")
            continue
        try:
        ...
        for key in ("reasoning_depth", "initial_action", "write_action_allowed"):
            if result.get(key) != expect.get(key):
                errors.append(
                    f"route case {case['name']} {key}: {result.get(key)} != {expect.get(key)}"
                )
    ...
    for field in ("execution_route", "decision_ledger"):
        if field not in schema.get("required", []):
            errors.append(f"pack schema does not require {field}")
    manifest = json.loads(
        (ROOT / "schemas" / "pack-manifest.schema.json").read_text(encoding="utf-8")
    )
    for field in (
        "reasoning_depth",
        "initial_action",
        ...
        if field not in manifest.get("required", []):
            errors.append(f"manifest schema does not require {field}")
    ...
    for snippet in (
        ...
        if snippet not in builder or snippet not in validator:
            errors.append(f"builder/validator missing {snippet}")
    if errors:
        for error in errors:
