...
def run(command: list[str], cwd: Path | None = None, *, env: dict[str, str] | None = None, expected_codes: set[int] | None = None) -> subprocess.CompletedProcess[str]:
...
def main() -> int:
    ...
    gate_section = skill_text.split("## Validation", 1)[-1].split(chr(10) + "## ", 1)[0]  # stop before next H2 (excludes Resource Map)
    fenced = chr(10).join(_re.findall(r"```bash\n(.*?)```", gate_section, _re.DOTALL))
    listed = set(_re.findall(r"scripts/(\w+\.py)", fenced))
    invoked = set(standalone) | {"build_commit_pack.py", "validate_commit_pack.py", "scan_capabilities.py", "measure.py", "self_test.py",
                                 "flag_inventory.py", "full_throttle.py", "build_flag_activation_pack.py"}
    if listed != invoked:
        raise RuntimeError(
            "SKILL.md ## Validation script list is not in parity with self_test; "
            "only_listed=" + str(sorted(listed - invoked)) + ", only_invoked=" + str(sorted(invoked - listed))
        )
    ...
    null_probe = {
        "pack_name": "probe", "status": "PR_READY", "repository": "r", "base_ref": "b",
        "branch": "br", "summary": "s", "optimization": {"utilization_gap_class": "artificial_delay"},
        "performance": {"improvement_percent": 10.0}, "wiring": None,
    }
    ...
    with tempfile.TemporaryDirectory(prefix="optimize-cli-pack-test-") as raw:
        ...
        command = [sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(spec_path), "--repo-root", str(repo), "--output", str(out)]
        ...
        def expect_pack_rejected(label, mutate):
            ...
            mutate(m)
            r = subprocess.run([sys.executable, str(scripts / "validate_commit_pack.py"), str(m)],
                               text=True, capture_output=True)
            if r.returncode == 0:
                raise RuntimeError("validate_commit_pack accepted a corrupted pack: " + label + " :: " + r.stdout)
        ...
        def _mut_patch_wrong_content(m):
            ...
            man = json.loads((m / "MANIFEST.json").read_text())
            man["patch_sha256"] = _h.sha256(fake).hexdigest()  # sha now matches -> H1 path check must catch it
            (m / "MANIFEST.json").write_text(json.dumps(man, indent=2), encoding="utf-8")

        def _mut_failed_command(m):
            cf = m / "evidence" / "commands.jsonl"
            recs = [json.loads(l) for l in cf.read_text().splitlines() if l.strip()]
        ...
        def _mut_underdeclared_route(m):
            ...
            for rel in ("evidence/EXECUTION_ROUTE.json", "evidence/DECISION_LEDGER.json"):
                ...
                obj["proof_obligations"] = [o for o in obj["proof_obligations"] if o.get("id") != "PO-REACHABILITY"]
                if "required_adapters" in obj:
                    obj["required_adapters"] = [a for a in obj["required_adapters"] if a != "latent_capability_reachability"]
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(blocked_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(external_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})

        unsafe = build_spec()
        unsafe["changed_files"] = ["../outside"]
        unsafe_path = temp / "unsafe.json"
        unsafe_path.write_text(json.dumps(unsafe), encoding="utf-8")
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(unsafe_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(missing_wiring_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(unknown_wiring_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(staged_rollout_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(missing_synthesis_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(bad_score_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(omitted_divergence_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(blocking_divergence_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(missing_route_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(pending_proof_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(selection_mismatch_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})

        material_unknown = build_spec()
        material_unknown["decision_ledger"]["unknowns"].append({  # type: ignore[index]
            "id": "UNK-002", "description": "Unresolved release safety", "material": True,
            "disposition": "block", "evidence": []
        })
        material_unknown["decision_ledger"]["convergence"]["remaining_material_unknown_ids"] = ["UNK-002"]  # type: ignore[index]
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(material_unknown_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        mini = temp / "miniscan"
        mini.mkdir()
        (mini / "a.py").write_text("def dead_symbol():\n    return 1\ndef used_symbol():\n    return 2\n", encoding="utf-8")
        (mini / "b.py").write_text("from a import used_symbol\nprint(used_symbol())\n", encoding="utf-8")
        ...
        if not all(isinstance(s, (int, float)) for s in scores) or scores != sorted(scores, reverse=True):
        ...
        mini2_scan = json.loads(run([sys.executable, str(scripts / "scan_capabilities.py"), str(mini2)]).stdout)
        ...
        if by_symbol.get("debug_enabled", {}).get("recommended_verdict") == "do_not_activate":
            raise RuntimeError("scanner over-flagged a plain off-by-default flag as do_not_activate")
        ...
        (mini3 / "_archived" / "legacy_mod.py").write_text("def legacy():\n    return 0\n", encoding="utf-8")
        ...
        m3 = json.loads(run([sys.executable, str(scripts / "scan_capabilities.py"), str(mini3)]).stdout)
        ...
        if "pkg/runner.py" in unwired:
            raise RuntimeError("scanner flagged a `python -m pkg.runner` script as unwired (F2 regression)")
        ...
        measure_out = run([
            sys.executable, str(scripts / "measure.py"),
            "--before", f"{sys.executable} -c \"pass\"",
            "--after", f"{sys.executable} -c \"pass\"",
            "--samples", "1",
        ])
        ...
        git_measure = run([
            sys.executable, str(scripts / "measure.py"),
            "--repo", str(repo), "--before-ref", "main",
            "--before", f"{sys.executable} runner.py --tasks 3",
            "--after", f"{sys.executable} {runner_abs} --tasks 3 --workers 2",
            "--samples", "1",
        ])
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(null_wiring_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(fourth_cycle_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(activation_path), "--repo-root", str(repo), "--output", str(out)], env=env)

        # OBS-003 / OBS-006: the shipped minimal (throughput, non-latent)
        # template must build+validate and must NOT carry a wiring block.
        minimal_spec = json.loads((scripts.parent / "assets" / "pack-spec.minimal.json").read_text(encoding="utf-8"))
        if "wiring" in minimal_spec:
            raise RuntimeError("pack-spec.minimal.json must not include a wiring block (throughput template)")
        minimal_path = temp / "minimal.json"
        minimal_path.write_text(json.dumps(minimal_spec), encoding="utf-8")
        run(command[:2] + ["--spec", str(minimal_path), "--repo-root", str(repo), "--output", str(out)], env=env)
        run([sys.executable, str(scripts / "validate_commit_pack.py"), str(out / str(minimal_spec["pack_name"]))])
        ...
        run([sys.executable, str(scripts / "build_commit_pack.py"), "--spec", str(foreign_route_path), "--repo-root", str(repo), "--output", str(out)], expected_codes={2})
        ...
        expected_cls = {"enable_cache": None, "benign_enabled": "safe", "disable_auth": "danger",
                        "allow_delete": "danger", "use_live_api": "danger", "kge_enabled": "staged"}
        ...
        if {"disable_auth", "allow_delete", "use_live_api", "kge_enabled"} & set(plan["would_flip"]):
            raise RuntimeError(f"full_throttle plan would flip an excluded flag: {plan['would_flip']}")
        ...
        run([sys.executable, str(scripts / "full_throttle.py"), str(ft), "--mode", "apply",
             "--test-cmd", f"{sys.executable} check.py", "--output", str(ft_report)])
        report = json.loads(ft_report.read_text())
        if report.get("activated_flags") != ["benign_enabled"]:
            raise RuntimeError(f"full_throttle activated the wrong set: {report.get('activated_flags')}")
        ...
        ft_out = temp / "ft-out"
        ftp = run([sys.executable, str(scripts / "build_flag_activation_pack.py"),
                   "--report", str(ft_report), "--repo-root", str(ft), "--output", str(ft_out)], env=env)
        ...
        run([sys.executable, str(scripts / "build_flag_activation_pack.py"),
             "--report", str(ft_report), "--repo-root", str(ft), "--output", str(ft_out)], env=env)
        if (ft_pack / "SHA256SUMS").read_text() != first_sums:
            raise RuntimeError("flag activation pack is not deterministic")

    print("PASS: identity, adaptive routing, decision ledger, latent wiring, divergence, leverage synthesis, deterministic packaging, full-throttle activation, and negative self-tests")
    return 0
