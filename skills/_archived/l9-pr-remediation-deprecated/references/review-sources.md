...
A source is enabled when `enabled_when: always` or its configured credentials are present. Record unavailable sources in `summary.review_sources`. Optional sources may be skipped. A source that owns or supplies evidence for a required check is mandatory; its unavailability blocks PR readiness and terminally escalates after the bounded loop.

**Default:** GitHub-native review comments, check annotations, and code-scanning evidence are enabled. Vendor sources are opt-in and activate only when configured. Missing optional vendor credentials are recorded; missing evidence for a required check is blocking.
