...
description: remediator for open github issues across all non-archived quantum-l9 repos — drain leverage-ranked clusters, land fixes on the matching open pr or a stacked pr on the newest, close resolved issues, and chain l9-pr-remediation only when open_issues=0. diagnose is the auditor opt-out. use when the user runs /issues or /l9-issue-remediation, or asks to fix, remediate, unblock, or converge open issues.
...
theater. Converge drains **all** automatable clusters, highest leverage first.
...
| Human/external | product/secrets/third-party | Note; do not fake close |
...
1. **All automatable clusters.** Default `max_clusters_per_invoke: all`,
   highest leverage first. Per-cluster max 3 cycles. Never start cycle 4 on
   one cluster.
2. **Codebase only** in owning repo(s). Never edit `.github/workflows/**`,
   branch protection, org secrets policy, or CI-only infra. Note and skip.
3. **Ownership before edit.** Load [ownership-boundary.md](references/ownership-boundary.md).
4. **Obvious owner.** Shared package/SSOT before ad-hoc consumer edits.
5. **One commit per cycle** in the mutated owning repo. Zero if nothing safe.
6. **PR landing.** Matching open PR wins; else stacked PR on newest; else
   first PR on `origin/main`. See [pr-landing.md](references/pr-landing.md).
7. **Local verify blocks publish.** ≤5 re-diag iterations.
8. **No gate weakening / suppressions** as “fixes.”
9. **Never** force-push, merge PRs, expose secrets, or mass-close HUMAN
   issues. Evidence-close HUMAN/EXTERNAL only as superseded/duplicate/already-fixed.
10. **`/l9-pr-remediation` only after `open_issues=0`.**
    [handoff-to-pr-remediation.md](references/handoff-to-pr-remediation.md).
11. **Breadcrumb law** is part of Done When — PICKUP failure or `status=fixed`
...
3. **Fix** the current highest-leverage automatable cluster (concurrent when
   independent). [fix-engine.md](references/fix-engine.md) (**Lesson Recall**).
4. **Local verify** → commit with trailer
   `Issue-Remediation-Cycle: {owner}/{repo}#{n}/cycle-{N}`.
5. **Land on PR** — [pr-landing.md](references/pr-landing.md).
6. **Breadcrumbs** — PICKUP + comments + **close if fixed** + conditional session-ref.
7. **Re-ingest + gate** — [convergence-loop.md](references/convergence-loop.md).
   Chain `/l9-pr-remediation` **only** when `open_issues_gate.py` says `chain: true`.

## Done When (Converge)

- Automatable clusters drained **or** only HUMAN/EXTERNAL remain
...
- `/l9-pr-remediation` invoked **only if** bound-target `open_issues=0`;
  otherwise status `BLOCKED_OPEN_ISSUES`
...
- `open_issues != 0` → `BLOCKED_OPEN_ISSUES`; do not chain `/l9-pr-remediation`
