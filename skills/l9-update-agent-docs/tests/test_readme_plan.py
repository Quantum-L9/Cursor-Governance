...
def test_a_conflict_is_a_plan_error_so_callers_fail_closed(tmp_path: Path):
    """A conflict item alone let every other mutation through.

    `ReadmePlan.errors` reads findings, not actions. `repo_docs.py` and the
    CLI apply path both gate on `errors`, so a plan whose only problem was an
    unreconcilable artifact presented as clean and wrote the rest beside it.
    """
    write(tmp_path / "pkg" / "mod.py", "x = 1\n")
    write(tmp_path / "fresh" / "mod.py", "y = 2\n")
    future = "<!-- l9-readme: generated-by=l9-update-agent-docs version=99 kind=module -->"
    write(tmp_path / "pkg" / "README.md", "# Newer\n\n" + future + "\n")
    plan = gm.plan_module_readmes(tmp_path)
    assert actions(plan)["pkg/README.md"] == "conflict"
    assert [finding.rule_id for finding in plan.errors] == ["readme.marker.future_version"]
    assert plan.errors[0].source == "pkg/README.md"


def test_an_unreadable_marker_version_is_a_conflict_not_a_refresh(tmp_path: Path):
    """`version=3.0` is a newer format, not an absent claim.

    `marker_version` returned None for a version it could not parse, which
    is what it also returns for a file carrying no marker at all. The
    planner's `future > MARKER_VERSION` guard never fired and the newer
    generator's output was overwritten as 'stale generated bytes'.
    """
    write(tmp_path / "pkg" / "mod.py", "x = 1\n")
...
def test_a_readme_that_only_documents_the_marker_is_preserved(tmp_path: Path):
    """Ownership needs a marker, not a mention.

    A handwritten README explaining the marker format quoted the prefix, and
    the substring check classified it as generator-owned — making it eligible
    for refresh, and for retirement once its target was unauthorized.
    """
    write(tmp_path / "pkg" / "mod.py", "x = 1\n")
    prose = (
        "# Marker format\n\n"
        "Generated READMEs end with a line beginning\n"
        "`<!-- l9-readme: generated-by=l9-update-agent-docs version=`\n"
        "followed by the format version and the kind.\n"
    )
    write(tmp_path / "pkg" / "README.md", prose)
    write(tmp_path / "gone" / "README.md", prose)
    plan = gm.plan_module_readmes(tmp_path)
    assert actions(plan)["pkg/README.md"] == "preserve"
    assert "gone/README.md" not in actions(plan)
    gm.apply_module_readme_plan(tmp_path, plan)
    assert (tmp_path / "pkg" / "README.md").read_text(encoding="utf-8") == prose
    assert (tmp_path / "gone" / "README.md").read_text(encoding="utf-8") == prose
