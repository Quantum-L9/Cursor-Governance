...
| Memory capability | Canonical `l9-graphite-memory` control plane over stdio (`ops/memory`); `memory.cli` / control plane / `memory.mcp`; no URL, no bearer, broker retired | `emit_claude_readiness.py --memory-probe`; adapter `mcp.template.json` (stdio, gated on `L9_MEMORY_INTERPRETER`) |
...
| PR validation | governance Makefile `pr` via `l9 pr` / `make -C "$GOV" pr WS="$PWD"` → `open_pr_after_gate.sh` (REST); changed-files gate; consumer repo needs no local `pr` target. Gate-only diagnosis is `OPEN_PR=0 l9 pr`. Same finish as Cursor: authorize-release then `PR_REMEDIATE=0 make pr` / `l9 pr`. Cursor skips tree kernels; this adapter still fires them. | governance Makefile `pr` (`docs/L9_DISPATCHER.md`); Profile `session_start_block` |
