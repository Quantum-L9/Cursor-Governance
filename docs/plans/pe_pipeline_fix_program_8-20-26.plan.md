---
name: PE Factory Repair: pipeline friction fixes + bounded-autonomy defaults
overview: "Repair the Cursor-Governance program-execution factory using evidence collected while driving campaign website-bot-five-push-v1: remove mechanical friction that stops or slows agent campaigns (workspace-resolution gaps, auth channels unreachable in-session, secrets path-guards, npm registry auth, unverified dependency publishes, stale Graphiti conflict edges) and encode the operator's new opera..."
todos:
  - id: todo-01-baseline-preflight
    content: "Lock immutable baseline SHA, verify worktree registration, re-check in-flight overlap against scope.in, record receipts"
    status: pending
    phase: execute
    depends_on: []
  - id: todo-02-merge-gate-human-channel
    content: "merge_gate.py: accept one-shot human authorization file (~/.l9/autonomy/merge-authorization.json with repo/pr/sha/expires_at) alongside L9_MERGE_AUTHORIZED; default remains deny; add unit tests (deny default, allow valid, deny stale/forged)"
    status: pending
    phase: execute
    depends_on: [todo-01-baseline-preflight]
  - id: todo-03-secrets-path-guard
    content: "resolve_secret.py + sync_secrets_registry.py: allow governance root (script-relative) in _safe_path allowed roots; regression test running from a consumer cwd"
    status: pending
    phase: execute
    depends_on: [todo-01-baseline-preflight]
  - id: todo-04-authed-npm-wrapper
    content: "Add ops/secrets/authed_npm.sh that resolves the sole PAT and exports npm_config_//npm.pkg.github.com/:_authToken; document npm mechanics in skills/l9-aws-secrets/SKILL.md"
    status: pending
    phase: execute
    depends_on: [todo-01-baseline-preflight]
  - id: todo-05-dep-publish-preflight
    content: "New ops/hooks/check_gh_package_deps.py: every @quantum-l9/* version in package.json must exist on GitHub Packages and lock resolved/integrity must match registry metadata; register in .pre-commit-config.yaml (+ precommit-hook-contract.json if required); fixtures + unit tests"
    status: pending
    phase: execute
    depends_on: [todo-01-baseline-preflight]
  - id: todo-06-graphiti-conflicts-freshness
    content: "graphiti_memory_client.py conflicts: exclude edges with expired_at in the past (--fresh-only); unit test with expired edge fixture"
    status: pending
    phase: execute
    depends_on: [todo-01-baseline-preflight]
  - id: todo-07-surface-profile-defaults
    content: "surface_profile.yaml: authorize pr_remediation_until_green_and_mergeable + open_stacked_pr_on_open_pr_head; remove remediation_after_pr/l9_pr_remediation_spawn from forbid; keep merge_pull_request + gh_pr_merge forbidden (halt before merge); forbid rebase_for_pr_stacking; post_push remediation_skill l9-pr-remediation, max_cycles 5, merge_requires never, older_open_prs merge_bottom_up; structural assertion tests"
    status: pending
    phase: execute
    depends_on: [todo-01-baseline-preflight]
  - id: todo-08-autonomy-manifest-routing
    content: "AUTONOMY_MANIFEST.yaml claude_routing: l9-pr-remediation auto_invoke under campaign/PE packet (remediate-until-green scope); merge intents remain explicit/human; validate with existing manifest validation"
    status: pending
    phase: execute
    depends_on: [todo-07-surface-profile-defaults]
  - id: todo-09-campaign-stacking-policy
    content: "CAMPAIGN_EXECUTION_POLICY.yaml: one feature branch off main per program, exclusive worktree lease per mutating agent, stacked PRs bottom-up without rebase (first PR base = integration/main, subsequent base = open PR head); add ops/scripts/stack_pr.py helper implementing base selection + bottom-up merge order check"
    status: pending
    phase: execute
    depends_on: [todo-07-surface-profile-defaults]
  - id: todo-10-rules-docs
    content: "Update rules/zz-autonomy-surface-override.mdc + rules/48-make-pr-remediation.mdc prose to the new defaults; regenerate generated llm-rules (project_llm_rules.py --root)"
    status: pending
    phase: execute
    depends_on: [todo-07-surface-profile-defaults, todo-08-autonomy-manifest-routing, todo-09-campaign-stacking-policy]
  - id: todo-11-prove
    content: "Run unit tests, lint, make pr-check, policy structural assertions; all PASS"
    status: pending
    phase: execute
    depends_on: [todo-02-merge-gate-human-channel, todo-03-secrets-path-guard, todo-04-authed-npm-wrapper, todo-05-dep-publish-preflight, todo-06-graphiti-conflicts-freshness, todo-10-rules-docs]
  - id: todo-12-converge-halt-before-merge
    content: "Open PR for pe/pipeline-fix-program (stacked on open PR head per policy if the in-flight PR is open), remediate until CI green + mergeable, then HALT before merge; human merges bottom-up"
    status: pending
    phase: execute
...
> **Suggested filename:** `pe-factory-repair-pipeline-friction-fixes-bounded-autonomy-defaults_bedaf3c9.plan.md`

## Objective (from PLAN_DOCUMENT)

Repair the Cursor-Governance program-execution factory using evidence collected while driving campaign website-bot-five-push-v1: remove mechanical friction that stops or slows agent campaigns (workspace-resolution gaps, auth channels unreachable in-session, secrets path-guards, npm registry auth, unverified dependency publishes, stale Graphiti conflict edges) and encode the operator's new operating defaults — default authorization = open PR, remediate until green and mergeable, then HALT before merge; stacked PRs merged bottom-up with no rebase; all work on a single feature branch off main; authorization YAMLs updated so approval requirements are unblocked before PRs open. Non-negotiable preserved contracts: merge stays human-only; gates stay fail-closed; scanners are never weakened; protected PE core templates and the concurrent pe/pipeline-fixes workstream files are not touched by this program. Success properties SP-01..SP-09 carry the evidence-typed acceptance criteria.

### Success properties (seed — complete evidence_type/proof in template sections)

| id | property | evidence_type | proof | blocking |
|----|----------|---------------|-------|----------|
| SP-01 | SP-01 Baseline matches locked SHA at execution start (git rev-parse HEAD == 95e4c0088919a3fe53fae36d82c36d69f7c13285) | quality_gate | observe during PE verify / make pr-check | true |
| SP-02 | SP-02 merge_gate denies by default and allows only a valid human authorization file (fixture hook runs) | quality_gate | observe during PE verify / make pr-check | true |
| SP-03 | SP-03 secrets resolver works from a consumer repo cwd (--check OK) | quality_gate | observe during PE verify / make pr-check | true |
| SP-04 | SP-04 authed npm install works via ops/secrets/authed_npm.sh wrapper (npm ci in scratch clone) | quality_gate | observe during PE verify / make pr-check | true |
| SP-05 | SP-05 dependency-publish preflight catches unpublished version and fabricated lock integrity (unit fixtures) | quality_gate | observe during PE verify / make pr-check | true |
| SP-06 | SP-06 graphiti conflicts filters expired edges (unit fixture) | quality_gate | observe during PE verify / make pr-check | true |
| SP-07 | SP-07 policy YAMLs parse and encode the new operating defaults (structural assertions) | quality_gate | observe during PE verify / make pr-check | true |
| SP-08 | SP-08 quality gates PASS on changed files (make pr-check) | quality_gate | observe during PE verify / make pr-check | true |
| SP-09 | SP-09 no in-flight pe/pipeline-fixes file modified by this program (diff name check) | quality_gate | observe during PE verify / make pr-check | true |

## Scope (from PLAN_DOCUMENT)

**In:** ops/autonomy/merge_gate.py, ops/autonomy/surface_profile.yaml, ops/secrets/resolve_secret.py, ops/secrets/sync_secrets_registry.py, ops/secrets/authed_npm.sh (new), skills/l9-aws-secrets/SKILL.md, ops/hooks/check_gh_package_deps.py (new), .pre-commit-config.yaml, ops/config/precommit-hook-contract.json (if registration requires), ops/graphiti/graphiti_memory_client.py, skills/AUTONOMY_MANIFEST.yaml, environment/program-execution/campaigns/CAMPAIGN_EXECUTION_POLICY.yaml, ops/scripts/stack_pr.py (new), rules/zz-autonomy-surface-override.mdc, rules/48-make-pr-remediation.mdc, unit tests alongside each changed module

**Out:**
- environment/agents/adapters/claude-code/hooks/memory_gate.py (in-flight pe/pipeline-fixes)
- environment/agents/adapters/claude-code/hooks/memory_lock.py (in-flight)
- environment/agents/adapters/claude-code/hooks/memory_prefetch.py (in-flight)
- environment/program-execution/MANIFEST.json (in-flight)
- environment/program-execution/core/MANIFEST.yaml (in-flight)
- environment/program-execution/core/program-execution-controller-template/MANIFEST.yaml (in-flight)
- environment/program-execution/core/program-execution-controller-template/scripts/pec/contracts.py (in-flight)
- environment/program-execution/core/program-execution-controller-template/scripts/pec/controller.py (in-flight)
- environment/program-execution/core/program-execution-controller-template/scripts/compile_campaign_source.py (in-flight)
- environment/program-execution/core/program-execution-controller-template/scripts/tests/test_compile_campaign_source.py (in-flight)
- ops/autonomy/local_execution_gate.py (in-flight)
- ops/autonomy/worktree_isolation_gate.py (in-flight)
- environment/program-execution/core/program-execution-controller-template/RUNBOOK.md (in-flight)
- /Users/macm2/.l9/gov-worktrees/pe-pipeline-fixes worktree (other agent's)
- Quantum-L9/SEO-Bot and Quantum-L9/Website-Bot repos
- $HOME/.l9/programs/website-bot-five-push-v1 campaign runtime

## Critical path (seed)

todo-01-baseline-preflight → todo-02-merge-gate-human-channel → todo-11-prove → todo-12-converge-halt-before-merge
...
- status: partial
- next_skill: l9-ynp
- stop_reason: plan delivered for operator approval; execution not started (merge of SEO-Bot PR #53 also still awaiting human)
