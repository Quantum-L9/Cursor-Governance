...
overview: "NOT LANDED. Shelf=pending (not current). Repo-side Claude Web/Mobile adapter contract is already COMPLETE in docs/CLAUDE_SURFACE_PARITY.md; these 12 todos are still pending and must be revalidated against origin/main before Build. Intent: make the Claude Code cloud/mobile adapter self-healing and truthful — wrong-workspace wiring, false READY, MCP drift, command parity; isolate the capability-broker gap."
todos:
  - id: T1
