---
name: Close /ff publish loop for Claude Code–clean worktrees
overview: "When /ff shelves leftover WIP/plans/campaigns, finish scoped-commit, L4 authorize-release, and PR_STACK=auto PR_REMEDIATE=0 make pr unless FF_SHELF_PUBLISH=0, then verify HEAD equals origin/main and dirty_unique is zero."
todos:
  - id: T1-doctrine-gap
    content: "Replace ask-before-publish with closed-loop contract in ff.md and execute.md"
    status: pending
    phase: execute
    depends_on: []
  - id: T2-opt-out
    content: "Document FF_SHELF_PUBLISH=0 opt-out in ff.md and l9-repo-sync SKILL.md"
    status: pending
    phase: execute
    depends_on: [T1-doctrine-gap]
  - id: T3-agents-fragment
    content: "Append AGENTS.md FF_CLOSE_PUBLISH_LOOP_V1 fragment"
    status: pending
    phase: execute
    depends_on: [T2-opt-out]
  - id: T4-rule-55
    content: "Update rules/55-ff-only-ssot-sync.mdc closed-loop MUST bullet"
    status: pending
    phase: execute
    depends_on: [T2-opt-out]
  - id: T5-verify-script
    content: "Add verify_worktree_clean.py and pytest coverage"
    status: pending
    phase: execute
    depends_on: []
  - id: T6-post-shelf-runner
    content: "Add run_ff_post_shelf.sh post-shelf prune and verify wrapper"
    status: pending
    phase: execute
    depends_on: [T5-verify-script, T2-opt-out]
  - id: T7-task-queue
    content: "Update root task-queue section for /ff shelf closed-loop doctrine"
    status: pending
    phase: execute
    depends_on: [T3-agents-fragment]
  - id: T8-validate
    content: "Run validate_plan_document and pytest on verify script"
    status: pending
    phase: execute
    depends_on: [T5-verify-script, T6-post-shelf-runner]
isProject: false
kind: simple
execute_via: cursor-build
---

# PLAN: Close /ff publish loop for Claude Code–clean worktrees
...
> **Suggested filename:** `close-ff-publish-loop-for-claude-code-clean-worktrees_11fafe9a.plan.md`

## Objective (from PLAN_DOCUMENT)

When /ff shelves leftover WIP/plans/campaigns, finish scoped-commit, L4 authorize-release, and PR_STACK=auto PR_REMEDIATE=0 make pr unless FF_SHELF_PUBLISH=0, then verify HEAD equals origin/main and dirty_unique is zero.

### Success properties (seed — complete evidence_type/proof in template sections)

| id | property | evidence_type | proof | blocking |
|----|----------|---------------|-------|----------|
| SP-01 | commands/ff.md closes the shelf loop by default with FF_SHELF_PUBLISH=0 opt-out | quality_gate | observe .pre-commit-config.yaml catalog | true |
| SP-02 | ops/scripts/verify_worktree_clean.py exits 0 on a clean tree with actionable stderr on failure | quality_gate | observe .pre-commit-config.yaml catalog | true |
| SP-03 | ops/scripts/run_ff_post_shelf.sh runs prune_open_pr_copies and verify after shelf publish | quality_gate | observe .pre-commit-config.yaml catalog | true |
| SP-04 | validate_plan_document.py PASS on this PLAN_DOCUMENT | quality_gate | observe .pre-commit-config.yaml catalog | true |
| SP-05 | pytest tests/ops/scripts/test_verify_worktree_clean.py PASS | quality_gate | observe .pre-commit-config.yaml catalog | true |

## Scope (from PLAN_DOCUMENT)

**In:** commands/ff.md, skills/l9-repo-sync/SKILL.md, skills/l9-repo-sync/references/execute.md, rules/55-ff-only-ssot-sync.mdc, AGENTS.md, TODO.md, ops/scripts/verify_worktree_clean.py, ops/scripts/run_ff_post_shelf.sh, tests/ops/scripts/test_verify_worktree_clean.py

**Out:**
- Changing ff.sh reset --keep primitive
- make pr inside ff.sh
- Program Lock or make campaign
- Rewriting CANONICAL_LAW.md
- Automatic git gc or unreachable object prune
- Merging shelf PRs

## Critical path (seed)

T5-verify-script → T1-doctrine-gap → T2-opt-out → T6-post-shelf-runner → T8-validate

## Stress (seed from PLAN_DOCUMENT)

- Blast radius: Unwanted shelf PR if opt-out ignored
- Rollback: Set FF_SHELF_PUBLISH=0; revert doctrine files

## Convergence (seed)

- status: partial
- next_skill: Build then stacked make pr
- stop_reason: plan validated; Build implements doctrine and verify tooling
- execute_via: cursor-build
