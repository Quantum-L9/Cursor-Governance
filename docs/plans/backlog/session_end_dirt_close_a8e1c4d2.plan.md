---
name: SessionEnd dirt close
overview: "After a completed sessionEnd, the session workspace has dirty_files=0: landed copies gone, novel unique bytes only on l9/dirt-shelf, absorbed parks pruned, --status the honest answer."
todos:
  - id: todo-branch
    content: "Cut feat/session-end-dirt-close from origin/main; emit PLAN_DOCUMENT"
    status: pending
    phase: execute
    depends_on: []
  - id: todo-close-module
    content: "Add session_end_dirt_close.py --apply and --status"
    status: pending
    phase: execute
    depends_on: [todo-branch]
  - id: todo-prune-absorbed
    content: "Prune absorbed dirt-shelf and leftover worktree-dirt refs"
    status: pending
    phase: execute
    depends_on: [todo-close-module]
  - id: todo-hook-gates
    content: "Wire dirt-close before hygiene --apply with sessionEnd gates"
    status: pending
    phase: execute
    depends_on: [todo-close-module]
  - id: todo-status-doctrine
    content: "Append AGENTS.md fragment; correct REPO_HYGIENE.md; rule 49 clause"
    status: pending
    phase: execute
    depends_on: [todo-hook-gates]
  - id: todo-tests
    content: "Fixture tests SP-01 through SP-11"
    status: pending
    phase: execute
    depends_on: [todo-close-module, todo-prune-absorbed]
  - id: todo-verify
    content: "Targeted pytest; scoped commits; stop before publish"
    status: pending
    phase: execute
    depends_on: [todo-tests, todo-status-doctrine]
isProject: false
kind: simple
execute_via: cursor-build
---

# PLAN: SessionEnd dirt close
...
> **Suggested filename:** `sessionend-dirt-close_f093eee8.plan.md`

## Objective (from PLAN_DOCUMENT)

After a completed sessionEnd, the session workspace has dirty_files=0: landed copies gone, novel unique bytes only on l9/dirt-shelf, absorbed parks pruned, --status the honest answer.

### Success properties (seed — complete evidence_type/proof in template sections)

| id | property | evidence_type | proof | blocking |
|----|----------|---------------|-------|----------|
| SP-01 | SP-01 unique modified plus unique untracked parks onto l9/dirt-shelf and porcelain is empty | quality_gate | observe .pre-commit-config.yaml catalog | true |
| SP-02 | SP-02 and SP-09 paths matching origin/main or an open-PR blob are removed and not parked | quality_gate | observe .pre-commit-config.yaml catalog | true |
| SP-03 | SP-11 --status reports dirty_unique=0 on a 50-file already-on-baseline pile | quality_gate | observe .pre-commit-config.yaml catalog | true |
| SP-04 | SP-10 absorbed dirt-shelf tip is deleted after tip SHA is recorded | quality_gate | observe .pre-commit-config.yaml catalog | true |

## Scope (from PLAN_DOCUMENT)

**In:** ops/scripts/session_end_dirt_close.py, ops/hooks/session_end_repo_hygiene.sh, ops/scripts/repo_hygiene.py protect l9/dirt-shelf, ops/scripts/REPO_HYGIENE.md, AGENTS.md L9_SESSION_END_DIRT_CLOSE_V1, rules/49-shared-worktree-isolation.mdc, tests/ops/scripts/test_session_end_dirt_close.py

**Out:**
- /ff drain or l9-repo-sync changes
- make pr or git push from sessionEnd
- SSOT write-gate or activate refuse-swap
- sibling worktree cleaning
- prune_execute.py / CANONICAL_LAW.md

## Critical path (seed)

todo-branch → todo-close-module → todo-prune-absorbed → todo-hook-gates → todo-tests → todo-status-doctrine → todo-verify

## Stress (seed from PLAN_DOCUMENT)

- Blast radius: wrong restore mid-edit destroys in-flight work; wrong open-PR match drops a same-path same-sha256 copy; wrong hygiene delete of dirt-shelf loses novel parked bytes
- Rollback: L9_HYGIENE_DIRT_CLOSE=0 restores prior hook behavior; shelf tip SHA in receipts; revert the feature commits

## Convergence (seed)

- status: partial
- next_skill: Build (current checkout)
- stop_reason: plan validated structurally; implementation in progress
- execute_via: cursor-build
