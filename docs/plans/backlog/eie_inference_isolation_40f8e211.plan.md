...
overview: "Emit a compile_brief-admissible INTENT for Quantum-L9/Enrichment.Inference.Engine that restores deterministic RuleEngine inference and tenant-authoritative idempotency/persistence (Green 6, then 45, then 35, then 40), then hand that memo to make campaign INTENT= without mutating app/ from the planning agent."
todos:
  - id: T-A
    content: "Restore deterministic RuleEngine inference against PlasticOS inference_rules and prove the HDPE cascade"
    status: pending
    phase: execute
    depends_on: []
  - id: T-B
    content: "Lock one Alembic tree on migrations/ with one head; leave the alembic/versions orphan out of the chain"
    status: pending
    phase: execute
    depends_on: [T-A]
  - id: T-C
    content: "Make Redis and Postgres idempotency tenant-scoped, fingerprint-safe, and UNIQUE(tenant_id, idempotency_key)"
    status: pending
    phase: execute
    depends_on: [T-B]
  - id: T-D
    content: "Require tenant predicates on ResultStore lookups and schema-proposal approval"
    status: pending
    phase: execute
    depends_on: [T-C]
  - id: T-E
    content: "Make transport tenant authoritative and reject payload tenant mismatch"
    status: pending
    phase: execute
    depends_on: [T-C]
isProject: false
...
# PLAN: EIE inference isolation v1
...
> **Suggested filename:** `eie-inference-isolation-v1_40f8e211.plan.md`

## Objective (from PLAN_DOCUMENT)

Emit a compile_brief-admissible INTENT for Quantum-L9/Enrichment.Inference.Engine that restores deterministic RuleEngine inference and tenant-authoritative idempotency/persistence (Green 6, then 45, then 35, then 40), then hand that memo to make campaign INTENT= without mutating app/ from the planning agent.

### Success properties (seed — complete evidence_type/proof in template sections)

| id | property | evidence_type | proof | blocking |
|----|----------|---------------|-------|----------|
| SP-01 | scoped brief wc -c is at most 61351 bytes (30 percent of the 204505-byte corpus) | quality_gate | observe during PE verify / make pr-check | true |
| SP-02 | brief contains five Release A through E headings and an It is: objective extractable by compile_brief.py | quality_gate | observe during PE verify / make pr-check | true |
| SP-03 | campaign_input.py classifies the brief as kind brief with route brief -> activate -> campaign_source -> blueprint -> PEC | quality_gate | observe during PE verify / make pr-check | true |
| SP-04 | validate_plan_document.py PASS on this PLAN_DOCUMENT | quality_gate | observe during PE verify / make pr-check | true |
| SP-05 | projected .plan.md retains Execute via @environment/program-execution + autonomy and names the INTENT path | quality_gate | observe during PE verify / make pr-check | true |
| SP-06 | PE Task Cards exclude Greens 30-34, 36-39, 41-50, HTTP tenant invention, and v2/DAG deletion | quality_gate | observe during PE verify / make pr-check | true |

## Scope (from PLAN_DOCUMENT)

**In:** .cursor-commands/WIP/PE EIE/eie-inference-isolation-v1.md, .cursor/plans/eie_inference_isolation.plan.json, .cursor/plans/eie_inference_isolation_<8hex>.plan.md, Release A Green 6 RuleEngine wiring files listed on T-A, Release B Alembic lineage files listed on T-B, Release C tenant Redis and Postgres idempotency files listed on T-C, Release D ResultStore and approval tenant predicates listed on T-D, Release E transport-tenant authority files listed on T-E

**Out:**
- Greens 30-34 peer-boundary mutations
- Greens 36-39 outbox, effect semantics, catalog, GRAPH inbox
- Greens 41-50 proposals, enrich-path collapse, convergence, ops, release closure
- PlasticOS rewrite to v2 or derived_from
- delete inference_bridge_v2
- direct HTTP /api/v1 tenant invention
- deprecated chassis/router.py production dispatch
- l9-plan or l9-pe-campaign-activate skill-pack edits
- COMPILE_ALLOWLIST.yaml edits
- make campaign / pec / push / PR / merge from this planning emit
- app/ or migrations/ writes by the planning agent
...
- Blast radius: Wrong isolation semantics leak cross-tenant cache or DB hits; wrong inference wiring leaves converge inert or invents v2; T4 handler edits can break C-13 lockstep.
- Rollback: Planning emit: delete the brief, PLAN_DOCUMENT JSON, and projected .plan.md. PE run: do not merge campaign/eie-inference-isolation-v1; revert the campaign branch.

## Convergence (seed)

- status: partial
- next_skill: l9-ynp
- stop_reason: planning emit complete; product execution is make campaign INTENT= the scoped brief, not pec and not this planning agent
