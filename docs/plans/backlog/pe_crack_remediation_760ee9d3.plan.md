...
> **Dual-artifact status law:** JSON `convergence.status=partial` because V1–V6 are pending. Markdown `status=executable` means the *plan contract* is ready to project through PE+autonomy. It does **not** mean implementation evidence has passed.
