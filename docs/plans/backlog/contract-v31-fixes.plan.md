---
name: Contract v3.1 — land the two unadopted review fixes and the three v3 regressions
overview: "Amend the claude_code_mobile_bootstrap_remediation contract from v3.0.0 to v3.1.0 so that it (a) restores and asserts partial-installer-run safety, (b) makes the SessionStart self-heal time budget a measured preflight obligation rather than an assumption, (c) replaces a C11B assertion that is guaranteed to fail once the remediation merges, (d) actually verifies Context7 reachability and defines..."
todos:
  - id: A1
    content: "Create the v3.1 contract from v3.0.0 and land partial-run safety: restore C1.failures (write_lock_unavailable, installer_timeout, receipt_still_mismatched), add the partial_installer_run_is_safe_to_rerun invariant and installer_steps_verified_idempotent precondition, add the P1 zero-diff verification action plus output and BLOCKED failure, and add the regression test"
    status: pending
    phase: execute
    depends_on: []
  - id: A2
    content: "Make the self-heal time budget a measured preflight obligation: add P1 actions measuring warm install.sh wall time under an isolated HOME and disposable workspace clone plus the SessionStart prefix, emit warm_install_seconds / sessionstart_prefix_seconds / available_self_heal_budget_seconds, BLOCK when the budget is below the measured install time, and gate C1 on it"
    status: pending
    phase: execute
    depends_on: [A1]
  - id: A3
    content: "Replace the C11B assertion observed_governance_revision == INPUT_EXECUTION_SHA with a staleness check against live origin/main at observation time, so the lane does not self-destruct the moment the remediation merges"
    status: pending
    phase: execute
    depends_on: [A1]
  - id: A4
    content: "Give C11B something that actually proves its objective: add an api.context7.io egress probe with an exact expectation, and replace the undefined rebuild_observed boolean with a stated measurement (observing session's receipt written after the C11A application time)"
    status: pending
    phase: execute
    depends_on: [A3]
  - id: A5
    content: "Record the brokered-path evidence on C11A and mark PENDING_NEXT_SESSION as the expected resting state on C11B until broker deployment, so the retained lane is honest about what it can and cannot achieve"
    status: pending
    phase: execute
    depends_on: [A4]
isProject: false
...
> **Suggested filename:** `contract-v3-1-land-the-two-unadopted-review-fixes-and-the-three-v3-regressions_730ad903.plan.md`

## Objective (from PLAN_DOCUMENT)

Amend the claude_code_mobile_bootstrap_remediation contract from v3.0.0 to v3.1.0 so that it (a) restores and asserts partial-installer-run safety, (b) makes the SessionStart self-heal time budget a measured preflight obligation rather than an assumption, (c) replaces a C11B assertion that is guaranteed to fail once the remediation merges, (d) actually verifies Context7 reachability and defines rebuild_observed as a measurable, and (e) records the brokered-path evidence behind the operator decision to retain the cloud_activation lane. This plan edits the contract document only; it does not modify install.sh, the SessionStart hook, or any adapter code.

### Success properties (seed — complete evidence_type/proof in template sections)

| id | property | evidence_type | proof | blocking |
|----|----------|---------------|-------|----------|
| SP-01 | C1_SESSIONSTART_SELF_HEAL carries a failures block again, satisfying governance.require and repairing the false internal_consistency: PASS claim in v3.0.0 | quality_gate | observe during PE verify / make pr-check | true |
| SP-02 | C1 declares partial_installer_run_is_safe_to_rerun as an invariant and installer_steps_verified_idempotent as a precondition | quality_gate | observe during PE verify / make pr-check | true |
| SP-03 | P1_REPOSITORY_PREFLIGHT emits installer_steps_verified_idempotent and BLOCKs on a non-idempotent install.sh step | quality_gate | observe during PE verify / make pr-check | true |
| SP-04 | P1 emits warm_install_seconds, sessionstart_prefix_seconds and available_self_heal_budget_seconds | quality_gate | observe during PE verify / make pr-check | true |
| SP-05 | P1 BLOCKs when available_self_heal_budget_seconds is below warm_install_seconds, halting C1 with a required-timeout-increase report | quality_gate | observe during PE verify / make pr-check | true |
| SP-06 | C12_CORE_REGRESSION_SUITE requires a self_heal_partial_run_is_safe_to_rerun test | quality_gate | observe during PE verify / make pr-check | true |
| SP-07 | C11B no longer compares observed_governance_revision against the frozen INPUT_EXECUTION_SHA | quality_gate | observe during PE verify / make pr-check | true |
| SP-08 | C11B validates egress to api.context7.io with an exact machine-checkable expectation | quality_gate | observe during PE verify / make pr-check | true |
| SP-09 | C11B defines rebuild_observed with a stated measurement method rather than an undefined boolean | quality_gate | observe during PE verify / make pr-check | true |
| SP-10 | C11A records the brokered-path evidence and C11B records that PENDING_NEXT_SESSION is the expected resting state until broker deployment | quality_gate | observe during PE verify / make pr-check | true |
| SP-11 | The v3.1 contract parses as YAML and passes the five-fix assertion script without manual inspection | quality_gate | observe during PE verify / make pr-check | true |

## Scope (from PLAN_DOCUMENT)

**In:** WIP/claude-code-mobile-environment/claude-code-mobile-remediation.contract.v3.1.yaml, contract task C1_SESSIONSTART_SELF_HEAL (failures, preconditions, invariants), contract task P1_REPOSITORY_PREFLIGHT (actions, outputs, failures), contract task C11A_ALLOWLIST_DECLARATION (brokered_path_note), contract task C11B_REBUILD_OBSERVATION (actions, outputs, validation, failure_behavior), contract task C12_CORE_REGRESSION_SUITE (required_tests)

**Out:**
- install.sh, session_start_claude_governance.sh, session_deps_cloud.sh and every other adapter file — this plan edits the contract, not the code it governs
- Deleting or retargeting the cloud_activation lane — operator decided on 2026-08-20 to retain it as-is
- Building completion-marker or rollback machinery for the self-heal — operator selected verify-and-assert
- Restoring failures on the seven other tasks that lack it (C2, C3, C4, C8, C10A, C11A, C12_CORE, V1) — pre-existing in v2, follow-on
- Restoring TODO to forbidden_markers — follow-on
- Naming the V3 assertion source — separate residual item
- git push, pull request creation, merge
...
- Blast radius: One new YAML file under WIP/. No executable path changes, no adapter code, no runtime behaviour until the contract is executed.
- Rollback: Delete the v3.1 file; v3.0.0 semantics are unchanged because v3.0.0 is not itself committed and nothing imports the new file.

## Convergence (seed)

- status: partial
- next_skill: l9-ynp
- stop_reason: Both operator decisions are taken and recorded: the cloud_activation lane is retained with A3 and A4 applied, and the self-heal fix is verify-and-assert rather than new machinery. U1 and U2 are execution-time probes that A2 and A1 themselves install as P1 obligations, so they are discharged by running the plan rather than blocking it. All five todos are grounded in files and line anchors verified this session at b406fee.
...
| Field | Value |
|-------|-------|
| captured_at | 2026-08-20 (session) |
| repository | `Quantum-L9/Cursor-Governance` |
| workspace | `/home/user/Cursor-Governance` |
| ssot_clone | `/root/.cursor-governance` @ `b406feeb4734f7029c36d718a68b004cacd6a68a` (branch `main`) |
| branch | `claude/mobile-bootstrap-readiness-audit-aoi0mn` |
| commit_sha | `941ab775c3e6d2a4d8b0425b10e9cb32b9a8e403` |
| dirty | `false` |
| plan_document | `docs/plans/claude-code/contract-v31-fixes.plan.json` |
| artifact_hashes | `{ "docs/plans/claude-code/contract-v31-fixes.plan.json": "sha256:f391774f587ad43f7888537a5ad00744400766a2afb91350bb7e5c58bef14c7b" }` |
| allowed_local_dirt | `.cursor/plans/**`, `reports/**` (gitignored working copies) |
| overlap_policy | `stop_if_dirty_overlaps_may_modify` |
| verification_rule | `reverify_at_execution_start` |
| on_drift | `stop_and_replan` |

**Operator decisions pinned into this baseline (2026-08-20).** The `cloud_activation` lane is **retained as-is**; fixes A3 and A4 are applied to it rather than deleting `C11A`/`C11B`. The self-heal fix is **verify-and-assert**, not new rollback or completion-marker machinery. Both decisions were taken with the P1-P6 probe evidence in hand.
