...
todos:
  - id: t0-isolate
    ...
    depends_on: []
  - id: t1-audit
    content: "Read the kernel by path; bind live root files vs skill write targets; record invented-file and competing-SSOT findings"
    status: pending
    phase: execute
    depends_on: [t0-isolate]
  - id: t2-skill
    content: "Rewrite l9-update-agent-docs as pointer maintainer with named write rules and a path citation"
    status: pending
    phase: execute
    depends_on: [t1-audit]
  - id: t3-agents-append
    content: "Append-only AGENTS.md root-doc authority map; do not fold"
    status: pending
    phase: execute
    depends_on: [t2-skill]
  - id: t4-companions
    content: "Align AUTONOMY_MANIFEST and regenerate skill-registry companions"
    status: pending
    phase: execute
    depends_on: [t2-skill]
  - id: t5-prove
    content: "Prove no-wrap and pointer invariants; make pr-check PASS"
    status: pending
    phase: execute
    depends_on: [t3-agents-append, t4-companions]
  - id: t6-plan-json
    content: "Emit validated PLAN_DOCUMENT JSON under docs/plans/"
    status: pending
    phase: execute
    depends_on: [t5-prove]
  - id: t7-vr-audit
    content: "Read kernels/Validate & Repair.md without copying; harvest named repair rules"
    status: pending
    phase: execute
    depends_on: [t6-plan-json]
  - id: t8-vr-skill
    content: "Add V&R path citation and named repair rules to l9-update-agent-docs without wrapping the kernel"
    status: pending
    phase: execute
    depends_on: [t7-vr-audit]
  - id: t9-vr-agents
    content: "Append-only AGENTS.md V&R path-citation marker; do not fold"
    status: pending
    phase: execute
    depends_on: [t8-vr-skill]
  - id: t10-vr-prove
    content: "Extend self_test for both kernel path citations; make pr-check PASS"
    status: pending
    phase: execute
    depends_on: [t9-vr-agents]
  - id: t11-vr-plan-json
    content: "Update PLAN_DOCUMENT JSON and PE projection for V&R scope"
    status: pending
    phase: execute
...
> **Suggested filename:** `port-recursive-alignment-and-validate-repair-to-root-docs-pointer-no-kernel-wrap_f3976fc4.plan.md`
...
|----|----------|---------------|-------|----------|
| SP-01 | CLAUDE.md first heading remains authority pointer and the skill write table does not add Always/Never or CI sections to CLAUDE.md | quality_gate | observe during PE verify / make pr-check | true |
| SP-02 | AGENTS.md gain is append-only (ROOT_DOC_AUTHORITY_MAP_V1 and ROOT_DOC_VR_REPAIR_V1) with no ALLOW-ROOT-DELETION marker | quality_gate | observe during PE verify / make pr-check | true |
| SP-03 | skills/l9-update-agent-docs contains path citations to kernels/Recursive Alignment.md and kernels/Validate & Repair.md and rg finds no Kernel-bind or artifact_type kernel YAML | quality_gate | observe during PE verify / make pr-check | true |
| SP-04 | skill write targets are existing AGENTS.md, CLAUDE.md, README.md only | quality_gate | observe during PE verify / make pr-check | true |
| SP-05 | self_test requires both kernel path citations and honest-validation status names | quality_gate | observe during PE verify / make pr-check | true |
| SP-06 | make pr-check PASS in the isolated worktree | quality_gate | observe during PE verify / make pr-check | true |
...
- Claude SessionStart parity work and primary-checkout dirt
- Merge, raw push, workflow edits

## Critical path (seed)

t0-isolate → t1-audit → t2-skill → t3-agents-append → t4-companions → t5-prove → t6-plan-json → t7-vr-audit → t8-vr-skill → t9-vr-agents → t10-vr-prove → t11-vr-plan-json

## Stress (seed from PLAN_DOCUMENT)

- Blast radius: every Claude and Cursor session that loads CLAUDE.md or AGENTS.md; every later root-doc refresh
- Rollback: revert the feature-branch commits in the worktree; do not rewrite CANONICAL_LAW.md

## Convergence (seed)

- status: converged
- next_skill: l9-ynp
- stop_reason: pointer stack landed; both kernels cited by path; self_test and make pr-check PASS; neither kernel wrapped
