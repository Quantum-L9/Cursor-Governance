...
todos:
  - id: fix-company-type-bug
    content: "BLOCKER FIX: Replace company_type with is_company in validation.py and partner_import_service.py (company_type is computed, not stored)"
    status: pending
  - id: enable-crm
...
## Phase 0: BLOCKER BUG FIX - company_type is Computed

**Error**: `ValueError: Cannot convert res.partner.company_type to SQL because it is not stored`

**Root Cause**: In Odoo 19, `res.partner.company_type` is a **computed field** derived from `is_company`. It cannot be used in:

- Domain searches (SQL WHERE clauses)
- Direct writes (vals dictionary)

### IMPORTANT DISTINCTION - Two Different "Company Types"


| Term                                 | Odoo Field                 | Values                                                                                            | Purpose                                           |
| ------------------------------------ | -------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| `company_type` (Odoo internal)       | Computed from `is_company` | `"company"` / `"person"`                                                                          | Legal entity type - NOT importable, NOT stored    |
| **"Company Type" (VanillaSoft CSV)** | `res.partner.category_id`  | Distribution Center, Pallet Recycler, Commercial Recycler, Compounder, E-Waste, Grinder_Processor | **Business classification - MUST IMPORT as tags** |


These are **completely different concepts**:

- The **bug fix** addresses Odoo's internal `company_type` field (replace with `is_company` in search domains)
- The **VanillaSoft "Company Type" column** is REAL DATA that must be imported to `category_id` (partner tags) - covered in Phase 7

**VanillaSoft "Company Type" values found in CSV (create as `res.partner.category`):**
...
**Fix**: Replace Odoo `company_type` references with `is_company` in search domains:

### File: [plasticos_partner_import/models/validation.py](plasticos_partner_import/models/validation.py)


| Line   | Old (BROKEN)                       | New (FIXED)                  |
| ------ | ---------------------------------- | ---------------------------- |
| 43-44  | `("company_type", "=", "company")` | `("is_company", "=", True)`  |
| 44     | `("company_type", "=", "person")`  | `("is_company", "=", False)` |
| 51, 56 | `p.company_type == "company"`      | `p.is_company`               |
| 62     | `p.company_type == "person"`       | `not p.is_company`           |
| 107    | `("company_type", "=", "company")` | `("is_company", "=", True)`  |
| 118    | `("company_type", "=", "person")`  | `("is_company", "=", False)` |
| 176    | `("company_type", "=", "company")` | `("is_company", "=", True)`  |


### File: [plasticos_partner_import/models/partner_import_service.py](plasticos_partner_import/models/partner_import_service.py)

Remove `company_type` from all vals dictionaries (lines 248, 347, 363, 453, 481, 526, 558). The `is_company` field is already set correctly - `company_type` is redundant and causes no effect.


| Line | Change                              |
| ---- | ----------------------------------- |
| 248  | DELETE `"company_type": "company",` |
| 347  | DELETE `"company_type": "company",` |
| 363  | DELETE `"company_type": "person",`  |
| 453  | DELETE `"company_type": "person",`  |
| 481  | DELETE `"company_type": "company",` |
| 526  | DELETE `"company_type": "company",` |
| 558  | DELETE `"company_type": "person",`  |


### File: [plasticos_partner_import/wizards/partner_bulk_update_wizard.py](plasticos_partner_import/wizards/partner_bulk_update_wizard.py)

The wizard already correctly writes to `is_company` (line 242), but the field name `company_type` is used for the wizard's own Selection field. This is fine - it's a wizard field, not writing to `res.partner.company_type`. No change needed here.
...
Categories to create (master data from VanillaSoft CSV + existing business types):

**From VanillaSoft "Company Type" column:**

- Distribution Center
- Pallet Recycler
- Commercial Recycler
- Compounder
- Grinder_Processor (or "Grinder / Processor")
- E-Waste
