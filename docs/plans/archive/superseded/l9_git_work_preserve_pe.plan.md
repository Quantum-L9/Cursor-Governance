...
status: superseded
built: true
overview: "Ship diagnose-first skill l9-git-work-preserve with /git-work-preserve: inventory unpushed/dirty/orphan/stale/stash work with receipts, extract unique value safely, prune report-only by default; fix sync_cursor_plan_template home-symlink check; append CANONICAL_LAW section 11 Diagnose First path to WIP/backlog full kernel."
todos:
  - id: built-marker
    content: Marked built after execution; session-start audit should skip
    status: cancelled
  - id: todo-00c-fix-template-sync
    content: "Fix sync_cursor_plan_template _under_repo so .cursor/plans home symlink mirror is allowed; --check PASS"
    status: cancelled
    phase: execute
    depends_on: []
  - id: todo-01-baseline
    content: "Create worktree fix/l9-git-work-preserve from origin/main; L4 begin; Program Lock; lock full SHA"
    status: cancelled
    phase: execute
    depends_on: []
  - id: todo-02-design
    content: "Write diagnose-first refs prune stash policy and receipt schema under skills/l9-git-work-preserve/references/"
    status: cancelled
    phase: execute
    depends_on: [todo-01-baseline]
  - id: todo-03-scripts
    content: "Implement inventory_git_work.py diagnose_ref_value.py and fixture self_test"
    status: cancelled
    phase: execute
    depends_on: [todo-02-design]
  - id: todo-04-compile
    content: "Compile SKILL.md and agents/meta.yaml zero-stub pack via l9-skill-compiler"
    status: cancelled
    phase: execute
    depends_on: [todo-02-design, todo-03-scripts]
  - id: todo-05-slash-docs
    content: "Add /git-work-preserve command manifests index rule 02; append CANONICAL_LAW section 11 to backlog Diagnose First path"
    status: cancelled
    phase: execute
    depends_on: [todo-04-compile]
  - id: todo-06-wire
    content: "Run l9-wire-skill-into-repo for l9-git-work-preserve SSOT-only symlink"
    status: cancelled
    phase: execute
    depends_on: [todo-05-slash-docs]
  - id: todo-07-prove
    content: "Run pack self_test make pr-check dry-run audit and sync_cursor_plan_template --check"
    status: cancelled
    phase: execute
    depends_on: [todo-00c-fix-template-sync, todo-06-wire]
  - id: todo-08-converge
    content: "Run Recursive Alignment and Validate Repair kernels; L4 authorize-release; make pr; l9-pr-remediation; merge per L4 Build"
    status: cancelled
    phase: execute
    depends_on: [todo-07-prove]
isProject: false
---
# PLAN: l9-git-work-preserve
...
> **Suggested filename:** `l9-git-work-preserve_eca65713.plan.md`

## Objective (from PLAN_DOCUMENT)

Ship diagnose-first skill l9-git-work-preserve with /git-work-preserve: inventory unpushed/dirty/orphan/stale/stash work with receipts, extract unique value safely, prune report-only by default; fix sync_cursor_plan_template home-symlink check; append CANONICAL_LAW section 11 Diagnose First path to WIP/backlog full kernel.

### Success properties (seed — complete evidence_type/proof in template sections)

| id | property | evidence_type | proof | blocking |
|----|----------|---------------|-------|----------|
| SP-01 | skills/l9-git-work-preserve pack self_test.py and validate_pack_structure.py exit 0 | quality_gate | observe during PE verify / make pr-check | true |
| SP-02 | commands/git-work-preserve.md registered in COMMANDS_MANIFEST.yaml commands-index.md and rules/02-slash-commands.mdc | quality_gate | observe during PE verify / make pr-check | true |
| SP-03 | inventory_git_work.py emits JSON covering unpushed dirty worktrees orphans stashes on fixture repo | quality_gate | observe during PE verify / make pr-check | true |
| SP-04 | skill refs bind WIP/backlog/kernels/diagnose-first/Diagnose First Kernel.md and forbid stash drop or branch delete without auth env plus receipt | quality_gate | observe during PE verify / make pr-check | true |
| SP-05 | l9-wire-skill-into-repo registers l9-git-work-preserve with SSOT symlink | quality_gate | observe during PE verify / make pr-check | true |
| SP-06 | python3 skills/l9-plan/scripts/sync_cursor_plan_template.py skills/l9-plan --check exits 0 with .cursor/plans home symlink | quality_gate | observe during PE verify / make pr-check | true |
| SP-07 | make pr-check PASS on changed files | quality_gate | observe during PE verify / make pr-check | true |
| SP-08 | Build runs only in dedicated worktree not dirty primary clone | quality_gate | observe during PE verify / make pr-check | true |

## Scope (from PLAN_DOCUMENT)

**In:** skills/l9-git-work-preserve pack, skills/l9-plan/scripts/sync_cursor_plan_template.py symlink confinement fix, commands/git-work-preserve.md and COMMANDS_MANIFEST.yaml and commands-index.md, rules/02-slash-commands.mdc and rules/88-shared-worktree-isolation.mdc cross-link, CANONICAL_LAW.md append-only section 11 Diagnose First path, skill registries via l9-wire-skill-into-repo

**Out:**
- worktree_isolation_gate semantics changes
- backup or sessionEnd hook rewrites
- landing untracked kernels/Diagnose First Kernel.md
- section 1 /plan table cell rewrite
- force-push hard-reset admin-merge

## Critical path (seed)

todo-01-baseline → todo-02-design → todo-03-scripts → todo-04-compile → todo-05-slash-docs → todo-06-wire → todo-07-prove → todo-08-converge

## Stress (seed from PLAN_DOCUMENT)

- Blast radius: skill misuse can delete local branches or stashes; sync fix affects all governed workspaces with home-linked .cursor/plans; law append changes Diagnose First pointer for all agents
- Rollback: revert feature PR; unwire skill; restore sync_cursor_plan_template.py; law append remains (append-only) with clarifying follow-on if needed; recover deleted refs via reflog SHA in receipts

## Convergence (seed)

- status: partial
- next_skill: /autonomy + @environment/program-execution
- stop_reason: PLAN_DOCUMENT validated; implementation Build starting
