...
todos:
  ...
  - id: phase-2a-runtime-config
    content: Create engine/orchestration/runtime_config.py
    status: completed
  - id: phase-2b-runtime-rewrite
    content: Rewrite engine/orchestration/runtime.py with full wiring
    status: completed
  - id: phase-2c-policy-context-builder
    content: Add build_policy_context() to engine/orchestration/policy_bridge.py
    status: completed
  - id: phase-3-handlers
    content: Refactor engine/handlers.py to invoke ExecutionRuntime
    status: completed
  - id: phase-4-env-config
    content: Create engine/config.py with load_runtime_config()
    status: completed
  - id: phase-5a-unit-runtime
    content: Create tests/unit/test_execution_runtime.py
    status: completed
  - id: phase-5b-unit-services
    content: Create tests/unit/test_services.py
    status: completed
  - id: phase-5c-integration
    content: Create tests/integration/test_policy_runtime_response.py
    status: completed
isProject: false
---
...
```python
# engine/orchestration/runtime_config.py
...
# Called once at startup in engine/handlers.py.
...
```python
# engine/orchestration/policy_bridge.py
# purpose: Map a TransportPacket to a PolicyContext.
# Kept separate from runtime.py for testability.
#
# VERIFIED field mapping (main @ b008e73):
#   classification → packet.security.classification  (str, NOT packet.governance)
#   pii_fields     → packet.security.pii_fields       (tuple[str,...], NOT governance.pii_paths)
#   compliance_tags→ packet.governance.compliance_tags (tuple[str,...])
#   audit_required → packet.governance.audit_required  (bool)
...
def build_policy_context(packet: TransportPacket) -> PolicyContext:
    ...
    return PolicyContext(
        ...
        pii_fields=packet.security.pii_fields,
...
```python
# engine/orchestration/runtime.py
...
class ExecutionRuntime:
    ...
    def _execute_inner(self, packet: TransportPacket) -> EngineResult:
        ...
        try:
            ...
            typed_command = self._command_factory.build(packet)
            ...
            if self._record_mapper is not None and self._ingestion is not None:
                records = self._record_mapper.map(packet, result)
...
#
# Thread safety: _RUNTIME_LOCK protects the singleton initialization
    ...
    if _runtime is None:
        with _RUNTIME_LOCK:
            if _runtime is None:  # re-check inside lock
                _runtime = ExecutionRuntime(load_runtime_config())
                logger.info("runtime.initialized: config=%r", _runtime._config)
    return _runtime


def _dispatch(packet: TransportPacket) -> dict[str, Any]:
    """Common dispatch wrapper used by all registered handlers."""
    result = _get_runtime().execute(packet, command=None)
    return result.model_dump(mode="json", exclude_none=True)


def _match_handler(packet: TransportPacket) -> dict[str, Any]:
    return _dispatch(packet)


def _sync_handler(packet: TransportPacket) -> dict[str, Any]:
    return _dispatch(packet)


def _admin_handler(packet: TransportPacket) -> dict[str, Any]:
    return _dispatch(packet)


def _replay_handler(packet: TransportPacket) -> dict[str, Any]:
    return _dispatch(packet)


# Register all handlers at import time.
# l9.chassis.registry.register_handler is thread-safe (uses threading.Lock).
register_handler("match", _match_handler)
register_handler("sync", _sync_handler)
register_handler("admin", _admin_handler)
register_handler("replay", _replay_handler)

logger.info("handlers.registered: actions=[match, sync, admin, replay]")
```
...
def _make_packet(
    ...
    packet.tenant.actor = "test-actor"
    packet.tenant.org_id = "test-org"
    ...
    packet.security.classification = classification
    packet.security.pii_fields = pii_fields
...
def test_runtime_idempotency_cache_hit():
    ...
    packet = _make_packet(idempotency_key="idem-key-1")
...
def test_runtime_idempotency_disabled_no_cache():
    ...
    packet = _make_packet(idempotency_key="idem-key-2")
...
def test_runtime_duplicate_packet_suppressed():
    ...
    rt = ExecutionRuntime(RuntimeConfig(enable_policy=False, enable_memory_ingestion=False))
    packet = _make_packet(idempotency_key=None)
...
def test_runtime_policy_allows_internal():
    ...
    packet = _make_packet(classification="internal")
...
def test_runtime_policy_denies_restricted_delegation():
    ...
    rt = ExecutionRuntime(RuntimeConfig(enable_idempotency=False, enable_memory_ingestion=False))
    packet = _make_packet(classification="restricted", packet_type="delegation")
    result = rt.execute(packet, command=None)
...
def test_runtime_deadline_exceeded_returns_failed_retryable():
    ...
    rt = ExecutionRuntime(RuntimeConfig(enable_policy=False, enable_memory_ingestion=False))
    packet = _make_packet(expires_at=datetime(2000, 1, 1, tzinfo=timezone.utc))
    result = rt.execute(packet, command=None)
...
def test_runtime_no_deadline_passes():
    ...
    rt = ExecutionRuntime(RuntimeConfig(enable_policy=False, enable_memory_ingestion=False))
    packet = _make_packet(expires_at=None)
    result = rt.execute(packet, command=None)
...
def test_runtime_match_action_dispatches_to_match_service():
    ...
    packet = _make_packet(action="match")
    result = rt.execute(packet, command=None)
    assert result.status == "completed"
    assert "enriched" in result.data
...
def test_runtime_admin_action_dispatches_to_admin_service():
    ...
    packet = _make_packet(action="admin")
    result = rt.execute(packet, command=None)
...
def test_runtime_boundary_error_retryable():
    ...
    with patch.object(rt._query_plan, "resolve") as mock_resolve:
        def _raise(_): raise L9BoundaryError(
...
from engine.application.results import EngineResult
from engine.application.services import AdminService, MatchService, ReplayService, SyncService


def _make_context(action: str = "match") -> ExecutionContext:
...
def test_classification_sourced_from_security():
    """classification must come from packet.security.classification."""
    p = _make_packet(classification="confidential")
    ctx = build_policy_context(p)
    assert ctx.classification == "confidential"


def test_pii_fields_sourced_from_security():
    """pii_fields must come from packet.security.pii_fields, not governance."""
    p = _make_packet(pii_fields=("payload.ssn", "payload.dob"))
    ctx = build_policy_context(p)
...
def test_compliance_tags_sourced_from_governance():
    p = _make_packet(compliance_tags=("GDPR", "SOC2"))
    ctx = build_policy_context(p)
...
def test_policy_context_is_valid_pydantic_model():
    ...
    p = _make_packet(classification="internal", pii_fields=(), compliance_tags=())
    ctx = build_policy_context(p)
...
def _make_packet(action="match", classification="internal", idempotency_key=None):
    p = MagicMock()
    ...
    p.header.replay_mode = False
    p.header.packet_type = "request"
    p.tenant.actor = "integ-actor"
    p.tenant.org_id = "integ-org"
    p.tenant.originator = "integ-origin"
    p.tenant.user_id = None
    p.address.source_node = "client"
    p.address.destination_node = "gate"
    p.security.classification = classification
    p.security.pii_fields = ()
...
def test_full_pipeline_match_action():
    ...
    packet = _make_packet(action="match", idempotency_key="integ-idem-1")
...
def test_full_pipeline_idempotency_second_call():
    ...
    packet = _make_packet(idempotency_key="integ-idem-2")
...
def test_full_pipeline_policy_rejection():
    ...
    packet = _make_packet(classification="restricted", action="match")
...
| Observability     | 30%    | 60%   | Structured logging throughout; `MetricsCollector`/`AuditLogger`/`TraceManager` wiring deferred to Phase 3 (after memory substrate confirmed stable) |
