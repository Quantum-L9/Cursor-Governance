...
overview: "Slim local make pr / make pr-check / Makefile push so every tool scans only new or changed code (pytest via mapped tests). Full-corpus work stays on make pr-full. GitHub required checks stay full-tree. Makefile and .pre-commit-config.yaml edits require ALLOW-ROOT-DELETION."
kind: pe-autonomy
execute_via: program-execution
...
todos:
  ...
  - id: todo-03-wire-gate
    ...
    status: pending
    phase: execute
    depends_on: [todo-02-pytest-mapper]
    side_effect_ref: SE-todo-03
  ...
  - id: todo-06-pr-full-owner
    ...
    status: pending
    phase: execute
    depends_on: [todo-04-precommit-skip, todo-05-dedupe-push]
  ...
  - id: todo-07-docs
    ...
    status: pending
    phase: execute
    depends_on: [todo-06-pr-full-owner]
  ...
  - id: todo-08-prove
    ...
    status: pending
    phase: validate
    depends_on: [todo-03-wire-gate, todo-04-precommit-skip, todo-05-dedupe-push, todo-06-pr-full-owner, todo-07-docs]
  ...
  - id: todo-09-converge
    ...
    status: pending
    phase: converge
    depends_on: [todo-08-prove]
    side_effect_ref: SE-todo-09
    evidence_property_refs: [SP-03]
    files:
      - ops/autonomy/l4_local.py
isProject: false
kernel_pass:
  bound_path: /Users/ib-mac/Cursor-Governance/docs/plans/pr_gate_velocity_8b9391f7.plan.md
  improve:
    kernel: kernels/Improve.md
    ran_at: 2026-08-21T20:04:00Z
    deltas:
      - "Bind absolute INTENT path so make -C $HOME/.cursor-governance can locate this workspace plan"
      - "Copy PLAN_DOCUMENT files onto frontmatter todos so compile_brief emits paths without a sidecar"
      - "Replace write_deny ellipsis with an explicit dirty-WIP bound"
  validate_repair:
    kernel: kernels/Validate & Repair.md
    ran_at: 2026-08-21T20:06:00Z
    body_sha256: "f0abfef3fbe5e588a8d4960be7a9c5d7e040e108795c79daf6b92331e30ed671"
    deltas:
      - "Bind target_repo Quantum-L9/Cursor-Governance so extract_target does not take HOME/.cursor-governance"
      - "Confirm no exclusive-list etc/ellipsis and no unresolved exclusive lock"
      - "Seal kernel_pass receipt on the latched plan path"
---
...
- plan_id: `plan.ops.pr-gate-changed-files-velocity.v1`
- status: `draft` (becomes `executable` only after todo-01 reverifies baseline on a clean worktree)
...
- on_drift: `stop_and_replan` (U1: tip may have moved by execute start)
...
- next_skill: `l9-ynp` → `/autonomy` + PE on a new `origin/main` worktree
- stop_reason: Plan repaired in place; implementation not run; sidecar JSON not writable in plan-mode
- executable_when: todo-01 reverifies SHA; envelope + ALLOW-ROOT-DELETION respected; U1 resolved by probe
...
- **U1** (probe): Will `origin/main` still be `8bc5781df8b246b83c14d359d0e0803e6d2ef868` at execute? Relock tip; stop_and_replan if gate scripts drifted.
...
```json
{
  ...
  "checkpoints": [
    {"id": "C1", "after": "M1", "evidence_required": "git rev-parse HEAD equals locked origin/main SHA on the worktree", "no_go_action": "stop_and_replan"},
  ...
  "unknowns": [
    {
      ...
      "decision_effect": "todo-01 relocks the then-current origin/main tip; stop_and_replan if the gate scripts drifted",
      "resolution": "probe"
    }
  ],
  "final_validation": [
