#!/usr/bin/env python3
"""
compile_contract.py — claude-coding-contract-compiler v2.7.0
...
"""
import argparse, json, re, shlex, sys, pathlib

HERE = pathlib.Path(__file__).resolve().parent
SCHEMA_DIR = HERE.parent / "schemas"
sys.path.insert(0, str(HERE))
from validate_chain import compute_chain_digest, validate_chain  # single digest + seam source
import plan_decomposition as pdc                                  # fits_one, thresholds
import validate_contract as vc                                    # per-instance gate
import generate_claude_settings as gcs                            # optional artifacts
import generate_preflight as gpf

DEFAULT_DENIED = [
    "Bash(git push:*)", "Bash(git merge:*)", "Bash(gh pr create:*)",
    "Bash(gh pr merge:*)", "Bash(gh api *repos*:*)", "Bash(npm publish:*)",
]
DEFAULT_HALT = ["git_push", "pr_open", "pr_merge",
                "required_check_change", "ruleset_change", "branch_protection_change"]

SECTION_NAMES = [
    "Resume-From","Mandate","Prerequisite Contract","Scope Lock","Package Layout",
    "Registry/Config Authority","Risk/Tier Policy","Context Normalization","Input Acquisition",
    "Deterministic Planner","Safe Executor","Evidence Enrichment","Fail-Closed Evaluator",
    "Workflow Architecture","Matrix/Parallel Execution","Runtime Guarantee Preservation",
    "Self-Modification Protection","Schemas","Test Contract","Failure Evidence","Documentation",
    "Required File Manifest","Commit Plan","Validation Commands","Acceptance Criteria",
    "Non-Goals Verification","Required Validation Report","Definition of Done","Handoff","Governance",
]
...
def validate_spec(spec):
    ...
    if not isinstance(validation, dict):
        ...
        errors.extend(validate_command_list(cold.get("commands"), "campaign.validation.cold_resume.commands"))
        errors.extend(validate_command_list(gate.get("commands"), "campaign.validation.commit_gate.commands"))
...
def owns_path_check(in_scope_paths, owns):
    ...
    return ([f"SCOPE: in_scope path '{p}' is outside manifest.boundaries.owns" for p in bad],
...
def dpk_readiness(cats, manifest, rollback_target, has_ai, eval_suite):
    ...
    if reasons:
        return {"total": 0, "band": "rejected", "categories": cats,
                "red_line_triggered": True, "red_line_reasons": reasons}
...
def build_sections(na_map):
    ...
    for n, name in enumerate(SECTION_NAMES):
        ...
        if str(n) in na_map:
            e["status"] = "not_applicable"; e["not_applicable_reason"] = na_map[str(n)]
        out.append(e)
    return out
...
def build_instance(camp, item, prev_item, idx, all_ids, prev_id, next_id, digest,
                   commit_range, dpk_block, errors):
    cid = all_ids[idx]
    branch = camp["target_branch"]
    is_terminal = next_id is None
    allowed = item["allowed_files"]
    in_scope = [{"id": i + 1, "deliverable": a["deliverable"], "path": a["path"]}
    ...
    return {
        ...
        "scope_lock": {"in_scope": in_scope, "hard_out_of_scope": hard_oos,
                       "preserved_files": preserved, "must_not_add": hard_oos},
        "sections": build_sections(na),
        "agent_self_approval_forbidden": True,
        "promotion_ready": False,
        "handoff": {"next_session_may_assume_green": [f"{cid} committed_and_validated"],
                    "next_contract": next_id, "chain_digest": digest},
        "dpk": dpk_block,
        "session_budget": {
...
def main(argv):
    ...
    for it in items:
        m = {"new_files": it["sizing"]["new_files"], "commits": it["sizing"]["commits"],
             "matrix_cases": it["sizing"].get("matrix_cases", 0), "deliverables": len(it["allowed_files"])}
        ok, reasons = pdc.fits_one(m, cfg)
        if not ok:
            errors.append(f"[PR-{it['key']}] DECOMPOSE_REQUIRED: {'; '.join(reasons)} — "
                          f"split this item into ordered sub-items in the spec; do not compress.")

    out = pathlib.Path(args.out); out.mkdir(parents=True, exist_ok=True)
    instances, commit_cursor = [], 1
    for idx, it in enumerate(items):
        n = 1  # schema + validate_spec enforce exactly one local commit per contract
        commit_range = [commit_cursor]; commit_cursor += 1
        prev_id = all_ids[idx - 1] if idx > 0 else None
        next_id = all_ids[idx + 1] if idx < len(items) - 1 else None
        dpk = camp["dpk"]
        readiness = dpk_readiness(it["readiness"]["categories"], dpk["manifest"],
                                  dpk["rollback_target"], dpk.get("has_ai_feature", False),
                                  dpk.get("evaluation_suite"))
        dpk_block = {"manifest": dpk["manifest"], "rollback_target": dpk["rollback_target"],
                     "has_ai_feature": dpk.get("has_ai_feature", False),
                     "broken_alert_runbook_links": False,
                     "categories": it["readiness"]["categories"], "readiness": readiness}
        ...
        inst = build_instance(camp, it, prev_item, idx, all_ids, prev_id, next_id, digest,
                              commit_range, dpk_block, errors)
