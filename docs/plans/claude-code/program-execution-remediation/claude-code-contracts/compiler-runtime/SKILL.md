---
name: claude-coding-contract-compiler
description: compile, revise, harden, and validate scoped Claude Code coding-contract chains from canonical campaign specs. Use for coding contracts, PR/execution specs, cold-resumable multi-contract campaigns, or compiler validation. Requires explicit target-native validation, exactly one local commit per contract on one shared branch, predecessor-proof preflight for contract 2+, direct remote-mutation denials, and one terminal make pr delivery; fails closed instead of inferring npm or another ecosystem.
---

<!-- META (ignored by Claude Code runtime; human reference only)
layer: controlplane
role: skillentrypoint
owner: igorbeylin
version: 2.7.0
updated: 2026-08-27
tier: exemplary
convergence_status: fixed_point
compiled_by: l9-skill-compiler v3.3.0
sibling: perplexity-coding-contract-compiler v2.1.0
executor_profile: claude-code (hardwired)
compiler: l9-skill-compiler v3.5.0
-->
