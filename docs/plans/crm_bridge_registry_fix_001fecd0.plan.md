...
```xml
<!-- ... CRM Lead Action — Add pivot + calendar to Pipeline action ... -->
<record id="crm.crm_lead_all_pipeline" model="ir.actions.act_window">
    <field name="view_mode">kanban,list,form,activity,pivot,calendar</field>
</record>
```
...
- Before editing, sync the working tree to the latest server code so the fix lands on the right base:
  - `git fetch origin`
  - reset local `Staging` to `origin/Staging` (the file `crm_lead_views.xml` on origin has the broken record at line 276 and manifest at `19.0.3.2.0` — confirmed).
...
- Push directly: `git push origin Staging` (or `git push origin <local>:Staging`).

## Verify
- After push, Odoo.sh rebuilds Staging. Confirm via SSH that `update.log` ends with `Registry loaded` and no `ERROR`:
  - `ssh <build>@cryptoxdog-ib-odoo-19-staging-32807868.dev.odoo.com "tail -100 ~/logs/update.log"`
