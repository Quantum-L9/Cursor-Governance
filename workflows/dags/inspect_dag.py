...
class InspectState(BaseModel):
    ...
    is_external: bool = Field(
        default=False, description="True if target is external code (markdown, non-repo file)"
    )
    ...
    routing_decision: Literal[
        "/harvest-analyze", "/refactor-sweep", "/wire", "/gmp", "STOP", "FIX-BEFORE-IMPORT"
    ] = Field(default="STOP")
...
async def routing_node(state: InspectState) -> dict[str, Any]:
    ...
    if state.is_external:
        error_count = sum(
            1 for i in state.validation_issues if i.get("severity") in ("critical", "high")
        )
        if error_count > 0:
...
async def report_node(state: InspectState) -> dict[str, Any]:
    ...
    if state.validation_issues:
        errors = [i for i in state.validation_issues if i.get("severity") in ("critical", "high")]
        warnings = [i for i in state.validation_issues if i.get("severity") in ("medium", "low")]
