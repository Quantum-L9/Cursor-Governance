...
class GovernanceMonitor:
    ...
    def get_detailed_metrics(self) -> Dict:
        ...
        return {
            'recent_metrics': [dict(zip([col[0] for col in cursor.description], row)) 
                             for row in recent_metrics],
            'performance_summary': [dict(zip(['component_id', 'avg_duration', 'operation_count', 'success_count'], row)) 
