...
class CursorProbabilisticEngine:
    ...
    def _load_registry(self):
        ...
        self.calibration_params = registry.get(
    ...
    def _get_default_thresholds(self) -> dict:
        ...
        return {
            ...
            "medium_risk": {"value": 0.65, "target_precision": 0.75, "auto_adjust": True},
    ...
    def assess_file_compliance_risk(
        ...
        reasoning = self._build_reasoning(
            evidence_list, raw_probability, calibrated_probability, subjective
        )
    ...
    def _decompose_subjective_logic(self, evidence: list[Evidence]) -> SubjectiveLogic:
        ...
        return SubjectiveLogic(
            trust=round(trust, 2), disbelief=round(disbelief, 2), uncertainty=round(uncertainty, 2)
        )
    ...
    def _build_reasoning(
        ...
        top_factors = sorted(evidence, key=lambda e: abs(e.value - 0.5) * e.weight, reverse=True)[
            :3
        ]
        ...
        for e in top_factors:
            ...
            reasoning_parts.append(
                f"  • {e.name}: {e.value:.2f} (weight {e.weight}) - {direction} risk"
            )
    ...
    def _log_decision(
        self, assessment: ProbabilisticAssessment, evidence: list[Evidence], file_path: str
    ):
    ...
    def record_outcome(self, decision_id: str, outcome: str, feedback: str | None = None):
        """
    ...
    def calculate_ece(self, decisions: list[dict] | None = None) -> float:
        ...
        for bin_idx, bin_decisions in enumerate(bins):
            ...
            confidence = sum(d["calibrated_probability"] for d in bin_decisions) / len(
                bin_decisions
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Subjective: T={assessment1.subjective_logic.trust}, D={assessment1.subjective_logic.disbelief}, U={assessment1.subjective_logic.uncertainty}"
    )
    ...
    print(
        f"Subjective: T={assessment2.subjective_logic.trust}, D={assessment2.subjective_logic.disbelief}, U={assessment2.subjective_logic.uncertainty}"
    )
