...
class AgentStubManager:
    ...
    def validate_stub(self, stub_path: str) -> tuple[bool, dict]:
        ...
        try:
            ...
            issues = {"schema": [], "rules": [], "staleness": False, "kernel": []}
            ...
            if self.rule_registry_path.exists():
                with open(self.rule_registry_path) as f:
            ...
            if stub.get("governance_kernel") != "6.0":
                issues["kernel"].append(
                    f"Expected kernel version 6.0, got {stub.get('governance_kernel')}"
                )
            ...
            has_issues = any(
                [issues["schema"], issues["rules"], issues["staleness"], issues["kernel"]]
            )
    ...
    def _extract_component_id(self, file_path: Path) -> str | None:
...
def main():
    ...
    if command == "generate":
        ...
        print(
            f"Overall Coverage: {coverage['coverage_percentage']:.1f}% ({coverage['components_with_stubs']}/{coverage['total_components']})"
        )
        print("\nBy Layer:")
        ...
        failed_validations = [
            comp_id
            for comp_id, result in coverage["validation_status"].items()
