...
class GovernanceValidator:
    ...
    def _has_canonical_header(self, content: str, filepath: Path) -> bool:
        ...
        if filepath.suffix == ".md":
            return (
                content.startswith("---") and "# === L9 GOVERNANCE CANONICAL HEADER ===" in content
            )
        ...
        try:
            if filepath.suffix == ".md":
                ...
                header_match = re.search(
                    r'# === L9 GOVERNANCE CANONICAL HEADER ===.*?"""', content, re.DOTALL
                )
                if header_match:
    ...
    def validate_workspace(self, workspace_path: Path) -> dict:
        ...
        required_files = [".cursorrules", ".cursor-commands", ".l9_governance-config.json"]
