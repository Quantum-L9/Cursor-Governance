...
class RegistryViewerIntelligence:
    ...
    def query_stale_agents(self, days_threshold: int = 30) -> dict:
        ...
        stale_agents.sort(
            key=lambda x: x.get("days_old", 0) if isinstance(x.get("days_old"), int) else 0,
            reverse=True,
        )
    ...
    def query_rule_coverage(self) -> dict:
        ...
        for rule in self.rule_registry.get("rules", []):
            ...
            coverage_percentage = (
                (usage["usage_count"] / total_agents * 100) if total_agents > 0 else 0
            )
        ...
        return {
            ...
            "validation_status": {
                ...
                "validation_rate": round(
                    (recent_validations / total_agents * 100) if total_agents > 0 else 0, 1
                ),
            },
            "timestamp": datetime.now().isoformat(),
        }
