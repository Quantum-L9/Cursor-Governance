...
| `make pr` is the single publication ceremony; Diagnose is `OPEN_PR=0 make pr`; `precommit-repo` is remediation-local verification only | `AGENTS.md` `MAKE_PR_SINGLE_GATE_SURFACE_V2`; `rules/48-make-pr-remediation.mdc` |
...
| One canonical writer per receipt plane: exactly one module writes a given receipt file, and no other module may import its writer, call it, or construct its path | `CANONICAL_LAW.md` §6.2.9; `ops/autonomy/kernel_gate.py`; [`tests/ops/autonomy/test_kernel_receipt_writers.py`](tests/ops/autonomy/test_kernel_receipt_writers.py) |
