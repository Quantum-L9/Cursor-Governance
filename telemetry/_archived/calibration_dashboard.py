...
class CalibrationDashboard:
    ...
    MCP_MEMORY_URL = os.environ.get("MCP_MEMORY_URL", "https://l9.quantumaipartners.com/mcp/memory")
...
"""
    ...
    def _calculate_metrics(self, decisions: list[dict]) -> CalibrationMetrics:
        ...
        high_risk_predictions = [
            ...
            or d.get("calibrated_probability", 0) > 0.80
        ]
    ...
    def _calculate_ece(self, decisions: list[dict]) -> float:
        ...
        for bin_decisions in bins:
            ...
            confidence = sum(d.get("calibrated_probability", 0.5) for d in bin_decisions) / len(
                bin_decisions
            )
    ...
    def _breakdown_by_model(self, decisions: list[dict]) -> str:
        ...
        for model, model_decisions in by_model.items():
            ...
            avg_confidence = sum(d.get("confidence", 0.5) for d in model_decisions) / len(
                model_decisions
            )
    ...
    def _analyze_thresholds(self, decisions: list[dict]) -> str:
        ...
        thresholds = {"high_risk": 0.85, "medium_risk": 0.65, "low_risk": 0.40}
    ...
    def _generate_recommendations(self, metrics: CalibrationMetrics, decisions: list[dict]) -> str:
        ...
        if metrics.ece > 0.08:
            ...
            recommendations.append(
                "⚠️ **Moderate calibration error** - Monitor closely, may need adjustment"
            )
        else:
        ...
        if metrics.accuracy < 0.85:
            ...
            recommendations.append(
                "ℹ️ **Acceptable accuracy** - Consider fine-tuning for improvement"
            )
        else:
        ...
        if metrics.precision < 0.75:
            recommendations.append(
                "⚠️ **Low precision** - Too many false positives, raise thresholds"
            )
        ...
        if metrics.decisions_evaluated < 50:
            recommendations.append(
                "ℹ️ **Limited data** - Metrics will stabilize with more decisions"
            )
...
"""
    ...
    def detect_calibration_drift(self) -> dict | None:
        """
    ...
    def get_mcp_memory_metrics(self) -> MCPMemoryMetrics | None:
        """
    ...
    def _query_mcp_stats(self) -> dict | None:
    ...
    def _identify_coverage_gaps(self, lessons: list[dict]) -> list[str]:
        ...
        for pattern in list(error_patterns)[:10]:  # Top 10 gaps
            ...
            if not any(
                tag.lower() in pattern_lower or pattern_lower in tag.lower() for tag in lesson_tags
            ):
                gaps.append(pattern)
...
"""
        ...
        gaps_str = (
            "\n".join(f"- {gap}" for gap in metrics.coverage_gaps)
