...
def main() -> int:
    ...
    require(
        "def patch_controller_abort" in apply_text
        and 'sub.add_parser("abort-execution")' in apply_text,
        "Controller-owned abort patch is missing",
    )
    require(
        '"worktree", "add", "--detach"' in apply_text
        and '"git", "apply", "--index", "--binary"' in apply_text
        and "applied tree mismatch" in apply_text,
        "pack application is not staged and tree-hash verified",
    )
    require(
