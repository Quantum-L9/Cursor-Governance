...
class CanonicalProviderResult:
    ...
    def __post_init__(self) -> None:
        ...
        _require_non_empty("provider result status", self.status)
        if self.status not in _PROVIDER_RESULT_STATUSES:
            raise ValueError(f"unsupported canonical provider result status: {self.status}")
        _require_mapping("structured_payload", self.structured_payload)
        _require_mapping("provider_metadata", self.provider_metadata)
        _require_mapping("usage", self.usage)
        _optional_string("raw_output_digest", self.raw_output_digest)
        _optional_string("session_or_run_id", self.session_or_run_id)
        _require_string_tuple(
            "provider result capabilities",
            self.observed_capabilities,
            non_empty=False,
        )
        _dict_tuple("provider result errors", self.errors)
        _dict_tuple("provider transport evidence", self.transport_evidence_refs)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "transport_evidence_refs": [
                dict(item) for item in self.transport_evidence_refs
            ],
        }
...
class CanonicalExecutionRequest:
    ...
    timeout_budget: Mapping[str, Any]
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> CanonicalExecutionRequest:
        ...
        return cls(
            ...
            timeout_budget=dict(
                _require_mapping("timeout_budget", value.get("timeout_budget"))
            ),
...
def validate_provider_invocation(
    ...
    if result.execution_id != request.execution_id:
        raise ValueError(
            "canonical provider result execution_id does not match execution request"
        )
