...
def load_rule_binding(root: Path, path: Path) -> RuleBinding:
    ...
    if loaded.get("schema_ref") != BINDING_SCHEMA:
        raise ValueError(
            f"{path}: schema_ref must equal {BINDING_SCHEMA!r}"
        )
    ...
    if declared not in {None, "", digest, f"sha256:{digest}"}:
        raise ValueError(
            f"{path}: binding digest mismatch: declared {declared!r}, "
    ...
    for left_pattern in left_items:
        ...
        for right_pattern in right_items:
            if right_pattern.startswith("!"):
                continue
            if (
                glob_is_narrower_or_equal(left_pattern, right_pattern)
                or glob_is_narrower_or_equal(right_pattern, left_pattern)
            ):
                return True
    return False
