...
def _conflict_membership(
    ...
    return {
        extraction_id: sorted(cluster_ids)
        for extraction_id, cluster_ids in result.items()
    }
...
def _priority_score(
    ...
    *,
    ...
    strength = str(
        classification.get("doctrine_strength", "unknown")
    )
    sharedness = str(
        classification.get("sharedness", "unknown")
    )
    kind = str(
        classification.get("candidate_kind", "unknown")
    )
    score = (
...
def _candidate_index(
    *,
    ...
    for record in extraction_records:
        extraction_id = str(
            record["extraction_identity"]["extraction_id"]
        )
        ...
        queue.append(
            {
                ...
                "doctrine_strength": (
                    candidate["classification"]["doctrine_strength"]
                ),
                "sharedness": candidate["classification"]["sharedness"],
                "risk": candidate["classification"]["risk"],
                "debt_class": ownership["debt_class"],
                "suggested_disposition": (
                    candidate["migration"]["suggested_disposition"]
                ),
                "duplicate_clusters": candidate["migration"][
                    "duplicate_clusters"
                ],
                "conflict_clusters": candidate["migration"][
                    "conflict_clusters"
                ],
            }
        )
...
def _summary(
    *,
    ...
    return {
        ...
        "unowned_debt_count": sum(
            1 for item in owner_records if item["debt"]
        ),
        "owned_contract_count": sum(
            1
            for item in owner_records
            if item["status"] == "owned_contract"
        ),
        "owned_skill_local_count": sum(
            1
            for item in owner_records
            if item["status"] == "owned_skill_local"
        ),
        "valid_exception_count": sum(
            1
            for item in owner_records
            if item["status"] == "valid_exception"
        ),
        "unresolved_contract_ref_count": sum(
            1
            for item in owner_records
            if item["status"] == "unresolved_contract_ref"
        ),
        "invalid_exception_count": sum(
            1
            for item in owner_records
            if item["status"] == "invalid_exception"
        ),
        "duplicate_cluster_count": clustering["statistics"][
            "duplicate_cluster_count"
        ],
        "potential_conflict_cluster_count": clustering["statistics"][
            "potential_conflict_cluster_count"
        ],
        "potential_conflict_pair_count": clustering["statistics"][
            "potential_conflict_pair_count"
        ],
        "by_kind": _safe_counter(
            [
                str(value["candidate_kind"])
                for value in classification_values
            ]
        ),
        "by_strength": _safe_counter(
            [
                str(value["doctrine_strength"])
                for value in classification_values
            ]
        ),
        "by_sharedness": _safe_counter(
            [
                str(value["sharedness"])
                for value in classification_values
            ]
        ),
        "by_risk": _safe_counter(
            [
                str(value["risk"])
                for value in classification_values
            ]
        ),
        "by_ownership_status": _safe_counter(
            [
                str(value["status"])
                for value in owner_records
            ]
        ),
        "by_debt_class": _safe_counter(
            [
                str(value["debt_class"])
                for value in owner_records
                if value["debt"]
            ]
        ),
...
def build_census(
    *,
    ...
    if extraction_errors:
        raise RuntimeError(
            "doctrine extraction reported integrity errors; census is unsafe"
        )
    ...
    conflict_pair_fingerprints = sorted(
        {
            ...
            for pair in clustering["conflict_pairs"]
        }
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build deterministic Skills Doctrine Census."
    )
    parser.add_argument(
        "--root",
        type=Path,
