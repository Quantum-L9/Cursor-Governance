...
def exact_duplicate_key(record: dict[str, Any]) -> str:
    ...
    source_text = normalize_space(
        str(record.get("source", {}).get("exact_text", ""))
    ).casefold()
    effect = str(record.get("normalized_semantics", {}).get("effect", "unknown"))
...
def _candidate_pairs(
    ...
    pairs = [
        ...
        if count >= MIN_SHARED_CONTENT_TOKENS
    ]
    return sorted(pairs)
    ...
    *,
    ...
    for left, right in _candidate_pairs(ordered):
        ...
        if similarity <= 0:
            continue
        left_effect = str(
            left_record.get("normalized_semantics", {}).get("effect", "unknown")
        )
        right_effect = str(
            right_record.get("normalized_semantics", {}).get("effect", "unknown")
        )
        ...
        duplicate_clusters.append(
            {
                ...
                "members": [
                    _member_summary(ordered[index]) for index in component
                ],
            }
        )
    ...
    try:
        ...
        if args.input is not None:
            ...
            if extraction["statistics"]["error_count"]:
                raise RuntimeError(
                    "extraction produced integrity errors; clustering aborted"
                )
