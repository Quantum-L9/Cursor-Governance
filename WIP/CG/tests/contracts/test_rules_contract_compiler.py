...
from render_cursor_rule import (
    enforce_context_budget,
    render_cursor_rule,
)
from resolve_rule_contracts import (
    canonical_contract_digest,
    load_contract_index,
    resolve_binding,
)
from validate_rule_binding import validate_binding
...
def _contract(
    ...
    *,
    ...
    document["integrity"]["content_digest"] = canonical_contract_digest(
        document
    )
    return document
def _binding(
...
def repo(tmp_path: Path) -> Path:
    ...
    (tmp_path / "contracts" / "domains" / "testing").mkdir(
        parents=True
    )
    (tmp_path / "contracts" / "projections" / "rules").mkdir(
        parents=True
    )
    return tmp_path
...
def test_missing_contract_is_blocking(repo: Path) -> None:
    ...
    assert any(
        finding.code == "RULE-BIND-CONTRACT-MISSING"
...
def test_missing_clause_is_blocking(repo: Path) -> None:
    ...
    findings = validate_binding(binding, index, repo)
    assert any(
        finding.code == "RULE-BIND-CLAUSE-MISSING"
        for finding in findings
    )
def test_retired_contract_is_blocking(repo: Path) -> None:
    ...
    findings = validate_binding(binding, index, repo)
    assert any(
        finding.code == "RULE-BIND-CONTRACT-LIFECYCLE"
        for finding in findings
    )
def test_always_requires_justification(repo: Path) -> None:
    ...
    findings = validate_binding(binding, index, repo)
    assert any(
        finding.code == "RULE-BIND-ALWAYS-JUSTIFICATION"
        for finding in findings
    )
def test_always_rejects_globs(repo: Path) -> None:
    ...
    findings = validate_binding(binding, index, repo)
    assert any(
        finding.code == "RULE-BIND-ALWAYS-GLOBS"
        for finding in findings
    )
def test_auto_attached_requires_globs(repo: Path) -> None:
    ...
    findings = validate_binding(binding, index, repo)
    assert any(
        finding.code == "RULE-BIND-AUTO-GLOBS"
        for finding in findings
    )
def test_advisory_must_is_rejected(repo: Path) -> None:
    ...
    findings = validate_binding(binding, index, repo)
    assert any(
        finding.code == "RULE-BIND-GUIDANCE-NORMATIVE"
        for finding in findings
    )
...
def test_scoped_contract_accepts_narrower_glob(repo: Path) -> None:
    ...
    findings = validate_binding(binding, index, repo)
    assert not any(
        finding.code == "RULE-BIND-SCOPE-WIDEN"
        for finding in findings
    )
...
def test_generated_output_digest_matches_content(repo: Path) -> None:
    ...
    assert (
        projection_index["rules/40-test-contract.mdc"]
