...
def parse_marker_fields(body: str) -> dict[str, str]:
    ...
    for part in _split_semicolon_fields(body):
        if "=" not in part:
            raise ValueError(
                f"annotation field must use key=value syntax: {part!r}"
            )
    ...
    if len(owners) != 1:
        raise ValueError(
            ...
            "contract, local_contract, or exception"
        )
    scope = fields.get("scope", "next")
    if scope != "next":
        raise ValueError(
            "only scope=next is supported; broad doctrine suppressions are forbidden"
        )
    fields["scope"] = scope
    if "exception" in fields:
        ...
        raise ValueError(
            "reason is valid only for an explicit normative exception"
        )
    for key in ("contract", "local_contract"):
        if key in fields and not CONTRACT_ID_RE.fullmatch(fields[key]):
            raise ValueError(
                f"{key} does not satisfy canonical contract ID grammar: "
                f"{fields[key]!r}"
            )
    return fields
def parse_markers(
            ...
            if match is None:
                findings.append(
                    Finding(
                        ...
                        line=line_number,
                        message=(
                            "l9-doctrine annotation must occupy a standalone "
                            "HTML-comment line"
                        ),
                    )
                )
...
def bind_markers(
    *,
    ...
    for path_records in records_by_path.values():
        path_records.sort(
            key=lambda record: int(record.get("source", {}).get("line_start", 0))
        )
        ...
        if target is None:
            findings.append(
                Finding(
                    ...
                    line=marker.line,
                    message=(
                        "annotation does not bind to the immediately following "
                        "doctrine candidate"
                    ),
                )
            )
            continue
        extraction_id = str(
            target["extraction_identity"]["extraction_id"]
        )
        if extraction_id in bindings:
            ...
            findings.append(
                Finding(
                    ...
                    line=marker.line,
                    message=(
                        f"candidate {extraction_id} already has annotation "
                        f"at line {previous.line}"
                    ),
                )
            )
            continue
    ...
    *,
    ...
    for index, raw_entry in enumerate(entries):
        ...
        if not isinstance(contract_id, str):
            raise ValueError(
                f"contract registry entry {index} has no contract_id"
            )
        if not CONTRACT_ID_RE.fullmatch(contract_id):
            raise ValueError(
                f"contract registry contains invalid contract_id {contract_id!r}"
            )
        if contract_id in contracts:
            raise ValueError(
                f"contract registry contains duplicate active identity {contract_id}"
            )
    ...
    *,
    ...
    if entry is None:
        if registry.path is None:
            return False, (
                f"{contract_id} cannot resolve because no contract registry exists"
            )
        return False, f"{contract_id} is absent from {registry.path}"
    status = entry.get("status")
    if status != "active":
        return False, (
            f"{contract_id} has status {status!r}; only active contracts own "
            "active doctrine"
        )
...
def analyze_ownership(
    *,
    ...
    if "exception" in marker.fields:
        ...
        if (
            classification.get("risk") == "critical"
            and exception not in CRITICAL_EXCEPTION_CLASSES
        ):
            return {
                **base,
    ...
    return {
        ...
        "statistics": {
            ...
            "unowned_debt_count": sum(
                1 for analysis in analyses if analysis["debt"]
            ),
