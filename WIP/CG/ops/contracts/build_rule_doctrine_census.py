...
def build_rule_census(
    ...
    result = {
        ...
        "summary": {
            ...
            "potential_conflict_cluster_count": len(
                clustering["conflict_clusters"]
            ),
            "always_rule_candidate_count": sum(
                1
                for item in candidate_index
                if item["rule"]["always_apply"]
            ),
        },
...
def bootstrap_baseline(
    ...
    baseline = {
        ...
        "allowances": {
            ...
            "potential_conflict_pairs": census[
                "potential_conflict_pair_fingerprints"
            ],
        },
    }
...
def check_baseline(
    ...
    current = {
        item["fingerprint"]: int(item["count"])
        for item in census["debt_allowance_candidate"]
    }
    failures: list[str] = []
    ...
    current_conflicts = set(
        census["potential_conflict_pair_fingerprints"]
    )
...
def tighten_baseline(
    ...
    if failures:
        raise ValueError(
            "cannot tighten a failing baseline: " + "; ".join(failures)
        )
    current = {
        item["fingerprint"]: int(item["count"])
        for item in census["debt_allowance_candidate"]
