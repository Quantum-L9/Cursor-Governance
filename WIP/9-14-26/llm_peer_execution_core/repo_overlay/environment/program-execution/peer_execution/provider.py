...
class ProviderInvocation:
    ...
    def __post_init__(self) -> None:
        ...
        if self.result is not None and not isinstance(
            self.result, CanonicalProviderResult
        ):
            raise ValueError(
                "provider invocation result must be CanonicalProviderResult"
            )
        ...
        if self.status == "PASS" and self.result is None:
            raise ValueError(
                "PASS provider invocation requires a canonical provider result"
            )
        if self.status in {"RUNNING", "UNSUPPORTED"} and self.result is not None:
            raise ValueError(
                f"{self.status} provider invocation must not include a result"
            )
...
class CanonicalExecutionRequest(BaseModel):
    ...
    def _validate_request(self) -> CanonicalExecutionRequest:
        ...
        object.__setattr__(
            ...
            "program_lock_digest",
            normalize_digest(
                _require_non_empty("program_lock_digest", self.program_lock_digest)
            ),
        )
        object.__setattr__(
            self,
        ...
        if permission.get("profile_ref") != self.permission_profile_ref:
            raise ValueError(
                "permission profile reference does not match resolved profile"
            )
        ...
        if not isinstance(allowed, list) or not set(requested) <= set(allowed):
            raise ValueError(
                "resolved permission profile does not allow requested capabilities"
            )
