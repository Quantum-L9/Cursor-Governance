...
class PeerExecutionAdapter(BaseExecutionAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        if bound_provider is not None and bound_provider != self.adapter_id:
            raise ValueError(
                f"binding provider_ref mismatch: expected {self.adapter_id}, "
                f"found {bound_provider}"
            )
        ...
        self.capabilities = tuple(
            _required_string("adapter capability", item) for item in actions
        )
    ...
    def prepare(self, contract: Mapping[str, Any]):
        ...
        denied = sorted(
            set(binding.requested_actions)
            - set(permission_profile.get("allowed_actions") or [])
        )
        if denied:
            raise AdapterFailure(
    ...
    def _request_from_record(
        ...
        if request_errors:
            raise ValueError(
                f"canonical execution request schema errors: {request_errors}"
            )
        ...
        if dict(request.timeout_budget) != dict(
            self.execution_profile.get("timeout_budget") or {}
        ):
    ...
    def _provider_evidence(result: CanonicalProviderResult) -> dict[str, Any]:
        return {
            ...
            "transport_evidence_refs": [
                dict(item) for item in result.transport_evidence_refs
            ],
        }
    ...
    def _verification_receipt(
        ...
        for key, value in gates.items():
            normalized_gates[
                _required_string("provider payload gate id", key)
            ] = _required_string("provider payload gate status", value)
    ...
    def _apply_invocation(
        ...
        if invocation.result is not None:
            ...
            if result_errors:
                raise ValueError(
                    f"canonical provider result schema errors: {result_errors}"
                )
