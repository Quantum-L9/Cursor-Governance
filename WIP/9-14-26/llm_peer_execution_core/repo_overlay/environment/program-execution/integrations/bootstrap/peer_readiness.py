...
def _registry_entry(subsystem_root: Path, provider_ref: str) -> dict[str, Any] | None:
    ...
    return next(
        (
            ...
            if item.get("adapter_id") == provider_ref
        ),
        None,
    )
...
def _autonomy_facts(subsystem_root: Path, repo_root: Path) -> dict[str, Any]:
    ...
    provider_ok = provider.get("owns_program_state") is False and bool(
...
def build_readiness(
    ...
    *,
    ...
    agents = _load_yaml(repo_root / "environment/agents/agent_registry.yaml").get(
        "agents"
    ) or {}
