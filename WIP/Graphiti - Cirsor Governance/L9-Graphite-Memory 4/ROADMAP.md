...
| Secrets management | Infisical Universal Auth → `os.environ` hydration |
...
- [ ] New `src/l9_graphite_memory/secrets.py` — Python Infisical adapter using `infisical-python` SDK (v2.3.6)
...
- Fail-soft: if Infisical is unreachable and `INFISICAL_REQUIRED` is not set, fall back to `os.environ`
- Rotation-aware: `signal.SIGHUP` handler (sync) or `loop.add_signal_handler(signal.SIGHUP, ...)` for asyncio servers
- Interval refresh: `start_refresh_interval(seconds)` returns a cancellable `asyncio.Task`

**Python Adapter Contract (`secrets.py`):**
```python
...
async def refresh_secrets(
    ...
    """Re-fetch with overwrite=True. Returns refreshed_at ISO timestamp."""
    ...

def install_sighup_reload(
    logger: logging.Logger | None = None,
) -> Callable[[], None]:
    """Register SIGHUP handler. Returns uninstall function."""
    ...
...
def start_refresh_interval(
    ...
    """Belt-and-suspenders interval refresh. Returns cancellable task."""
    ...
...
**Contract:**
- CLI interface remains stable: `search`, `write`, `health`, `bootstrap`, `phase-lock`, `conflicts`
