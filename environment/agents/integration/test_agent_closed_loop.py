...
class ClosedLoopTests(unittest.TestCase):
    ...
    def test_happy_path_chain(self):
        start = compose_start.compose_subagent_start(
            {
                "assignment": {
                    "assignment_id": "loop-1",
                    "campaign_id": "c1",
                    "action_id": "a1",
                    "subagent_role": "l9-recon",
        ...
        ret = compose_stop.compose_subagent_stop(
            {"assignment_id": "loop-1", "output": {"ok": True}}
        )
        self.assertEqual(ret["status"], "RETURNED")
        acc = gateway.accept(
            return_receipt=ret["return_receipt"],
            surface_result={
        ...
        ing = ingest.ingest_accepted_result(
            accepted_result={},
            generated_data_packet={"units": [{"class": "repository_fact"}]},
            acceptance_receipt=acc,
        )
        self.assertEqual(ing["outcome"], "CAPTURED")
        self.assertTrue(
            readiness.completion_evidence_ok(
                return_receipt=ret["return_receipt"], acceptance_receipt=acc, ingress_receipt=ing
            )["allowed"]
        )
    ...
    def test_failure_matrix_terminals(self):
        ...
        ing = ingest.ingest_accepted_result(
            ...
            acceptance_receipt={"status": "ACCEPTED", "receipt_digest": "acc-sec"},
        )
