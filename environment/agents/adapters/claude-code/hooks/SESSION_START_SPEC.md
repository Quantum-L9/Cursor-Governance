...
6. **L9 Claude environment status block** projected from the
   `~/.l9/claude/bootstrap-state.json` this ceremony generated (schema
   `l9.claude-bootstrap.v1`, written by `install.sh`, read back bound to the
   ceremony id — see below): surface, execution (anthropic-cloud / local), governance
...
## Bootstrap receipt: generated every ceremony, read back bound

Every SessionStart ceremony **generates** its bootstrap receipt: it runs
`install.sh` with `L9_BOOTSTRAP_ID=<ceremony id>`, and the installer stamps that
id into `~/.l9/claude/bootstrap-state.json` (`bootstrap_id`). The environment
block then reads **that very receipt** with
`claude_bootstrap_receipt.py --bootstrap-id <ceremony id>`; a receipt carrying any
other id — or none — reads as `unknown` ("not generated by this bootstrap"),
never as the verdict of this session.

The installer runs in the projection slot and **is** the ceremony's projection
(it runs the same engine); the standalone `claude_projection.py` step runs only
when no installer is present. Generation is clamped to the remaining budget
minus `L9_BOOTSTRAP_REPORT_FLOOR` (6 s) kept for the must-emit lines, and is
...
receipt its installer wrote. Readiness reuse (`--reuse-fresh`) is likewise bound
to the bootstrap receipt it was built from.
