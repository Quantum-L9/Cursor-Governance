...
from environment.agents.lifecycle import receipts as life_receipts
...
def accept(
    *,
    ...
    try:
        ...
        rid = hashlib.sha256(json.dumps(surface_result, sort_keys=True, default=str).encode()).hexdigest()[:16]
        return result_receipts.write_acceptance({
    ...
    elif (return_receipt or {}).get("base_sha") and normalized.get("base_sha") and normalized.get("base_sha") != (return_receipt or {}).get("base_sha"):
        status, reason = "REJECTED", "wrong base_sha"
    else:
        status, reason = "ACCEPTED", "ok"

    result_digest = hashlib.sha256(json.dumps(normalized, sort_keys=True, default=str).encode()).hexdigest()
