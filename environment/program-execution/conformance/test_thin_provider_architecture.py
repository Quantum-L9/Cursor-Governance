...
class ThinProviderArchitectureTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.registry = yaml.safe_load(
            (cls.root / "registry/EXECUTION_ADAPTER_REGISTRY.yaml").read_text(encoding="utf-8")
        )
        cls.adapter_schema = json.loads(
            (cls.root / "conformance/schemas/execution-adapter-spec.schema.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_bounded_execution_is_not_a_provider(self) -> None:
        ...
        self.assertFalse((self.root / "adapters/claude-code-bounded-autonomy").exists())
    ...
    def test_topology_uses_provider_and_profile_refs(self) -> None:
        ...
        schema = json.loads(
            (
                self.repo_root / "environment/agents/schemas/peer-runtime-bindings.schema.json"
