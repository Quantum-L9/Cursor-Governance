...
Define workstreams, then Task Cards (set `autonomy_action_id` when bridging to Cursor/Claude autonomy), then encode all task-to-task dependency edges in `DEPENDENCY_GRAPH.yaml`, then assign tasks to waves.

## 6. Define proof before mutation

Set acceptance, validation (include `make pr`), negative cases, rollback, risk, authorization ceiling, and completion gates before importing the task into a Controller.
