...
_pec_helpers = _load(
    "pec_execution_completeness_controller_helpers",
    PEC_SCRIPTS / "tests" / "helpers.py",
)


class ScopedRelockTest(unittest.TestCase):
    """PEC-P0-002 as seen from the runner: a scoped relock is semantic, not digest-wide.

    These three cases previously pinned the pre-remediation contract, under
    which a relock "absorbed" every program-wide digest that had moved. That
    contract is exactly the laundering finding P0-002 names: a compile that
    changed a gate, an authority owner, or a dependency edge was recorded as
    current under a task-scoped relock. The Controller now derives the semantic
    delta between the lock and the Blueprint and admits the relock only when
    every difference is a definition inside the requested scope. The fixtures
    here are real Blueprints, not hand-built locks, so the lock passes its own
    integrity check and each test observes the contract it asserts.
    """

    def setUp(self) -> None:
        self._tmp = tempfile.TemporaryDirectory()
        tmp = Path(self._tmp.name)
        self.blueprint = _pec_helpers.make_blueprint(tmp / "blueprint", two_tasks=True)
        self.lock_path = tmp / "program-lock.json"
        pec_blueprint.write_program_lock(self.blueprint, self.lock_path)
        self.lock = json.loads(self.lock_path.read_text(encoding="utf-8"))

    def tearDown(self) -> None:
        self._tmp.cleanup()

    def _edit(self, name: str, mutate) -> None:
        path = self.blueprint / name
        doc = yaml.safe_load(path.read_text(encoding="utf-8"))
        mutate(doc)
        _pec_helpers.write_yaml(path, doc)

    def _edit_task(self, task_id: str, **fields) -> None:
        def mutate(doc: dict) -> None:
            for task in doc["tasks"]:
                if task["id"] == task_id:
                    task.update(fields)

        self._edit("TASK_CARDS.yaml", mutate)

    def _assert_refused(self, task_ids: list[str], code: str) -> pec_blueprint.BlueprintError:
        before = self.lock_path.read_bytes()
        with self.assertRaises(pec_blueprint.BlueprintError) as ctx:
            pec_blueprint.relock_tasks(self.lock_path, task_ids)
        self.assertEqual(ctx.exception.error_code, code, str(ctx.exception))
        self.assertEqual(self.lock_path.read_bytes(), before, "a refused relock writes nothing")
        return ctx.exception

    def test_relock_refuses_non_task_program_drift(self) -> None:
        """A gate edit beside the named task is wider than the scope: refused.

        The superseded version of this test asserted the opposite ("absorbs
        non-task program drift"). Program-wide sources may be regenerated by a
        compile, but a regenerated artifact whose *semantics* moved is program
        drift, and the digest-current lock it would have produced is the
        laundering P0-002 forbids. Compile stamps alone are masked and do not
        trip this; see the Controller's own semantic-delta suite.
        """
        self._edit_task("TASK-001", objective="new one")
        self._edit(
            "CONVERGENCE_GATES.yaml",
            lambda doc: doc["gates"][0].__setitem__("pass_condition", "weaker"),
        )
        exc = self._assert_refused(["TASK-001"], "PROGRAM_LOCK_GLOBAL_DRIFT")
        self.assertIn("gates", str(exc))

    def test_relock_refuses_non_selected_task_drift(self) -> None:
        self._edit_task("TASK-001", objective="new one")
        self._edit_task("TASK-002", objective="drifted")
        exc = self._assert_refused(["TASK-001"], "RELOCK_SCOPE_INSUFFICIENT")
        self.assertIn("TASK-002", str(exc))

    def test_relock_adopts_named_definition_and_only_the_digests_that_moved(self) -> None:
        """The named task is updated; the unnamed task and every other digest are not.

        The discriminating fixture principle from the superseded version is
        kept: the recorded and current TASK_CARDS digests differ, so the
        assertion cannot pass by coincidence, and the remaining digests are
        asserted equal to the previous lock so an implementation that refreshed
        the whole set would be caught.
        """
        self._edit_task("TASK-001", objective="new one")
        outcome = pec_blueprint.relock_tasks(self.lock_path, ["TASK-001"])
        self.assertEqual(outcome["status"], "RELOCKED")
        self.assertEqual(outcome["relocked"], ["TASK-001"])
        self.assertEqual(outcome["delta"]["source_digests_moved"], ["TASK_CARDS.yaml"])
        relocked = json.loads(self.lock_path.read_text(encoding="utf-8"))
        self.assertEqual(relocked["tasks"][0]["source"]["objective"], "new one")
        self.assertEqual(relocked["tasks"][1], self.lock["tasks"][1])
        self.assertNotEqual(
            relocked["source_digests"]["TASK_CARDS.yaml"],
            self.lock["source_digests"]["TASK_CARDS.yaml"],
        )
        untouched = {k: v for k, v in self.lock["source_digests"].items() if k != "TASK_CARDS.yaml"}
        self.assertEqual(
            {k: v for k, v in relocked["source_digests"].items() if k != "TASK_CARDS.yaml"},
            untouched,
        )
        self.assertTrue(pec_blueprint.verify_program_lock(self.lock_path)[0])
