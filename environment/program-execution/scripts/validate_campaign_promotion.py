...
MANIFEST digests are deliberately **not** checked here. This validator gated
them once; that coupling made every ordinary PE edit fail promotion, and it was
removed. ``environment/program-execution/MANIFEST.json`` is a generated
artifact -- regenerated by ``sync_generated_artifacts.py --pe-manifest`` on the
PR gate, drift-checked in ``governance-self-check.yml``, and validated by
``validate_manifest.py`` under ``make program-execution-conformance``.
