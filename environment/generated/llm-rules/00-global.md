...
When writing timestamps in files:
- **Format**: `YYYY-MM-DD HH:MM EST` (e.g., `2026-01-04 19:34 EST`)
- **Date source**: The `Current Date` in system `user_info` is ALWAYS the correct date. Use it exactly.
- **NEVER assume tomorrow**: If system says "Jan 4", use Jan 4. Do not increment to Jan 5.
- **Time handling**: If user provides time, use it. If not provided and timestamp is needed, ASK the user for the current time.
- **Timezone**: Default to EST unless user specifies otherwise.

---

## Auto-Wiring Rules (LAW)

### New Singletons — MUST Use `@register_singleton`

**Effective: 2026-01-19**

All **new** singleton services MUST use the decorator-based auto-registration pattern:
...
@register_singleton(
    category="memory",  # or: core, config, clients, observability, research, agents
    lifecycle=SingletonLifecycle.LAZY,  # or: STARTUP, MANUAL
    dependencies=["other_singleton"],  # if depends on other singletons
...
@register_singleton_closer("my_new_service")
async def close_my_new_service():
    # cleanup
    pass
```

❌ **DO NOT** add new try/except blocks to `_register_core_singletons()` in `singleton_registry.py`
✅ **DO** let singletons self-register from their own modules

### New Event Types — MUST Use `register_event_type()`

All **new** event types MUST use the dynamic event registry:
...
register_event_type(
    name="my_new_event",
    category="coordination",  # or: security, observability, custom
    description="Human-readable description",
    schema={"type": "object", ...}  # optional JSON schema
)
```

❌ **DO NOT** add new values to `EventKind` or `SecurityEventType` enums
✅ **DO** use `register_event_type()` for extensibility

### Existing Code

Existing singletons and event types (pre-2026-01-19) may remain unchanged. Migration is optional but encouraged for reducing boilerplate.

### Reference

- `core/auto_registry.py` — Generic AutoRegistry[T] base class
- `core/singleton_auto_registry.py` — Singleton decorators and wiring
- `core/event_type_registry.py` — Dynamic event type registration
- `docs/AUTO_WIRING_PHASE2_GUIDE.md` — Full migration guide
