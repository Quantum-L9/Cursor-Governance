...
Governance scripts source `resolve_governance_paths.sh` and use the exported `$GOV_ROOT` and `$GLOBAL_COMMANDS` values. Hooks installed as real files must resolve the clone explicitly rather than deriving the root from a cloud path.
...
<!-- generated-from: rules/97-governance-ssot-paths.mdc; do-not-edit -->
