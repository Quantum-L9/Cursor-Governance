...
| Episodic | Graphiti / L9 shared-memory (**one store**; CLI + MCP transports — ADR-0005) | What was decided? Which ADR or operating lesson applies? |
...
## Agent memory vs product memory (ADR-0005)

- **Agent episodic memory** — Graphiti / L9 shared-memory only. CLI and MCP are access paths, not competing products.
...
- Do not invent a second agent-memory stack when one transport (CLI or MCP) is unhealthy — fix wiring.
