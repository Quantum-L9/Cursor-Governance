...
Failure policy: fail-open on missing telemetry (gh absent, api failure —
never brick `make pr` on network loss); fail-open on precision, fail-closed
on the decision (no probe ⇒ filename overlap still blocks); fail-closed on a
