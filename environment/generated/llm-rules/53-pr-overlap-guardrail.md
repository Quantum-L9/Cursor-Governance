...
with routing instructions. A raw `git push` of a branch with no open PR, or
`gh pr create`, is denied as a first publication (CANONICAL_LAW §6.2.8); a push
advancing an open PR is not blocked (§6.2.4). Prefer `make pr` because it runs
this gate.
...
| `PR_STACK=` (`make pr` default since 2026-09-19) | Do not stack: keep `PR_BASE` (usually `origin/main`); overlap still blocks |
| `PR_STACK=auto` | Opt in. Instead of blocking, re-resolves the PR base to the overlapping open PR's head branch (never main) when the blocking set is one unambiguous chain; ambiguous sibling chains still block |
...
`PR_STACK=auto` binds the unique open-PR chain tip at **pr-preflight /
run_pr_gate.sh start** (`ops/scripts/lib/resolve_pr_stack.sh`), not only after
the gate.
...
## On overlap: required order

**Wait for merge is forbidden.** It is not a fourth option.

1. **Commit into the overlapping open PR branch this turn** — same-agent
   or the PR the user already named as owning the path. "Do not put X in
   *this* PR" means put X in the PR that already has X.
2. **Stack** — opt in with `PR_STACK=auto make pr` when the overlapping
   path genuinely belongs to the open PR's chain. It is no longer the
   `make pr` default: auto resolves a base only for one unambiguous
   chain, so with several sibling chains open it fails preflight and the
   caller passes `PR_STACK=` anyway. Publishing against `PR_BASE` is the
   default; this option stays for a deliberate train.
   Policy: rebase and conflict resolution forbidden; bottom-up merge
   order. A stack parent must use `--merge` or children must land first.
3. **`PR_OVERLAP=ignore`** — only with a stated justification. Not a wait.

## MUST NOT

- Finish with wait / defer-until-#N-merges / "after those PRs land"
- Drop the overlapping path from this branch so a sibling can publish,
  then park the dropped work as a future follow-up
- Treat `session_debt.py defer` as converting a live overlap into an
  acceptable later task
