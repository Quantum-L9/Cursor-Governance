---
description: Before opening any PR, the pre-open overlap gate must pass; overlapping work routes into the existing open PR or stacks on its head, never a sibling PR that conflicts.
---
...
The gate detects whether the current branch textually conflicts with any
already-open PR in the same repo, and blocks BEFORE the push with routing
instructions. Raw `git push` / `gh pr create` bypass this check entirely and are
NOT denied (CANONICAL_LAW §6.2.4) — the choke point covers the sanctioned path,
not the whole publish surface, so overlap detection depends on using `make pr`.
...
| `PR_STACK=auto` (`make pr` default) | Start-time default: `agent_worktree_start.sh` bases the worktree on the unique open-PR chain tip (never invent a main fork and restack at `make pr`). At publish, the overlap gate also re-resolves `PR_BASE` to that tip when the blocking set is one unambiguous chain; ambiguous sibling chains still block. |
...
2. **Stack** — start on the tip (`PR_STACK=auto` via
   `agent_worktree_start.sh`, or `PR_BASE=origin/<open-pr-head>`). Default
   for `make pr`. Opt out with `PR_STACK= make pr` to publish against
   `main`. Policy: rebase and conflict resolution forbidden; bottom-up
   merge order. Do not invent a main-based sibling and restack at `make pr`.
