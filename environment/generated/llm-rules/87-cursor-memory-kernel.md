---
description: Cursor Memory Kernel enforcement — authoritative source for memory operations, kinds, and session lifecycle
---
...
2. **Endpoint:** the canonical memory control plane via the locked governance venv — `python -m ops.memory.cli` (stdio to the bound `l9-graphite-memory` runtime). No tunnel, no URL, no bearer on this surface; C1 legacy `:9002` / `/memory` **retired**.
...
2. **Health check** if hydration degraded: `memcli health`
3. **Hydrate** if `next=` empty: `memcli hydrate "current task"`
...
2. **Dedupe-check:** `memcli search "lesson topic"`
...
memcli write \
  "SESSION: 2026-02-16. WORK: Fixed 2,584 ruff errors. LESSONS: (1) ..." \
  --kind note
```
...
memcli write \
  "_require_X() guard pattern fixes nullable service attrs." \
  --kind lesson
```
...
2. **Write each as a separate memory** — one `write` per fact
...
- [ ] Resume path uses sessionStart canonical hydrate (continuation capsule), not `memory-bank/`
...
| `ops/memory/cli.py` | Canonical memory CLI (`python -m ops.memory.cli` health, resolve, search, write, hydrate, conflicts, readiness) |
| `ops/memory/README.md` | Memory boundary, binding (INV-11) and egress firewall (INV-03) |
