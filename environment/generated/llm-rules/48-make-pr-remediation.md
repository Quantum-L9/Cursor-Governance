---
description: After make pr opens a PR, remediate until green + mergeable, then halt before merge (stacked bottom-up PRs, no rebase)
---

# make pr → checkers + PR open → remediate-until-green → HALT before merge
...
4. After the PR is open: remediate with `l9-pr-remediation` until CI is green
   and the PR is mergeable (bounded: `surface_profile.remediation.max_cycles`),
   resolving review threads — then **HALT before merge**.
5. **Not** merge. Merge is human-only: `L9_MERGE_AUTHORIZED=<reason>` or a
   one-shot entry in `~/.l9/autonomy/merge-authorization.json`
   (`ops/autonomy/merge_gate.py` enforces both channels).
...
human override only. The standing agent contract is remediate-until-green,
then halt; merge stays human.
