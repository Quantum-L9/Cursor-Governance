...
Do **not** treat `make pr-check && make pr` as two full gates. `pr-check`
writes `.l9/pr/gate-receipt.json`; `make pr` reuses it when HEAD, the
worktree porcelain digest, and `PR_BASE` are unchanged.

Happy path:

```bash
make improve
# apply kernels; commit revisions
