...
def infisical_req(
    ...
    for attempt in range(retries):
        req = urllib.request.Request(
            f"{host}{path}", data=data, method=method, headers=headers
        )
        try:
...
def ensure_folder(
    ...
    if status in {400, 409} and any(
        x in err.lower() for x in ("already", "exist", "duplicate")
    ):
        return
...
def main(argv: list[str] | None = None) -> int:
            ...
            root_items.append(
                {
                    ...
                    "secretValue": raw,
                    "secretComment": (
                        "ported from openclaw-igorbot/google-service-account"
                    ),
    ...
    for suffix, items in structured.items():
        ...
        c, u = upsert_batch(
            host, token, args.project_id, args.environment, folder_path, items
        )
        created_total += c
        updated_total += u
