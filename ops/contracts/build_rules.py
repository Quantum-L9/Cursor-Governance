...
def resolve_all(
    ...
    if findings:
        raise ValueError(
            "invalid rule bindings: " + "; ".join(str(finding) for finding in findings)
        )
...
def build_projection_index(
    ...
    if conflicts:
        raise ValueError("cross-rule semantic conflicts: " + "; ".join(conflicts))
    ...
    for resolution in resolutions:
        ...
        projection_index[resolution["output_path"]] = {
            ...
            "projection": {
                ...
                "contract_bundle_digest": (f"sha256:{resolution['bundle_digest']}"),
                "drift": False,
            },
            "context": {
                **metrics,
...
def expected_outputs(
    ...
    has_bound = any(
        entry.get("migration_state") in {"generated", "contract_bound"}
        for entry in projection_index.values()
    )
    if has_bound:
        outputs.update(_manifest_outputs(root, projection_index))
...
def explain(
    ...
    for resolution in resolutions:
        ...
        if selector in {
            ...
            return {key: value for key, value in resolution.items() if key != "_binding"}
...
def impact(
    ...
    for resolution in resolve_all(root, registry_path):
        matching = [
            item for item in resolution["resolved_contracts"] if item["contract_id"] == contract_id
        ]
        if matching:
            binding = resolution["_binding"]
            results.append(
                {
                    ...
                    "clauses": sorted({clause for item in matching for clause in item["clauses"]}),
                }
            )
    return results
