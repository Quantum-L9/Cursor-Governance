...
CONTRACT_ID_RE = re.compile(r"^[A-Z][A-Z0-9]*(?:\.[A-Z][A-Z0-9_]*)+\.\d{3}$")
...
def version_matches(version: str, constraint: str) -> bool:
    ...
    if constraint.startswith("^"):
        ...
        if len(expected) >= 2 and parsed[1] < expected[1]:
            return False
        if len(expected) == 3 and parsed[1] == expected[1] and parsed[2] < expected[2]:
            return False
        return True
    raise ValueError(
...
def _contract_from_document(
    ...
    for index, clause in enumerate(raw_clauses):
        if not isinstance(clause, dict):
            raise ValueError(f"{path}: normative_clauses[{index}] must be a mapping")
        clause_id = str(clause.get("clause_id") or "")
        if not clause_id:
            raise ValueError(f"{path}: normative_clauses[{index}] missing clause_id")
...
def _load_registry(root: Path, path: Path) -> ContractIndex:
    ...
    for index, entry in enumerate(raw_entries):
        ...
        if not relative_path:
            raise ValueError(f"{path}: registry entry {contract_id!r} missing path")
        contract_path = root / relative_path
        if not contract_path.is_file():
            raise ValueError(f"{path}: registry path does not exist: {relative_path}")
        ...
        if registry_version != record.version:
            raise ValueError(f"{path}: registry version mismatch for {contract_id}")
        if registry_status != record.status:
            raise ValueError(f"{path}: registry status mismatch for {contract_id}")
        registry_digest = str(entry.get("digest") or "")
        if registry_digest and registry_digest not in {
            record.digest,
            f"sha256:{record.digest}",
        }:
            raise ValueError(f"{path}: registry digest mismatch for {contract_id}")
...
def resolve_binding(
    ...
    for ref in refs:
        ...
        if record is None:
            raise ValueError(f"{binding.relative_path}: unresolved contract {contract_id!r}")
        ...
        if not isinstance(clause_ids, list) or not clause_ids:
            raise ValueError(f"{binding.relative_path}: {contract_id} requires explicit clauses")
        selected: list[str] = []
        for raw_clause_id in clause_ids:
            ...
            if clause is None:
                raise ValueError(
                    f"{binding.relative_path}: {contract_id} has no clause {clause_id!r}"
                )
    ...
    if conflicts:
        rendered = "; ".join(f"{item['left']} <> {item['right']}" for item in conflicts)
        raise ValueError(f"{binding.relative_path}: conflicting projected clauses: {rendered}")
    bundle_payload = {
...
def directives_conflict(
    ...
    if left.get("contract_id") == right.get("contract_id") and left.get("clause_id") == right.get(
        "clause_id"
    ):
        return False
...
def detect_directive_conflicts(
    ...
    for index, left in enumerate(directives):
        for right in directives[index + 1 :]:
            ...
            conflicts.append(
                {
                    "left": (f"{left['contract_id']}:{left['clause_id']}"),
                    "right": (f"{right['contract_id']}:{right['clause_id']}"),
                }
            )
    return conflicts
