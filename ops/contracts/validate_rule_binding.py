...
def _validate_scope_narrowing(
    ...
    for ref in binding_contract_refs(binding):
        ...
        if isinstance(scope_narrowing, dict):
            narrowed_paths = [str(item) for item in _as_list(scope_narrowing.get("paths"))]
        projected_paths = narrowed_paths or [str(item) for item in activation_globs]
        if not owner_paths:
            continue
        for candidate in projected_paths:
            if candidate.startswith("!"):
                continue
            if any(glob_is_narrower_or_equal(candidate, owner) for owner in owner_paths):
                continue
            findings.append(
                Finding(
...
def validate_binding(
    ...
    if status not in {"draft", "review", "active", "deprecated", "retired"}:
        findings.append(Finding("RULE-BIND-STATUS", path, f"invalid status {status!r}"))
    if not isinstance(target, dict):
        findings.append(Finding("RULE-BIND-TARGET", path, "missing target mapping"))
    else:
        if target.get("platform") != "cursor":
            findings.append(Finding("RULE-BIND-PLATFORM", path, "target.platform must be cursor"))
        if target.get("format") != "mdc":
            findings.append(Finding("RULE-BIND-FORMAT", path, "target.format must be mdc"))
    ...
    if not RULE_ID_RE.fullmatch(rule_id):
        findings.append(Finding("RULE-BIND-RULE-ID", path, f"invalid rule_id {rule_id!r}"))
    mode = str(activation.get("mode") or "")
    if mode not in ACTIVATION_MODES:
        findings.append(Finding("RULE-BIND-ACTIVATION", path, f"invalid activation mode {mode!r}"))
    ...
    if mode in {"agent_requested", "manual"}:
        description = str(
            activation.get("description") or rule_identity.get("description") or ""
        ).strip()
        if not description:
            findings.append(
                Finding(
    ...
    for ref in refs:
        ...
        try:
            ...
            findings.append(Finding("RULE-BIND-VERSION-CONSTRAINT", path, str(exc)))
            matches = False
        if not matches:
            findings.append(
                Finding(
