...
DEFAULT_BASELINE = Path("ops") / "config" / "doctrine-baseline.yaml"
...
def _current_debt_counts(census: dict[str, Any]) -> Counter[str]:
    return Counter(
        {str(item["fingerprint"]): int(item["count"]) for item in census["debt_inventory"]}
    )


def _current_conflict_pairs(census: dict[str, Any]) -> set[str]:
    return {str(value) for value in census["potential_conflict_pair_fingerprints"]}
...
def _representatives_by_fingerprint(
    ...
    return {
        str(item["fingerprint"]): dict(item["representative"]) for item in census["debt_inventory"]
    }
...
def _baseline_payload_without_digest(
    ...
    return {key: value for key, value in baseline.items() if key != "integrity_digest"}
...
def build_baseline(
    *,
    ...
    reason = reason.strip()
    if len(reason) < 20:
        raise ValueError("bootstrap reason must contain at least 20 meaningful characters")
    ...
    baseline = {
        ...
        "allowances": {
            ...
            "potential_conflict_pairs": sorted(_current_conflict_pairs(census)),
        },
    }
...
def validate_baseline_structure(
    ...
    if unknown:
        errors.append(f"baseline contains unknown top-level fields: {sorted(unknown)}")
    missing = REQUIRED_BASELINE_TOP_LEVEL - set(baseline)
    if missing:
        errors.append(f"baseline is missing required top-level fields: {sorted(missing)}")
    if baseline.get("schema_id") != BASELINE_SCHEMA_ID:
        errors.append(f"schema_id must equal {BASELINE_SCHEMA_ID!r}")
    ...
    if not isinstance(policy, dict):
        ...
        for key, expected in required_policy.items():
            if policy.get(key) is not expected:
                errors.append(f"policy.{key} must remain {expected!r}")
        ...
        if not isinstance(debt, list):
            ...
            for index, item in enumerate(debt):
                if not isinstance(item, dict):
                    errors.append(f"allowances.unowned_debt[{index}] must be a mapping")
                    continue
                fingerprint = item.get("fingerprint")
                max_count = item.get("max_count")
                if not isinstance(fingerprint, str) or len(fingerprint) != 64:
                    errors.append(
                        f"allowances.unowned_debt[{index}].fingerprint must be a SHA-256 string"
                    )
                    continue
                if fingerprint in seen:
                    errors.append(f"duplicate debt fingerprint {fingerprint}")
                seen.add(fingerprint)
                if not isinstance(max_count, int) or isinstance(max_count, bool) or max_count < 1:
                    errors.append(
                        f"allowances.unowned_debt[{index}].max_count must be a positive integer"
                    )
        if not isinstance(conflicts, list):
            errors.append("allowances.potential_conflict_pairs must be a list")
        elif len(conflicts) != len(set(conflicts)):
            errors.append("allowances.potential_conflict_pairs contains duplicates")
    ...
    if not isinstance(expected_digest, str):
        ...
        errors.append("baseline integrity_digest does not match canonical content")
    return errors
...
def compare_to_baseline(
    *,
    ...
    if current_scope_digest != expected_scope_digest:
        return {
            ...
            "configuration_errors": [
                "scan-profile digest changed; baseline and current census are not comparable"
            ],
            "violations": [],
            "improvements": [],
        }
    ...
    all_fingerprints = sorted(set(current_counts) | set(allowed_counts))
            ...
            violations.append(
                {
                    ...
                    "message": ("unowned doctrine occurrence count exceeds grandfathered baseline"),
                }
            )
        ...
        violations.append(
            {
                ...
                "message": (
                    "new potential contradiction is absent from the grandfathered conflict baseline"
                ),
            }
        )
...
def tighten_baseline(
    *,
    ...
    old_conflicts = set(baseline["allowances"]["potential_conflict_pairs"])
    current_conflicts = _current_conflict_pairs(census)
    tightened = dict(baseline)
    tightened["allowances"] = {
        "unowned_debt": tightened_debt,
        "potential_conflict_pairs": sorted(old_conflicts & current_conflicts),
    }
    ...
    new_total = sum(int(item["max_count"]) for item in tightened_debt)
    if new_total > old_total:
        raise AssertionError("ratchet attempted to increase debt allowance")
    if not set(tightened["allowances"]["potential_conflict_pairs"]).issubset(old_conflicts):
        raise AssertionError("ratchet attempted to increase potential conflict allowance")
    return tightened
...
def _print_text_result(
    *,
    ...
    print(f"  normative candidates: {summary['normative_candidate_count']}")
    print(f"  current unowned debt: {summary['unowned_debt_count']}")
    print(f"  potential conflict pairs: {summary['potential_conflict_pair_count']}")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Enforce the no-new-hidden-doctrine ratchet.")
    parser.add_argument(
        "--root",
        type=Path,
        default=ROOT,
    ...
    operation.add_argument(
        "--tighten",
        action="store_true",
        help=("Monotonically reduce baseline allowances to current passing state."),
    )
    parser.add_argument(
        "--reason",
...
def main() -> int:
    ...
    try:
        ...
        if args.tighten:
            ...
            previous = sum(
                int(item["max_count"]) for item in baseline["allowances"]["unowned_debt"]
            )
            current = sum(
                int(item["max_count"]) for item in tightened["allowances"]["unowned_debt"]
            )
            print(
                f"Doctrine baseline tightened: {previous} -> {current} allowed legacy occurrences."
            )
            return 0
        comparison = compare_to_baseline(
            census=census,
