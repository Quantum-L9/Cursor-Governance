...
| hook | Plan prefetch (`plan-prefetch`), PR publish (`pr-publish`), PE/SGD ingest (`pe-sgd-ingest`), post-publish governance friction (`claude-governance-friction`, namespace-restricted to `cursor-governance`) | `cli.py --surface <name>` / `MemoryControlPlaneClient(surface=…)` | read / `write` / `ingest_candidate` per envelope | bounded; refused client-side outside `ops/config/memory-hook-envelopes.json` |
