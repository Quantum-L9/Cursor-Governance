...
class UserScopeSettingsTests(unittest.TestCase):
    ...
    def test_every_hook_registration_lands_in_user_scope(self) -> None:
        ...
        # The tenth is session_deps_cloud on SessionStart. It used to be called
        # BY the governance SessionStart hook, which meant a 20s dependency wait
        # ran inside that hook's 30s budget and the reporting it exists for was
        # killed before it emitted. SessionStart hooks run concurrently, so deps
        # now has its own registration and its own timeout.
