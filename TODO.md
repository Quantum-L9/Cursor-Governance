...
> **Correction (2026-08-22): the enforcement claim below is stale.**
> `validate_campaign_promotion.py:280-305` hard-fails on this manifest's digests,
> and `environment/program-execution/scripts/tests/test_validate_campaign_promotion.py`
> is **not** in the repo-root pytest ignore list (`ops/config/python-contract.json`),
> so it runs in the PR gate and in CI on every change. The artifact is therefore
> declared advisory — and so deliberately left out of auto-sync — while a
> default-suite test hard-enforces it. That contradiction is what produced the
> stale-hash failures in #263 (commit `19ee1044`) and a repeat of the same
> failure on 2026-08-22.
>
> **This needs a decision, and neither option is taken here:**
> (a) stop the promotion validator gating these digests, which restores the
> disposition recorded below and keeps `--pe-manifest` opt-in; or
> (b) treat the promotion validator as the "stated consumer with an integrity
> requirement" this entry asks for, fold the manifest into auto-sync, and rewrite
> this section. The two template manifests under `core/` had the same gap and
> were fixed by (b) — they are now generated by
> `sync_generated_artifacts.py --force` and attributed to the `l9-generated`
> merge driver.
